...
static bool psy_ui_component_has_flag(const psy_ui_Component*,
	psy_ui_ComponentFlags);
static void psy_ui_component_add_flag(psy_ui_Component*, psy_ui_ComponentFlags);
static void psy_ui_component_remove_flag(psy_ui_Component*,
	psy_ui_ComponentFlags);
...
psy_ui_Colour psy_ui_component_background_colour(const psy_ui_Component* self)
{
	...
	while (curr) {
		...
		if (colour.overlay != 0) {
			...
			if (!curr) {
				colour = psy_ui_style_background_colour(psy_ui_style_const(psy_ui_STYLE_ROOT));
			}
	...
	if (!curr) {
		base = psy_ui_style_background_colour(psy_ui_style_const(psy_ui_STYLE_ROOT));
	}
	return base;
}
...
psy_ui_Colour psy_ui_component_colour(const psy_ui_Component* self)
{
	...
	while (curr) {
		...
		if (colour.overlay != 0) {
			...
			if (!curr) {
				colour = psy_ui_style_colour(psy_ui_style_const(psy_ui_STYLE_ROOT));
			}
			return psy_ui_colour_overlayed(&colour, &overlaycolour,
				overlay / 100.0);
		}
	...
	if (!curr) {
		base = psy_ui_style_colour(psy_ui_style_const(psy_ui_STYLE_ROOT));
	}
	return base;
}
...
void psy_ui_replace_default_font(psy_ui_Component* main,
	const psy_ui_Font* font)
{
	if (main) {
		psy_ui_Style* root;
...
static void scrollto(psy_ui_Component*, psy_ui_RealPoint delta,
...
static void on_align(psy_ui_Component* self)
{
	psy_List* p;
	psy_List* q;
...
static void on_preferred_size(psy_ui_Component*, const psy_ui_Size* limit,
	psy_ui_Size* rv);
...
static void on_mouse_down(psy_ui_Component* self, psy_ui_MouseEvent* ev)
{
	...
	if (psy_ui_component_has_flag(self, psy_ui_COMPONENTFLAGS_DRAGGABLE)) {
		psy_ui_DragEvent* dragevent;

		dragevent = &psy_ui_app()->dragevent;
		psy_ui_event_settarget(&dragevent->mouse.event, self);
		dragevent->mouse = *ev;
		psy_ui_app_start_drag(psy_ui_app());
		self->vtable->ondragstart(self, dragevent);
		psy_signal_emit(&self->signal_drag_start, self, 1, dragevent);
	}
}
...
static void on_mouse_move(psy_ui_Component* self, psy_ui_MouseEvent* ev)
{
	...
	if (dragevent->active) {
		...
		if (psy_ui_event_default_prevented(&dragevent->mouse.event)) {
			psy_ui_component_set_cursor(self, psy_ui_CURSORSTYLE_GRAB);
...
static void onmouseenter(psy_ui_Component* self)
{
	...
	if (psy_strlen(self->tooltip) > 0) {
		psy_Logger* logger;

		logger = psy_ui_component_logger(self, psy_ui_LOG_TOOLTIP);
		if (logger) {
			psy_logger_output(logger, self->tooltip);
		}
	}
}
...
static void on_key_up(psy_ui_Component* self, psy_ui_KeyboardEvent* ev) { }
...
static void vtable_init(void)
{
	if (!vtable_initialized) {
		...
		vtable.onalign = on_align;
		vtable.onpreferredsize = on_preferred_size;
...
void psy_ui_component_init(psy_ui_Component* self, psy_ui_Component* parent, psy_ui_Component* view)
{
	...
	psy_ui_component_init_base(self);
	psy_ui_component_init_signals(self);
	if (parent && parent->aligner && parent->aligner->insertaligntype !=
		psy_ui_ALIGN_NONE) {
		psy_ui_component_set_align(self, parent->aligner->insertaligntype);
		psy_ui_component_set_margin(self, parent->aligner->insertmargin);
	}
...
bool draw_visible(const psy_ui_Component* self)
{
	...
	return self->imp->vtable->dev_draw_visible(self->imp);
}
...
void resize(psy_ui_Component* self, psy_ui_Size size)
{
	...
	if (!psy_ui_app()->alignvalid ||
		...
		if (self->scroll->overflow != psy_ui_OVERFLOW_HIDDEN) {
			psy_ui_component_update_overflow(self);
		}
	}
}
...
void setposition(psy_ui_Component* self, psy_ui_Point topleft,
	psy_ui_Size size)
{
	self->imp->vtable->dev_set_position(self->imp, topleft, size);
	if (!psy_ui_app()->alignvalid ||
...
void scrollto(psy_ui_Component* self, psy_ui_RealPoint delta,
	const psy_ui_RealRectangle* r)
{
	self->imp->vtable->dev_scrollto(self->imp, delta, r);
}
...
void psy_ui_component_init_signals(psy_ui_Component* self)
{
	...
	psy_signal_init(&self->signal_key_down);
	psy_signal_init(&self->signal_key_up);
	psy_signal_init(&self->signal_mouse_down);
	psy_signal_init(&self->signal_mouse_up);
	psy_signal_init(&self->signal_mouse_move);
	psy_signal_init(&self->signal_mouse_wheel);
	psy_signal_init(&self->signal_mouse_double_click);
	psy_signal_init(&self->signal_mouse_enter);
	psy_signal_init(&self->signal_mousehover);
	psy_signal_init(&self->signal_mouse_leave);
	...
	psy_signal_init(&self->signal_focus_lost);
	psy_signal_init(&self->signal_align);
	psy_signal_init(&self->signal_before_align);
	psy_signal_init(&self->signal_preferred_size_changed);
	psy_signal_init(&self->signal_command);
	psy_signal_init(&self->signal_select_section);
	psy_signal_init(&self->signal_scroll_range_changed);
	psy_signal_init(&self->signal_language_changed);
	psy_signal_init(&self->signal_drag_start);
	psy_signal_init(&self->signal_style_update);
	psy_signal_init(&self->signal_reparent);
}
...
void psy_ui_component_init_base(psy_ui_Component* self) {
	...
	psy_table_init_keysize(&self->loggers, 4);
	...
	self->ncpaint = FALSE;
	self->blitscroll = psy_ui_SCROLLREDRAW_BLIT;
	psy_ui_bitmap_init(&self->bufferbitmap);
	self->flags_ = psy_ui_COMPONENTFLAGS_NONE;
...
void psy_ui_component_dispose(psy_ui_Component* self)
{
	...
	psy_ui_app_stop_timer(psy_ui_app(), self, psy_INDEX_INVALID);
	psy_table_remove(&psy_ui_app()->components, (uintptr_t)self);
	psy_table_dispose(&self->loggers);
	free(self->tooltip);
	self->tooltip = NULL;
}
...
void psy_ui_component_dispose_signals(psy_ui_Component* self)
{
	...
	psy_signal_dispose(&self->signal_key_down);
	psy_signal_dispose(&self->signal_key_up);
	psy_signal_dispose(&self->signal_mouse_down);
	psy_signal_dispose(&self->signal_mouse_up);
	psy_signal_dispose(&self->signal_mouse_move);
	psy_signal_dispose(&self->signal_mouse_wheel);
	psy_signal_dispose(&self->signal_mouse_double_click);
	psy_signal_dispose(&self->signal_mouse_enter);
	psy_signal_dispose(&self->signal_mousehover);
	psy_signal_dispose(&self->signal_mouse_leave);
	...
	psy_signal_dispose(&self->signal_focus_lost);
	psy_signal_dispose(&self->signal_align);
	psy_signal_dispose(&self->signal_before_align);
	psy_signal_dispose(&self->signal_command);
	psy_signal_dispose(&self->signal_preferred_size_changed);
	psy_signal_dispose(&self->signal_select_section);
	psy_signal_dispose(&self->signal_scroll_range_changed);
	psy_signal_dispose(&self->signal_language_changed);
	psy_signal_dispose(&self->signal_drag_start);
	psy_signal_dispose(&self->signal_style_update);
	psy_signal_dispose(&self->signal_reparent);
}
	...
	if (stepy != 0 || stepx != 0) {
		self->vtable->scrollto(self,
			psy_ui_realpoint_make(
				psy_ui_component_scroll_step_width_px(self) * stepx,
				psy_ui_component_scroll_step_height_px(self) * stepy),
			NULL);
	}
}
...
void psy_ui_component_show_state(psy_ui_Component* self, int state)
{
	self->vtable->showstate(self, state);
}

void psy_ui_component_show_maximized(psy_ui_Component* self)
{
#ifdef DIVERSALIS__OS__MICROSOFT	
	psy_ui_component_show_state(self, SW_MAXIMIZE);
#else	
	psy_ui_component_show_state(self, 1);
#endif	
}
...
#else
	psy_ui_component_resize(self, psy_ui_size_make_px(1024.0, 768.0));
	psy_ui_component_show_state(self, 0);
#endif	
}
...
void psy_ui_component_set_position(psy_ui_Component* self, psy_ui_Rectangle position)
{
...
void psy_ui_component_align(psy_ui_Component* self)
{
	if (self->aligner) {
		...
		psy_signal_emit(&self->signal_before_align, self, 0);
...
void psy_ui_component_align_invalidate(psy_ui_Component* self)
{
	psy_ui_component_align(self);
	psy_ui_component_invalidate(self);
}
...
void on_preferred_size(psy_ui_Component* self, const psy_ui_Size* limit,
	psy_ui_Size* rv)
{
	if (self->aligner) {
...
void onpreferredscrollsize(psy_ui_Component* self, const psy_ui_Size* limit,
	psy_ui_Size* rv)
{
	on_preferred_size(self, limit, rv);
}
...
void psy_ui_component_set_maximum_size(psy_ui_Component* self, psy_ui_Size size)
{
...
void psy_ui_component_set_cursor(psy_ui_Component* self, psy_ui_CursorStyle style)
{
	self->imp->vtable->dev_set_cursor(self->imp, style);
}
...
static int dev_draw_visible(psy_ui_ComponentImp* self) { return 0; }
...
static void dev_set_position(psy_ui_ComponentImp* self, psy_ui_Point topleft, psy_ui_Size size) { }
...
static void dev_scrollto(psy_ui_ComponentImp* self, psy_ui_RealPoint delta,
	const psy_ui_RealRectangle* r) { }
...
static void dev_set_cursor(psy_ui_ComponentImp* self, psy_ui_CursorStyle cursorstyle) { }
static void dev_seticonressource(psy_ui_ComponentImp* self, intptr_t ressourceid) { }
...
static void dev_center(psy_ui_ComponentImp* self)
{
}
...
static void imp_vtable_init(void)
{
	if (!imp_vtable_initialized) {
		...
		imp_vtable.dev_draw_visible = dev_draw_visible;
		...
		imp_vtable.dev_set_position = dev_set_position;
		...
		imp_vtable.dev_set_cursor = dev_set_cursor;
		...
		imp_vtable.dev_center = dev_center;
...
psy_ui_Component* psy_ui_component_intersect(psy_ui_Component* self,
...
void psy_ui_component_set_scroll_redraw(psy_ui_Component* self,
	psy_ui_ScrollRedraw scroll_redraw)
{
	assert(self);

	self->blitscroll = scroll_redraw;
}
...
void psy_ui_component_set_scroll_left(psy_ui_Component* self, psy_ui_Value left)
{
	...
	if (psy_ui_component_draw_visible(self)) {
		...
		if (self->blitscroll == psy_ui_SCROLLREDRAW_FULL) {
			psy_ui_component_invalidate(self);
		}
		else if (self->blitscroll == psy_ui_SCROLLREDRAW_BLIT) {
			psy_ui_component_scroll_to(self, psy_ui_realpoint_make(
				newleft - position.left, 0.0), &r);
			psy_ui_component_invalidate_scroll_diff(self);
		}
	}
	psy_signal_emit_double(&self->signal_scrolled, self, 2,
		newleft - position.left, 0.0);
}
...
void psy_ui_component_set_scroll_top(psy_ui_Component* self, psy_ui_Value top)
{
	...
	if (psy_ui_component_draw_visible(self)) {
		...
		if (self->blitscroll == psy_ui_SCROLLREDRAW_FULL) {
			...
			psy_ui_component_scroll_to(self, psy_ui_realpoint_make(
				0.0, newtop - position.top), &r);
			psy_ui_component_invalidate_scroll_diff(self);
		}
	}
	psy_signal_emit_double(&self->signal_scrolled, self, 2, 0.0,
		newtop - position.top);
}

void psy_ui_component_invalidate_scroll_diff(psy_ui_Component* self)
{
	psy_ui_RealRectangle position;
	...
	client_position = psy_ui_realrectangle_make(psy_ui_realpoint_zero(),
		psy_ui_component_scroll_size_px(parent));
	position = psy_ui_component_position(self);
	psy_ui_realrectangle_intersection(&position, &client_position);
	diff = psy_ui_realrectangle_diff(&client_position, &position);
	for (p = diff; p != NULL; p = p->next) {
		psy_ui_RealRectangle* rc;

		rc = (psy_ui_RealRectangle*)p->entry;
		if (rc) {
			psy_ui_component_invalidate_rect(parent, *rc);
		}
	}
	p = NULL;
...
void psy_ui_component_update_overflow(psy_ui_Component* self)
{
	if (psy_ui_componentscroll_has_vertical_overflow(self->scroll)) {
	...
	if (psy_ui_componentscroll_has_horizontal_overflow(self->scroll)) {
		...
		size = psy_ui_component_scroll_size(psy_ui_component_parent(self));
		scrollsize = psy_ui_component_preferred_scrollsize(self, &size);
		scrollstepx_px = floor(psy_ui_component_scroll_step_width_px(self));
		tm = psy_ui_component_textmetric(self);
		currrow = lines(
			psy_ui_component_scroll_left_px(self),
...
void psy_ui_component_use_scroll(psy_ui_Component* self)
{
	if (self->scroll == &componentscroll) {
		self->scroll = psy_ui_componentscroll_alloc_init();
	}
}
...
bool psy_ui_component_stylestate_active(const psy_ui_Component* self,
	psy_ui_StyleState state)
{
	return psy_ui_componentstyle_state_active(&self->style, state);
}
...
void psy_ui_notify_style_update(psy_ui_Component* main)
{
	if (main) {
		...
		psy_signal_emit(&main->signal_style_update, main, 0);
		for (p = q = psy_ui_component_children(main, psy_ui_RECURSIVE);
			...
			psy_signal_emit(&child->signal_style_update, child, 0);
			if (child->imp) {
...
{
	if (!(psy_ui_component_has_flag(self, psy_ui_COMPONENTFLAGS_PREVENT_DRAW)))
	{
		...
		psy_ui_RealSize size;
		psy_Encoding restore_encoding;

		temp_g = NULL;
		oldclip = psy_ui_graphics_cliprect(g);
		if (psy_ui_component_has_flag(self, psy_ui_COMPONENTFLAGS_BUFERDRAW)) {
			if (psy_ui_bitmap_empty(&self->bufferbitmap)) {
				...
				psy_ui_graphics_draw_full_bitmap(g, &self->bufferbitmap,
					psy_ui_realpoint_zero());
				return;
			}
		}
		psy_ui_graphics_set_font(g, psy_ui_component_font(self));
		restore_encoding = self->encoding;
		psy_ui_graphics_set_encoding(g, self->encoding);
		size = psy_ui_component_scroll_size_px(self);
		...
		/* prepare colours */
		psy_ui_graphics_set_colour(g, psy_ui_component_colour(self));
		psy_ui_graphics_set_text_colour(g, psy_ui_component_colour(self));
		...
		psy_ui_graphics_set_encoding(g, restore_encoding);
	}
}
...
void psy_ui_component_draw_children(psy_ui_Component* self, psy_ui_Graphics* g)
{
	...
	if (q) {
		...
		for (p = q; p != NULL; p = p->next) {
			...
			component = (psy_ui_Component*)psy_list_entry(p);
			if (psy_ui_component_visible(component) &&
				psy_ui_component_is_light_weight(component)) {
				psy_ui_RealRectangle position;
...
{
	psy_ui_component_add_flag(self, psy_ui_COMPONENTFLAGS_BUFERDRAW);
}
...
void psy_ui_component_center(psy_ui_Component* self)
{
	assert(self);
	assert(self->imp);

	self->imp->vtable->dev_center(self->imp);
}

void psy_ui_component_start_timer(psy_ui_Component* self, uintptr_t id,
	uintptr_t interval)
{
	psy_ui_app_start_timer(psy_ui_app(), self, id, interval);
}

void psy_ui_component_stop_timer(psy_ui_Component* self, uintptr_t id)
{
	psy_ui_app_stop_timer(psy_ui_app(), self, id);
}
...
void psy_ui_component_set_logger(psy_ui_Component* self, uintptr_t group,
	psy_Logger* logger)
{
	assert(self);

	if (logger) {
		psy_table_insert(&self->loggers, group, logger);
	}
	else {
		psy_table_remove(&self->loggers, group);
	}
}

psy_Logger* psy_ui_component_logger(psy_ui_Component* self, uintptr_t group)
{
	psy_Logger* rv;

	assert(self);

	rv = psy_table_at(&self->loggers, group);
	if (rv) {
		return rv;
	}
	if (psy_ui_component_parent(self)) {
		return psy_ui_component_logger(psy_ui_component_parent(self), group);
	}
	return NULL;
}

bool psy_ui_component_has_flag(const psy_ui_Component* self,
	psy_ui_ComponentFlags flag)
{
	return (self->flags_ & flag) == flag;
}

void psy_ui_component_add_flag(psy_ui_Component* self,
	psy_ui_ComponentFlags flag)
{
	self->flags_ = self->flags_ | flag;
}

void psy_ui_component_remove_flag(psy_ui_Component* self,
	psy_ui_ComponentFlags flag)
{
	self->flags_ = self->flags_ & ~flag;
}

void psy_ui_component_enable_drag(psy_ui_Component* self)
{
	psy_ui_component_add_flag(self, psy_ui_COMPONENTFLAGS_DRAGGABLE);
}

void psy_ui_component_disable_drag(psy_ui_Component* self)
{
	psy_ui_component_remove_flag(self, psy_ui_COMPONENTFLAGS_DRAGGABLE);
}

void psy_ui_component_set_tooltip(psy_ui_Component* self, const char* text)
{
	assert(self);

	psy_strreset(&self->tooltip, text);
}

bool psy_ui_component_app_focus_out_prevented(const psy_ui_Component* self)
{
	return psy_ui_component_has_flag(self,
		psy_ui_COMPONENTFLAGS_PREVENT_APP_FOCUS_OUT);
}

void psy_ui_component_prevent_app_focus_out(psy_ui_Component* self)
{
	assert(self);

	psy_ui_component_add_flag(self,
		psy_ui_COMPONENTFLAGS_PREVENT_APP_FOCUS_OUT);
}

void psy_ui_component_prevent_app_focus_out_recursive(psy_ui_Component* self)
{
	psy_List* q;
	psy_List* p;

	psy_ui_component_prevent_app_focus_out(self);
	q = psy_ui_component_children(self, TRUE);
	for (p = q; p != NULL; p = p->next) {
		psy_ui_Component* curr;

		curr = (psy_ui_Component*)p->entry;
		psy_ui_component_prevent_app_focus_out(curr);
	}
	psy_list_free(q);
	q = NULL;
}

void psy_ui_component_enable_app_focus_out(psy_ui_Component* self)
{
	assert(self);

	psy_ui_component_remove_flag(self,
		psy_ui_COMPONENTFLAGS_PREVENT_APP_FOCUS_OUT);
}
