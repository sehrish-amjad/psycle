...
void psy_ui_fontinfo_string(const psy_ui_FontInfo* self, char* rv,
	uintptr_t max_len)
{
	psy_snprintf(rv, max_len, "%s;%g%s%s", self->lfFaceName, self->lfHeight, 
		(self->lfItalic)
		? ";italic"
		: "",
		(self->lfUnderline)
...
void psy_ui_font_init_string(psy_ui_Font* self, const char* str)
{	
	...
	psy_ui_font_init(self, &font_info);
}
...
psy_ui_FontInfo psy_ui_font_font_info(const psy_ui_Font* self)
{
