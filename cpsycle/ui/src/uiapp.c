...
#elif PSYCLE_USE_TK == PSYCLE_TK_TEXT
#include "imps/text/uitextapp.h"
#include "imps/text/uitextimpfactory.h"
#elif PSYCLE_USE_TK == PSYCLE_TK_EMPTY
#include "imps/empty/uiemptyapp.h"
#include "imps/empty/uiemptyimpfactory.h"
#else
...
void psy_ui_appzoom_set_rate(psy_ui_AppZoom* self, double rate)
{
	assert(self);
...
void psy_ui_appzoom_update_base_font_size(psy_ui_AppZoom* self,
	const psy_ui_Font* basefont)
{
	...
	fontinfo = psy_ui_font_font_info(basefont);
...
void ui_app_init_imp_factory(psy_ui_App* self)
{
	...
	self->impfactory = (psy_ui_ImpFactory*)
		psy_ui_win_impfactory_alloc_init();
#elif PSYCLE_USE_TK == PSYCLE_TK_CURSES
	/* todo */
#elif PSYCLE_USE_TK == PSYCLE_TK_X11	
	self->impfactory = (psy_ui_ImpFactory*)
		psy_ui_x11_impfactory_alloc_init();
#elif PSYCLE_USE_TK == PSYCLE_TK_TEXT
	self->impfactory = (psy_ui_ImpFactory*)
		psy_ui_text_impfactory_alloc_init();
#elif PSYCLE_USE_TK == PSYCLE_TK_EMPTY
	self->impfactory = (psy_ui_ImpFactory*)
		psy_ui_empty_impfactory_alloc_init();
#else
	#error "Platform not supported"
#endif	
	if (!self->impfactory) {
...
{	
	...
	psy_ui_icons_dispose(&self->icons);
	psy_ui_clipboard_dispose(&self->clipboard);
}

void psy_ui_app_set_main(psy_ui_App* self, psy_ui_Component* main)
{
	self->main = main;
}
...
void psy_ui_app_notify_theme_change(psy_ui_App* self)
{
	assert(self);
	...
	psy_ui_notify_style_update(psy_ui_app_main(self));	
}
...
int psy_ui_app_wait(psy_ui_App* self)
{
	assert(self);

	if (self->imp) {
		return self->imp->vtable->dev_wait(self->imp);
	}
	return PSY_ERRRUN;
}

int psy_ui_app_continue(psy_ui_App* self)
{
	assert(self);

	if (self->imp) {
		return self->imp->vtable->dev_continue(self->imp);
	}
	return PSY_ERRRUN;
}
...
void psy_ui_app_start_mouse_hook(psy_ui_App* self)
{
	assert(self);
...
void psy_ui_app_stop_mouse_hook(psy_ui_App* self)
{
	assert(self);
...
{
	psy_ui_appzoom_set_rate(&self->zoom, rate);
	psy_ui_app_change_default_font_size(self,
		(int)(self->zoom.basefontsize * rate));
}

double psy_ui_app_zoom_rate(const psy_ui_App* self)
{
...
void psy_ui_app_change_default_font_size(psy_ui_App* self, double size)
{
	...
	fontinfo = psy_ui_font_font_info(&psy_ui_style_const(
	...
	for (; r != NULL; psy_list_next(&r)) {
		...
		psy_ui_replace_default_font(curr_toplevel, &font);
		psy_ui_component_invalidate(curr_toplevel);
	}
	psy_list_free(t);
...
{	
	...
	psy_ui_appzoom_update_base_font_size(&self->zoom, font);
	if (self->zoom.rate != 1.0) {
		font_info = psy_ui_font_font_info(font);
		font_info.lfHeight = (int)(self->zoom.basefontsize *
			self->zoom.rate);
		psy_ui_font_init(&zoomed_font, &font_info);
		psy_ui_replace_default_font(self->main, &zoomed_font);
		psy_ui_font_dispose(&zoomed_font);
	} else {
		psy_ui_replace_default_font(self->main, font);
	}
	psy_ui_component_invalidate(self->main);
}

void psy_ui_app_light_theme(psy_ui_App* self)
{
	assert(self);

	psy_ui_defaults_init_theme(&self->defaults, psy_ui_LIGHTTHEME, TRUE);
	if (self->imp) {
		self->imp->vtable->dev_onappdefaultschange(self->imp);
	}
}

void psy_ui_app_dark_theme(psy_ui_App* self)
{
	assert(self);

	psy_ui_defaults_init_theme(&self->defaults, psy_ui_DARKTHEME, TRUE);
	if (self->imp) {
		self->imp->vtable->dev_onappdefaultschange(self->imp);
	}
}

bool psy_ui_app_has_dark_theme(const psy_ui_App* self)
{
	assert(self);
...
void psy_ui_app_remove_capture(psy_ui_App* self, psy_ui_Component* component)
{
	...
	p = psy_list_find_entry(self->captures_, component);
	if (p) {
		psy_list_remove(&self->captures_, p);
	}	
}
...
static int psy_ui_appimp_wait(psy_ui_AppImp* self) { return PSY_ERRRUN; }
static int psy_ui_appimp_continue(psy_ui_AppImp* self) { return PSY_ERRRUN; }
static void psy_ui_appimp_stop(psy_ui_AppImp* self) { }
static void psy_ui_appimp_close(psy_ui_AppImp* self) { }
static void psy_ui_appimp_on_app_defaults_change(psy_ui_AppImp* self) { }
static void psy_ui_appimp_start_mouse_hook(psy_ui_AppImp* self) { }
static void psy_ui_appimp_stop_mouse_hook(psy_ui_AppImp* self) { }
static void psy_ui_appimp_send_event(psy_ui_AppImp* self,
...
static void imp_vtable_init(void)
{
	if (!imp_vtable_initialized) {
		...
		imp_vtable.dev_wait = psy_ui_appimp_wait;
		imp_vtable.dev_continue = psy_ui_appimp_continue;
		imp_vtable.dev_stop = psy_ui_appimp_stop;
		imp_vtable.dev_close = psy_ui_appimp_close;	
		imp_vtable.dev_onappdefaultschange = psy_ui_appimp_on_app_defaults_change;
		imp_vtable.dev_startmousehook = psy_ui_appimp_start_mouse_hook;
		imp_vtable.dev_stopmousehook = psy_ui_appimp_stop_mouse_hook;
		imp_vtable.dev_sendevent = psy_ui_appimp_send_event;
...
void psy_ui_app_start_drag(psy_ui_App* self)
{
	self->dragevent.active = TRUE;
	psy_ui_component_set_cursor(self->main, psy_ui_CURSORSTYLE_GRAB);
}

void psy_ui_app_stop_drag(psy_ui_App* self)
{
...
void psy_ui_app_start_timer(psy_ui_App* self, psy_ui_Component* component, uintptr_t id,
	uintptr_t interval)
{	
	psy_timers_add_timer(&self->wintimers, (uintptr_t)component, component,
		(psy_fp_timerwork)component->vtable->on_timer, 
		&component->signal_timer, id, interval);
}

void psy_ui_app_stop_timer(psy_ui_App* self, psy_ui_Component* component, uintptr_t id)
{
	psy_timers_remove_timer(&self->wintimers, (uintptr_t)component, id);
}
	...
	_In_ PSTR szCmdLine, _In_ int iCmdShow)
{
	...
	err = psycle_run((uintptr_t)hInstance, iCmdShow, __argc, __argv);
