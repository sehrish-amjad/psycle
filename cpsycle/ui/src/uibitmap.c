...
void psy_ui_bitmap_init_size(psy_ui_Bitmap* self, psy_ui_RealSize size)
{
	vtable_init();
	self->vtable = &vtable;
	self->imp_ = psy_ui_impfactory_allocinit_bitmapimp(
...
{
	if (self->imp_) {
		self->imp_->vtable->dev_dispose(self->imp_);
		free(self->imp_);
		self->imp_ = NULL;
	}
}
...
{
	createimp(self);
	return self->imp_->vtable->dev_load(self->imp_, self, path);
}
...
{
	if (self->imp_) {
		return self->imp_->vtable->dev_size(self->imp_);
	}
...
bool empty(const psy_ui_Bitmap* self)
{
	...
	createimp(self);
	return self->imp_->vtable->dev_empty(self->imp_);
}

void settransparency(psy_ui_Bitmap* self, psy_ui_Colour colour)
{
	createimp(self);
	self->imp_->vtable->dev_settransparency(self->imp_, colour);
}
...
{	
	if (!other->imp_) {
		...
		createimp(self);
		self->imp_->vtable->dev_copy(self->imp_, other->imp_);
	}
}
...
{	
	if (self->imp_) {
		return self->imp_->vtable->dev_native(self->imp_);
	}
	return 0;
}
...
{
	if (self->imp_) {
		return self->imp_->vtable->dev_native_mask(self->imp_);
	}
	return 0;
}
