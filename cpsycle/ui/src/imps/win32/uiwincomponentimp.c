...
static int dev_draw_visible(psy_ui_win_ComponentImp*);
...
static void dev_set_position(psy_ui_win_ComponentImp*, psy_ui_Point topleft,
	psy_ui_Size);
...
static void dev_scrollto(psy_ui_win_ComponentImp*, psy_ui_RealPoint delta,
...
static void dev_set_cursor(psy_ui_win_ComponentImp*, psy_ui_CursorStyle);
...
static void dev_center(psy_ui_win_ComponentImp*);
...
void dev_center(psy_ui_win_ComponentImp* self)
{
	psy_ui_RealRectangle position;
	psy_ui_RealSize size;

	position = dev_position(self);
	size = psy_ui_realrectangle_size(&position);
	dev_move(self, psy_ui_point_make_px(
...
int dev_draw_visible(psy_ui_win_ComponentImp* self)
{
	return IsWindowVisible(self->hwnd);
}
...
void dev_set_position(psy_ui_win_ComponentImp* self, psy_ui_Point topleft,
	psy_ui_Size size)
{
...
void dev_scrollto(psy_ui_win_ComponentImp* self, psy_ui_RealPoint delta,
	const psy_ui_RealRectangle* r)
{
	...
	ScrollWindow(self->hwnd, (int)delta.x, (int)delta.y, prc, prc);
}
...
void dev_set_cursor(psy_ui_win_ComponentImp* self, psy_ui_CursorStyle
	cursorstyle)
{
	HCURSOR hc;
