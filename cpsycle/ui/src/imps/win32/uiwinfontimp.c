...
{	
	...
	if (text) {
		...
		out = (char_dyn_t*)malloc((count + 1) * sizeof(char_dyn_t));
		if (out == NULL) {
			assert(0);
		...
		inlen = psy_min(psy_strlen(text), count) + 1;
		if (UTF8Toisolat1(out, &outlen, text, &inlen) == PSY_OK) {
...
LOGFONT logfont(psy_ui_FontInfo lf)
{
	...
	rv.lfHeight = (LONG)lf.lfHeight;
	rv.lfWidth = (LONG)lf.lfWidth;
