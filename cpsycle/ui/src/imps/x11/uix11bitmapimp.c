...
static int load(psy_ui_x11_BitmapImp*, struct psy_ui_Bitmap* bitmap,
	const char* path);
static void copy(psy_ui_x11_BitmapImp*, const struct psy_ui_x11_BitmapImp*
	other);
static int loadresource(psy_ui_x11_BitmapImp*, int resourceid);
static psy_ui_RealSize dev_size(const psy_ui_x11_BitmapImp*);
...
void copy(psy_ui_x11_BitmapImp* self, const struct psy_ui_x11_BitmapImp* other)
{
	psy_ui_RealSize size;
	
	dispose(self);
	size = dev_size(other);	
	if (!other->pixmap || (size.width == 0 && size.height == 0)) {
		self->pixmap = 0;
	} else {
		psy_ui_X11App* x11app;
		GC gc;
		...
		XCopyArea(x11app->dpy, other->pixmap, self->pixmap, gc,
			0, 0,
...
psy_ui_RealSize dev_size(const psy_ui_x11_BitmapImp* self)
{
	psy_ui_RealSize size;    
...
uint8_t* psy_ui_x11_bitmapimp_make_x11_wm_icon(
	psy_ui_x11_BitmapImp* self, uintptr_t* amount)
{
	uint8_t* rv;
	psy_ui_RealSize size;
	psy_ui_X11App* x11app;
	...
	*amount = width * height + 2;	
	rv = (uint8_t*)malloc((*amount) * sizeof(uint32_t));	
	rv[0] = width;
	rv[4] = height;  
	i = 8;
	for(y = 0; y < height; y++) {
