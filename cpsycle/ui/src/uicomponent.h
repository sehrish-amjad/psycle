...
#define psy_ui_LOG_TERMINAL  0
#define psy_ui_LOG_STATUS  1
#define psy_ui_LOG_TOOLTIP 2
#define psy_ui_LOG_PROGRESS 3

	struct psy_ui_Component;


	typedef enum psy_ui_ScrollRedraw {
		psy_ui_SCROLLREDRAW_NONE = 0,
		psy_ui_SCROLLREDRAW_FULL = 1,
		psy_ui_SCROLLREDRAW_BLIT = 2
	} psy_ui_ScrollRedraw;
...
#define psy_ui_POPUP		3
	...
	typedef void (*psy_ui_fp_component_scrollto)(struct psy_ui_Component*,
		psy_ui_RealPoint, const psy_ui_RealRectangle*);
	...
	typedef enum psy_ui_ComponentFlags {
		psy_ui_COMPONENTFLAGS_NONE = 0,
		psy_ui_COMPONENTFLAGS_PREVENT_DRAW = 1,
		psy_ui_COMPONENTFLAGS_BUFERDRAW = 2,
		psy_ui_COMPONENTFLAGS_DRAGGABLE = 4,
		psy_ui_COMPONENTFLAGS_PREVENT_APP_FOCUS_OUT = 8,
		psy_ui_COMPONENTFLAGS_PREVENT_MOUSE_INPUT = 16
	...
	typedef struct psy_ui_Component {
		...
		psy_Signal signal_key_down;
		psy_Signal signal_key_up;
		psy_Signal signal_mouse_down;
		psy_Signal signal_mouse_up;
		psy_Signal signal_mouse_move;
		psy_Signal signal_mouse_wheel;
		psy_Signal signal_mouse_double_click;
		psy_Signal signal_mouse_enter;
		psy_Signal signal_mousehover;
		psy_Signal signal_mouse_leave;
		...
		psy_Signal signal_before_align;
		psy_Signal signal_select_section;
		psy_Signal signal_command;
		psy_Signal signal_focus_lost;
		psy_Signal signal_focus;
		psy_Signal signal_preferred_size_changed;
		psy_Signal signal_scroll_range_changed;
		psy_Signal signal_language_changed;
		psy_Signal signal_drag_start;
		psy_Signal signal_style_update;
		psy_Signal signal_reparent;
		/*! @internal */
		uintptr_t id;
		uintptr_t dbg;
		bool doublebuffered;
	...
	void psy_ui_component_show_align(psy_ui_Component*);
	void psy_ui_component_hide(psy_ui_Component*);
	void psy_ui_component_hide_align(psy_ui_Component*);
	void psy_ui_component_show_state(psy_ui_Component*, int cmd);
	void psy_ui_component_show_maximized(psy_ui_Component*);
	...
	void psy_ui_component_use_scroll(psy_ui_Component*);
	...
	{
		psy_ui_component_use_scroll(self);
		range.y = psy_max(range.x, range.y);
		if ((self->scroll->vrange.x != range.x) ||
			(self->scroll->vrange.y != range.y)) {
			psy_ui_componentscroll_setvrange(self->scroll, range);
			psy_signal_emit(&self->signal_scroll_range_changed, self, 1,
				psy_ui_VERTICAL);
		}
	}
		...
		psy_ui_IntPoint range)
	{
		psy_ui_component_use_scroll(self);
		range.y = psy_max(range.x, range.y);
		if ((self->scroll->hrange.x != range.x) ||
			(self->scroll->hrange.y != range.y)) {
	...
	void psy_ui_component_set_position(psy_ui_Component*, psy_ui_Rectangle position);
	...
	void psy_ui_component_align_invalidate(psy_ui_Component*);
	...
	psy_ui_RealSize psy_ui_component_preferredscrollsize_px(psy_ui_Component*,
		const psy_ui_Size* limit);
	void psy_ui_component_set_maximum_size(psy_ui_Component*, psy_ui_Size);
	...
	void psy_ui_component_set_cursor(psy_ui_Component*, psy_ui_CursorStyle);
	...
	void psy_ui_component_center(psy_ui_Component*);
	...
	typedef int (*psy_ui_fp_componentimp_dev_draw_visible)(
		struct psy_ui_ComponentImp*);
	...
	typedef psy_ui_RealRectangle(*psy_ui_fp_componentimp_dev_screenposition)(
		const struct psy_ui_ComponentImp*);
	typedef void (*psy_ui_fp_componentimp_dev_set_position)(
	...
	typedef void (*psy_ui_fp_componentimp_dev_scrollto)(
		struct psy_ui_ComponentImp*, psy_ui_RealPoint,
	...
	typedef void (*psy_ui_fp_componentimp_dev_center)(
		struct psy_ui_ComponentImp*);
	...
	typedef struct psy_ui_ComponentImpVTable {
		...
		psy_ui_fp_componentimp_dev_draw_visible dev_draw_visible;
		...
		psy_ui_fp_componentimp_dev_set_position dev_set_position;
		...
		psy_ui_fp_componentimp_dev_set_cursor dev_set_cursor;
		...
		psy_ui_fp_componentimp_dev_center dev_center;
	...
	void psy_ui_component_invalidate_scroll_diff(psy_ui_Component*);

	void psy_ui_component_clear(psy_ui_Component*);

	/* returns the elements size that includes padding and border */
	...
	*/
	...
	psy_ui_Colour psy_ui_component_colour(const psy_ui_Component*);
	void psy_ui_component_set_background_colour(psy_ui_Component*, psy_ui_Colour);
	psy_ui_Colour psy_ui_component_background_colour(const psy_ui_Component*);
		...
		psy_ui_Component* parent)
	{
		...
		psy_signal_emit(&self->signal_reparent, self, 0);
	}
	...
	INLINE void psy_ui_component_set_order(psy_ui_Component* self,
		psy_ui_Component* insertafter)
	{
		self->imp->vtable->dev_setorder(self->imp, insertafter->imp);
	}

	INLINE void psy_ui_component_set_wheel_scroll(psy_ui_Component* self,
		uintptr_t lines)
	{
		psy_ui_component_use_scroll(self);
		psy_ui_componentscroll_set_wheel(self->scroll, lines);
	}

	INLINE uintptr_t psy_ui_component_wheel_scroll(const psy_ui_Component* self)
	{
	...
	{
		psy_signal_emit(&self->signal_select_section, self, 2, section, options);
	}

	void psy_ui_component_set_scroll_redraw(psy_ui_Component*, psy_ui_ScrollRedraw);
	...
	INLINE psy_ui_RealPoint psy_ui_component_scroll_px(const psy_ui_Component* self)
	{
		psy_ui_RealRectangle position;

		position = psy_ui_component_position(self);
		return psy_ui_realpoint_make(-floor(position.left), -floor(position.top));
	}


	void psy_ui_component_update_overflow(psy_ui_Component*);
	...
	INLINE double psy_ui_component_scroll_step_width_px(const psy_ui_Component* self)
	{
	...
	INLINE void psy_ui_component_scroll_to(psy_ui_Component* self,
		psy_ui_RealPoint delta, const psy_ui_RealRectangle* r)
	{
		self->vtable->scrollto(self, delta, r);
	}
	...
	void psy_ui_component_prevent_app_focus_out(psy_ui_Component*);
	void psy_ui_component_prevent_app_focus_out_recursive(psy_ui_Component*);
	void psy_ui_component_enable_app_focus_out(psy_ui_Component*);
	bool psy_ui_component_app_focus_out_prevented(const psy_ui_Component*);


	INLINE bool psy_ui_component_is_light_weight(const psy_ui_Component* self)
	{
		return ((self->imp->vtable->dev_flags(self->imp)
			& psy_ui_COMPONENTIMPFLAGS_HANDLECHILDREN));
	}

	INLINE void psy_ui_component_set_encoding(psy_ui_Component* self,
		psy_Encoding encoding)
	{
		assert(self);
	...
	void psy_ui_component_set_logger(psy_ui_Component*, uintptr_t group,
		psy_Logger*);
	psy_Logger* psy_ui_component_logger(psy_ui_Component*, uintptr_t group);


	void psy_ui_component_enable_drag(psy_ui_Component*);
	void psy_ui_component_disable_drag(psy_ui_Component*);

	void psy_ui_component_set_tooltip(psy_ui_Component*, const char* text);
