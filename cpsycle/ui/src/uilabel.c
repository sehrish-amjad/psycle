...
void psy_ui_label_init(psy_ui_Label* self, psy_ui_Component* parent)
{
	...
	self->fade_r = self->fade_g = self->fade_b = 0.0;	
	self->property = NULL;
...
void psy_ui_label_exchange(psy_ui_Label* self, psy_Property* property,
...
void psy_ui_label_on_property_changed(psy_ui_Label* self, psy_Property* sender)
{
	...
	if (self->property_display_mode == psy_ui_PROPERTY_MODE_VALUE) {
		if (psy_property_is_font(self->property)) {
			...
			psy_snprintf(text, 40, "%f", psy_property_item_double(
				self->property));
			psy_ui_label_set_text(self, text);
		}
		...
		psy_snprintf(text, 256, "%s %d %s %d",
			...
			(int)psy_property_item_min(self->property),
			psy_ui_translate("settings.to"),
			(int)psy_property_item_max(self->property));
		psy_ui_label_set_text(self, text);		
	}
}
	...
	if (psy_strlen(text) == 0) {		
		...
		if (psy_ui_textformat_has_word_wrap(&self->format)) {
			if (self->charnumber == 0 && limit) {
				psy_ui_textformat_calc(&self->format, text,
				...
				psy_ui_textformat_calc(&self->format, text,
					self->charnumber * tm->tmAveCharWidth, font);
			}			
			rv->height = psy_ui_value_make_px(
				psy_ui_textformat_num_lines(&self->format) *
...
void psy_ui_label_on_draw(psy_ui_Label* self, psy_ui_Graphics* g)
{
	...
	if (self->fadeoutcounter > 0) {				
		psy_ui_graphics_set_text_colour(g, psy_ui_label_fade_colour(self));
	}	
	...
	psy_ui_Alignment alignment)
{
	...
	psy_ui_textformat_set_alignment(&self->format, alignment);
}
...
psy_ui_Colour psy_ui_label_fade_colour(const psy_ui_Label* self)
{
	...
	if (self->fadeoutcounter > 0 && self->fadeoutcounter <= 80) {
		...
		step = (double)(80 - self->fadeoutcounter);
