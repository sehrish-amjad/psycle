...
{  	
	...
	vtable_init(self);
	self->property = NULL;
	psy_ui_component_init(&self->checkmarkbar, psy_ui_checkbox_base(self), NULL);	
	psy_ui_component_set_align(&self->checkmarkbar, psy_ui_ALIGN_LEFT);
	psy_ui_component_init(&self->checkmark, &self->checkmarkbar, NULL);	
	psy_ui_component_set_align(&self->checkmark, psy_ui_ALIGN_CENTER);
...
void psy_ui_checkbox_init_text_connect(psy_ui_CheckBox* self,
	psy_ui_Component* parent, const char* text, void* context, void* fp)
{
	psy_ui_checkbox_init_text(self, parent, text);
...
{
	psy_ui_checkbox_init(self, parent);
	psy_ui_checkbox_exchange(self, property);
}
...
void psy_ui_checkbox_exchange(psy_ui_CheckBox* self,
