...
#define TWOPI            2.0*psy_dsp_PI_F // math::pi
	...
	if(AmpEnvStage)
	{
		...
		if ( syntp->interpolate)  // helper's interpolation method
		{
			if ( syntp->wave1noise) {
				...
				output = (psy_rand16() - 16384)*OSC1Vol;				
			} else {
				output = psy_dsp_resampler_work_float(
					psy_dsp_multiresampler_base(&resampler),
					syntp->pWave, OSC1Position, iwaveTableSize, syntp->pWave, syntp->pWave+iwaveTableSize-1)*OSC1Vol;
			}
			if ( syntp->wave2noise) {
				//This assumes MAX_RAND is 0x7fff
				//output += (std::rand() - 16384)*OSC2Vol;
				output += (psy_rand16() - 16384)*OSC2Vol;				
			}
			else {
                output += psy_dsp_resampler_work_float(
					psy_dsp_multiresampler_base(&resampler), 
					syntp->pWave2, OSC2Position, iwaveTableSize, syntp->pWave2, syntp->pWave2+iwaveTableSize-1)*OSC2Vol;
			}
		}
		else
		{			
