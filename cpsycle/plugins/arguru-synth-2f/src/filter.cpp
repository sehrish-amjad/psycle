...
void filter::SetFilter_4PoleLP(int CurCutoff, int Resonance)
{
	...
	if (cf>=sr/2) cf= (float)(sr/2); // próba wprowadzenia nieliniowoœci przy koñcu charakterystyki
	...
	B=fB*ncf*ncf;
	a0=float(1/(1+A+B));
	Biquad2.m_b1=2*(Biquad2.m_b2=Biquad2.m_b0= (float)(0.35*a0));// obliczenie wspó³czynników filtru cyfrowego (przekszta³cenie dwuliniowe)
...
void filter::SetFilter_4PoleEQ1(int CurCutoff, int Resonance)
{
	float CutoffFreq=(float)(264*pow(32.,CurCutoff/240.));
	float cf=(float)CutoffFreq;
	if (cf>= (float)(sr/2)) cf= (float)(sr/2); // próba wprowadzenia nieliniowoœci przy koñcu charakterystyki
	if (cf<33) cf=(float)(33.0);
	// not used: float ScaleResonance=1.0;
	// not used: float fQ=(float)(1.01+30*Resonance*ScaleResonance/240.0);
	Biquad.SetParametricEQ(cf,(float)(1.0+Resonance/12.0),float(6+Resonance/30.0),(float)sr,0.4f/(1+(240-Resonance)/120.0f));
	Biquad2.SetParametricEQ(float(cf/(1+Resonance/240.0)),float(1.0+Resonance/12.0),float(6+Resonance/30.0), (float)sr,0.4f);
}
...
void filter::SetFilter_4PoleEQ2(int CurCutoff, int Resonance)
{
	...
	// not used: float ScaleResonance=1.0;
	// not used: float fQ=(float)(1.01+30*Resonance*ScaleResonance/240.0);
	Biquad.SetParametricEQ(cf,8.0f,9.0f, (float)sr,0.5f);
	Biquad2.SetParametricEQ(float(cf/(3.5-2*Resonance/240.0)),8.0f,9.0f, (float)sr,0.4f);
}
...
void filter::SetFilter_Vocal1(int CurCutoff, int Resonance)
{
	...
	Biquad.SetParametricEQ(Cutoff1,2.0f+Resonance/48.0f,6.0f+Resonance/24.0f, (float)sr,0.3f);
	Biquad2.SetParametricEQ(Cutoff2,2.0f+Resonance/48.0f,6.0f+Resonance/24.0f,(float)sr,0.3f);
}
...
void filter::SetFilter_Vocal2(int CurCutoff, int Resonance)
{
	...
	Biquad.SetParametricEQ(Cutoff1,2.0f+Resonance/56.0f,6.0f+Resonance/16.0f, (float)sr,0.3f);
	Biquad2.SetParametricEQ(Cutoff2,2.0f+Resonance/56.0f,6.0f+Resonance/16.0f,(float)sr,0.3f);
}
