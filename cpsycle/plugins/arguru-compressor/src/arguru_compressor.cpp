...
void mi::Work(float *psamplesleft, float *psamplesright , int numsamples, int tracks) {
	...
	if (Vals[paramClip] != 0) {
		...
		float* pright = psamplesright;
		for (int cont = numsamples; cont > 0; cont--) {
			*pleft = (float)tanh(*pleft);
			*pright = (float)tanh(*pright);
			pleft++;
			pright++;
		}
	}
...
bool mi::DescribeValue(char* txt,int const param, int const value) {
	switch(param) {
	case paramGain:
		std::sprintf(txt,"+%.1f dB", psy_dsp_convert_amp_to_db(1 + value * 0.015625f));
		return true;
	case paramThreshold:
		if(value == 0)
			std::sprintf(txt,"-inf. dB");
		else
			std::sprintf(txt,"%.1f dB", psy_dsp_convert_amp_to_db(value * 0.0078125f));
		return true;
	case paramRatio:
		if(value >= 16)
