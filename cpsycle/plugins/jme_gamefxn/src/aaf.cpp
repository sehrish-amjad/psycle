...
{ 
	...
	{ double theta = (order & 1) ? (i* psy_dsp_PI) / order : ((i+0.5)* psy_dsp_PI) / order;
...
{ 
	...
	double 	warped_alpha = tan(psy_dsp_PI * raw_alpha) / psy_dsp_PI;

	double w1 = 2.0* psy_dsp_PI * warped_alpha;
	switch(mode) {
...
void AAF16::expandpoly(double raw_alpha, mode_t mode)
{
	...
	for (int i = 0; i <= zplane.numzeros; i++) xcoef[i] = (float)(+(topcoeffs[i].real() / botcoeffs[zplane.numpoles].real()));
	for (int i = 0; i <= zplane.numpoles; i++) ycoef[i] = (float)(-(botcoeffs[i].real() / botcoeffs[zplane.numpoles].real()));
	...
	case lowpass:
		{ 
			...
			for (int i = 0; i <= zplane.numzeros; i++) xcoef[i] /= (float)gain;
			break;
		}
	case highpass:	
		{
