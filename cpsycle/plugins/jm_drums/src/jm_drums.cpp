...
void mi::SequencerTick()
{
	if(currentSR != pCB->GetSamplingRate())
	{
		...
		globalpar.DecLength=Vals[2] * pCB->GetSamplingRate() / 1000.f;
		...
		if ( Vals[8] == 0 ) {
			globalpar.DecayDec=(float)((Vals[3]-Vals[4])*globalpar.sinmix)/(globalpar.SLength*100.f);
		}
		else {
			if ( Vals[10] != 0 ) { // If it's 0, they don't change.
				globalpar.AttackPos=globalpar.SLength*Vals[10]/100;
				globalpar.AttackInc=(float)((327.67*globalpar.sinmix)/globalpar.AttackPos);
			}
			if ( Vals[10] > Vals[11] ) globalpar.DecayPos=globalpar.AttackPos+1;
			else globalpar.DecayPos=globalpar.SLength*Vals[11]/100;
			globalpar.DecayDec= (float)(((327.67-(3.2767*Vals[12]))*globalpar.sinmix)/(globalpar.DecayPos-globalpar.AttackPos));
			globalpar.SustainDec= (float)((3.2767*Vals[12]*globalpar.sinmix)/(globalpar.SLength-globalpar.DecayPos));
		}
...
void mi::ParameterTweak(int par, int val)
{
	...
	switch (par) {
		...
		case 1:
			...
			break;
		case 2:
			globalpar.DecLength= (float)(val * pCB->GetSamplingRate() / 1000.0);
			globalpar.IncSpeed= (Vals[1] - Vals[0]) * (float)(MAX_ENVPOS)/ (globalpar.DecLength*pCB->GetSamplingRate());
			break;
		case 3:
			if ( Vals[8] == 0 ) {
				globalpar.SinVol= (float)(val*globalpar.sinmix/100.0);
				globalpar.DecayDec= (float)(((val-Vals[4])*globalpar.sinmix)/(globalpar.SLength*100.0));
			}
			break;
		case 4:
			if ( Vals[8] == 0 ) {
				globalpar.DecayDec= (float)(((Vals[3]-val)*globalpar.sinmix)/(globalpar.SLength*100.0));
		...
		case 5:
			...
			if ( Vals[8] == 0 ) {
				...
				globalpar.DecayDec= (float)(((Vals[3]-Vals[4])*globalpar.sinmix)/(globalpar.SLength*100.0));
				//globalpar.SustainDec; // <- Not Used
			}
			else {
				if ( Vals[10] != 0 ) { // If it's 0, they don't change.
			...
			break;
		case 6:				globalpar.OutVol= (float)(val/32767.0);
			break;
		...
		case 8: 
			if ( val == 0 ) {
				...
				globalpar.SinVol= (float)(Vals[3]*globalpar.sinmix/100.0);
				globalpar.DecayPos=globalpar.SLength;
				globalpar.DecayDec= (float)(((Vals[3]-Vals[4])*globalpar.sinmix)/(globalpar.SLength*100.0));
				//globalpar.SustainDec; // <- Not Used.
			}
				...
				if ( Vals[10] != 0 ) {
					...
					globalpar.AttackPos=0; // globalpar.AttackInc; //<- Not used
					globalpar.SinVol= (float)(327.67*globalpar.sinmix);
				}
				if ( Vals[10] > Vals[11] ) globalpar.DecayPos=globalpar.AttackPos+1;
				else globalpar.DecayPos=globalpar.SLength*Vals[11]/100;
				globalpar.DecayDec= (float)(((327.67-(3.2767*Vals[12]))*globalpar.sinmix)/(globalpar.DecayPos-globalpar.AttackPos));
		...
		case 11:
			if ( Vals[8] > 0 ) {
				...
				else globalpar.DecayPos= (int)(globalpar.SLength*val/100);
				globalpar.DecayDec= (float)(((327.67-(3.2767*Vals[12]))*globalpar.sinmix)/(globalpar.DecayPos-globalpar.AttackPos));
				globalpar.SustainDec= (float)((3.2767*Vals[12]*globalpar.sinmix)/(globalpar.SLength-globalpar.DecayPos));
			}
			break;
		case 12:
			if ( Vals[8] > 0 ) {
				globalpar.DecayDec= (float)(((327.67-(3.2767*val))*globalpar.sinmix)/(globalpar.DecayPos-globalpar.AttackPos));
				globalpar.SustainDec= (float)((3.2767*val*globalpar.sinmix)/(globalpar.SLength-globalpar.DecayPos));
			}
			break;
		case 13: 
			if ( val > 0 ) { globalpar.sinmix=(-1)*(val-100); globalpar.ThumpVol=32767; }
			else { globalpar.sinmix=100; globalpar.ThumpVol= (float)(327.67*(val+100)); }
			if ( globalpar.AttackPos == 0 ) {
				if ( Vals[8] == 0 ) globalpar.SinVol= (float)(Vals[3]*globalpar.sinmix/100.0);
				else globalpar.SinVol= (float)(327.67*globalpar.sinmix);
			}
			else {
				globalpar.SinVol=0;
				globalpar.AttackInc= (float)((327.67*globalpar.sinmix)/globalpar.AttackPos);
...
void mi::SeqTick(int channel, int note, int ins, int cmd, int val)
{
	...
	if(note<NOTE_NOTEOFF) // Note on
	{
		...
		{
			if ( cmd == 0x0C ) globalpar.OutVol= (float)(val*Vals[3]/8388352.0);  // (val*(Vals[3]/32767))/256

			DTrack[vidx].NoteOn(48,&globalpar); // C-4 always
		}
		else
		{
			if ( cmd == 0x0C ) globalpar.OutVol=(float)(val*tmp/256.0);
	...
	else if (vidx != -1)
	{
		...
		{
			if ( Vals[8]==0 ) DTrack[vidx].OutVol= (float)(val/256.0); // Comp 1.x
			else DTrack[vidx].OutVol=(float)((val*globalpar.OutVol/256.0)); // Comp 2.x
		}
	}
}
...
int mi::GetVoice(int channel,bool getnew)
{
	...
	if ( j == MAX_SIMUL_TRACKS)
	{
		...
		if (j == MAX_SIMUL_TRACKS)
		{
			j = (unsigned int) (  (double)psy_rand16() * MAX_SIMUL_TRACKS /(((double)RAND_MAX) + 1.0 ) );
		}
	}
