...
float Drum::GetSample() {
	...
	if (Spos<ThumpLength){
		output=(float)((sin(Drummy.GetPos()) * SinVol) + (sin(Thump.GetPos()) * ThumpVol)) * OutVol;
		ThumpVol-=ThumpDec;
	}
	else output=(float)(sin(Drummy.GetPos()) * SinVol) * OutVol;
	...
	case ST_NOTERELEASE:
		if ( OutVol > 0.00003 ) OutVol -= (float)VolDec;
		else {
