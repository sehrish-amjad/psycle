...
void mi::SetParams() {
	...
	fluid_synth_reverb_on(synth, -1, globalpar.reverb_on);
#else	
	...
	if(globalpar.reverb_on) {
		...
		fluid_synth_set_reverb_group_level(synth, -1, r_level);
#else
		fluid_synth_set_reverb(synth, roomsize, damping, width, r_level);
#endif		
	}		
#if FS_API_VERSION >= 0x0202		
		fluid_synth_chorus_on(synth, -1, globalpar.chorus_on);
#else
		fluid_synth_set_chorus_on(synth, globalpar.chorus_on);
#endif
	if(globalpar.chorus_on) {
#if FS_API_VERSION >= 0x0202
		fluid_synth_set_chorus_group_nr(synth, -1, nr);
		fluid_synth_set_chorus_group_level(synth, -1, c_level);
		fluid_synth_set_chorus_group_speed(synth, -1, speed);
		fluid_synth_set_chorus_group_depth(synth, -1, depth_ms);
		fluid_synth_set_chorus_group_type(synth, -1, type);
#else
		...
		case e_paraChannel:
			...
			if(preset != NULL && max_bank_index != -1) {
				for(int b = 0; b <= max_bank_index; ++b)
					if(banks[b] == fluid_preset_get_banknum(preset)) {
						Vals[e_paraBank] = b;
						break;
					}
...
void mi::Command() {
	char name[1024];
	name[0]='\0';
	bool open=pCB->FileBox(true,(char*)"SF2 (*.sf2)|*.sf2|All Files (*.*)|*.*||",name);
	if (open) {
		LoadSF(name);
	}
}
