...
void mi::ParameterTweak(int par, int val)
{
	...
	if(par==0) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].VCA.attack((int)(val*srMult));
	if(par==1) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].VCA.decay((int)(val*srMult));
	if(par==2) for (i=0; i<MAX_TRACKS ; i++) { 
		if (val<16) Tracks[i].VCA.sustain(0);
		else Tracks[i].VCA.sustain((int)(val*srMult));
	}
	if(par==3) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].VCA.sustainv((float)val/255);
	if(par==4) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].VCA.release((int)(val*srMult));
	
	if(par==5) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].ENV.attack((int)(val*srMult));
	if(par==6) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].ENV.decay((int)(val*srMult));
	if(par==7) for (i=0; i<MAX_TRACKS ; i++) {
		if(val<16) Tracks[i].ENV.sustain(0);
		else Tracks[i].ENV.sustain((int)(val*srMult));
	}
	if(par==8) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].ENV.sustainv((float)val/255);
	if(par==9) for (i=0; i<MAX_TRACKS ; i++) Tracks[i].ENV.release((int)(val*srMult));
...
void mi::SeqTick(int channel, int note, int ins, int cmd, int val)
{
	...
	if(cmd==0x0E)
		medBreakNote[channel]= (int)(val*32.f* (pCB->GetSamplingRate()/44100.f));
