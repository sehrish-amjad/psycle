...
mi::mi()
{
	...
	{
		randvals1[i] = ((psy_rand16() % 30000)/15000.f) - 1.f;
		randvals2[i] = ((psy_rand16() % 30000)/15000.f) - 1.f;
	}
...
void mi::Work(float *psamplesleft, float *psamplesright , int numsamples, int tracks)
{
	...
	do{
		...
		if(counter<=0)
		{
			randFreq = psy_rand16() % (MAX_FREQ-MIN_FREQ);
			counter = Vals[PRM_FREQ]+ (Vals[PRM_RANDFREQ]==0? 0: (int)((randFreq-Vals[PRM_FREQ]) * (Vals[PRM_RANDFREQ]/100.0)));

			if (Vals[PRM_L1TOFREQ] != 100) {
				counter += (int)(((MAX_FREQ-MIN_FREQ)/2) * (GetLFO(LFO1) * ((Vals[PRM_L1TOFREQ]-100)/100.0)));
			}
			if (Vals[PRM_L2TOFREQ] != 100) {
				counter += (int)(((MAX_FREQ-MIN_FREQ)/2) * (GetLFO(LFO2) * ((Vals[PRM_L2TOFREQ]-100)/100.0)));
			}			
			if (counter > MAX_FREQ) {
				counter = MAX_FREQ;
			} else if (counter < MIN_FREQ) {
				counter = MIN_FREQ;
			}
			counter = (int)(counter * srMultiplyer);
			StartLayer();
		}
		...
		if(lfo1counter >= (Vals[PRM_LFO1SPEED]) * LSPEED_MULTIPLYER)
		{
			...
			for(i=0;i<NUM_RANDS;++i)
				randvals1[i] = (float)(((psy_rand16() % 30000)/15000.0) - 1);
		}
		...
		if(lfo2counter >= (Vals[PRM_LFO2SPEED]) * LSPEED_MULTIPLYER)
		{
			lfo2counter = 0;
			for(i=0;i<NUM_RANDS;++i)
				randvals2[i] = (float)(((psy_rand16() % 30000)/15000.0) - 1);
		}
		...
		for(int i=0;i<MAX_LAYERS;i++)
			if(layers[i]->in_use)
			{
				...
				{
					outDataL += (int)(layers[i]->process(*psamplesleft, LCHAN));
					outDataR += (int)(layers[i]->process(*psamplesright, RCHAN));
		...
		if(abs(outDataL) > Vals[PRM_THRESH])
		{
			...
			{
				if(dispCounter <= 0)
					limitAmtL = (int)((abs(outDataL) / (float) Vals[PRM_THRESH]) * LIM_AMT_SCALER);
		...
		if(abs(outDataR) > Vals[PRM_THRESH])
		{
			...
			{
				if(dispCounter <= 0)
					limitAmtR = (int)((abs(outDataR) / (float) Vals[PRM_THRESH]) * LIM_AMT_SCALER);
		...
		*psamplesleft=(float)outDataL;
		*psamplesright=(float)outDataR;
...
void mi::StartLayer()
{
	...
	float length = (float)Vals[PRM_SIZE];
	float attack = (float)Vals[PRM_ATTACK];
	float decay  = (float)Vals[PRM_DECAY];
	float pstart = (float)Vals[PRM_PSTART];
	float pend   = (float)Vals[PRM_PEND];

	float randLength = (float)(psy_rand16() % (MAX_LENGTH-MIN_LENGTH)	+ MIN_LENGTH);
	float randAttack = (float)(psy_rand16() % (MAX_ATTACK-MIN_ATTACK)	+ MIN_ATTACK);
	float randDecay  = (float)(psy_rand16() % (MAX_DECAY-MIN_DECAY)		+ MIN_DECAY);
	float randPStart = (float)(psy_rand16() % (MAX_PITCH-MIN_PITCH)		+ MIN_PITCH);
	float randPEnd   = (float)(psy_rand16() % (MAX_PITCH-MIN_PITCH)		+ MIN_PITCH);

	if(Vals[PRM_RANDSIZE] != 0)
		length += (float)((randLength-length)	* (Vals[PRM_RANDSIZE]/100.0));
	if(Vals[PRM_RANDATTACK] != 0)
		attack += (float)((randAttack-attack)	* (Vals[PRM_RANDATTACK]/100.0));
	if(Vals[PRM_RANDDECAY] != 0)
		decay += (float)((randDecay-decay)		* (Vals[PRM_RANDDECAY]/100.0));
	if(Vals[PRM_RANDPSTART] != 0)
		pstart += (float)((randPStart-pstart)	* (Vals[PRM_RANDPSTART]/100.0));
	if(Vals[PRM_RANDPEND] != 0)
		pend += (float)((randPEnd-pend)			* (Vals[PRM_RANDPEND]/100.0));


	if(Vals[PRM_L1TOSIZE] != 100)
		{length += (float)(((MAX_LENGTH-MIN_LENGTH)/2) * (GetLFO(LFO1) * ((Vals[PRM_L1TOSIZE]-100)/100.0)));				}
	if(Vals[PRM_L1TOATTACK] != 100)
		{attack += (float)(((MAX_ATTACK-MIN_ATTACK)/2) * (GetLFO(LFO1) * ((Vals[PRM_L1TOATTACK]-100)/100.0)));				}
	if(Vals[PRM_L1TODECAY] != 100)
		{decay += (float)(((MAX_DECAY-MIN_DECAY)/2)   * (GetLFO(LFO1) * ((Vals[PRM_L1TODECAY]-100)/100.0)));				}
	if(Vals[PRM_L1TOPSTART] != 100)
		{pstart += (float)(((MAX_PITCH-MIN_PITCH)/2)   * (GetLFO(LFO1) * ((Vals[PRM_L1TOPSTART]-100)/100.0)));				}
	if(Vals[PRM_L1TOPEND] != 100)
		{pend += (float)(((MAX_PITCH-MIN_PITCH)/2)   * (GetLFO(LFO1) * ((Vals[PRM_L1TOPEND]-100)/100.0)));				}

	if(Vals[PRM_L2TOSIZE] != 100)
		{length += (float)(((MAX_LENGTH-MIN_LENGTH)/2) * (GetLFO(LFO2) * ((Vals[PRM_L2TOSIZE]-100)/100.0)));				}
	if(Vals[PRM_L2TOATTACK] != 100)
		{attack += (float)(((MAX_ATTACK-MIN_ATTACK)/2) * (GetLFO(LFO2) * ((Vals[PRM_L2TOATTACK]-100)/100.0)));				}
	if(Vals[PRM_L2TODECAY] != 100)
		{decay += (float)(((MAX_DECAY-MIN_DECAY)/2)   * (GetLFO(LFO2) * ((Vals[PRM_L2TODECAY]-100)/100.0)));				}
	if(Vals[PRM_L2TOPSTART] != 100)
		{pstart += (float)(((MAX_PITCH-MIN_PITCH)/2)   * (GetLFO(LFO2) * ((Vals[PRM_L2TOPSTART]-100)/100.0)));				}
	if(Vals[PRM_L2TOPEND] != 100)
		{pend += (float)(((MAX_PITCH-MIN_PITCH)/2)   * (GetLFO(LFO2) * ((Vals[PRM_L2TOPEND]-100)/100.0)));				}
	...
	if(layers[next]->in_use==false)
	{
		...
		for(i=0;i<(int)(attack);++i)				
		{
			...
			else	layers[next]->attackBuf.push_back((float)(3.4 * splineX - 1.2));							// .4<=x<=.6:				3.4x - 1.2
		}
		...
		for(i=0;i<(int)(decay);++i)
		{
			...
			else	layers[next]->decayBuf.push_back((float)(3.4 * (1-splineX) - 1.2));								// .4<=x<=.6:				3.4(1-x) - 1.2
		}

		layers[next]->pitchCurrent	= (pstart / (float)MAX_PITCH);
		layers[next]->pitchEnd		= (pend	  / (float)MAX_PITCH);
		layers[next]->pitchStep		= (layers[next]->pitchEnd - layers[next]->pitchCurrent) / (length!=0.f ? length : 0.0001f);
	}
...
//												destSamp =( dataL[(int)floor(relPos)] + (dataL[(int)floor(relPos)] -dataL[(int)ceil(relPos)]) * (relPos - floor(relPos)));
			destSamp = mi::SplineInterpolate(				dataL[(int)ceil(relPos)], 
												...
												(unsigned int)(relPos-floor(relPos) * CUBIC_RESOLUTION));
		}
	}
	else
	{
