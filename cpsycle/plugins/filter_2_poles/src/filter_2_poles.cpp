...
inline void Filter_2_Poles::update_coefficients(Real coefficients[poles + 1], const Real & modulation_stereo_dephase)
{
	...
	dsp_erase_all_nans_infinities_and_denormals_double(&dsp, coefficients,
		poles + 1);	
}
...
void Filter_2_Poles::Work(Sample l[], Sample r[], int samples, int)
{
	...
	dsp_erase_all_nans_infinities_and_denormals_double(&dsp, buffers_[left],
		channels);
	dsp_erase_all_nans_infinities_and_denormals_double(&dsp, buffers_[right],
		channels);	
