...
float mi::Cutoff(int v) {
	return (float)(std::pow((v + 5.0) / (127.0 + 5.0), 1.7) * 13000.0 + 30.0);
}

float mi::Resonance(float v) {
	return (float)(std::pow(v / 127.0, 4.0) * 150.0 + 0.1);
}

float mi::Bandwidth(int v) {
	return (float)(std::pow(v / 127.0, 4.0) * 4.0 + 0.1);
}

float mi::LFOFreq(int v) {
	return (float)((std::pow((v + 8.0) / (116.0 + 8.0), 4.0) - 0.000017324998565270) * 40.00072);
}

float mi::EnvTime(int v) {
	return (float)(std::pow((v + 2.0) / (127.0 + 2.0), 3.0) * 10000);
}
////////////////////////////////
////////////////////////////////
////////////////////////////////
...
inline float CTrack::Osc() {
	...
	if(noise2) {
		...
		o2 = (float)((pwavetab2[Ph2 >> 16] * B2) >> 7);
...
inline float CTrack::VCA() {
	...
	Amp += AmpAdd;
	if(LFO_Amp) {
		float a = (float)(Amp + (pwavetabLFO1[((unsigned)PhaseLFO1)>>21]*LFO1Amount)/(127.0*0x8000));
		if(a < 0) a = 0;
		return a;
	} else return Amp;
}
...
inline float CTrack::Filter( float x) {
	...
	int c, r;
	if(LFO_Cut) {
		c = (int)(Cutoff + Cut + // Cut = EnvMod
		((pwavetabLFO1[((unsigned)PhaseLFO1) >> 21] * LFO1Amount) >> (7 + 8)));
	}
	else c = (int)(Cutoff + Cut); // Cut = EnvMod
...
inline void CTrack::NewPhases() {
	if(PitchModActive) {
		if(GlideActive) {
			...
			} else Phase2 += (int)(PhaseAdd2 * GlideFactor * PitchFactor);
			...
			if(LFO_Osc2) Phase2 += (int)(PhaseAdd2 * PitchFactor * LFOOscTab[(pwavetabLFO2[((unsigned)PhaseLFO2) >> 21] * LFO2Amount >> 7) + 0x8000]);
			else Phase2 += (int)(PhaseAdd2 * PitchFactor);
		}
		...
		if(!PEGCount--)
			if(++PEGState == 2) { // DECAY-PHASE begin
				...
				PitchMul = (float)(std::pow(std::pow(1 / 1.01, PEnvMod), 1.0 / PEGDecayTime));
		...
		if(GlideActive) {
			...
			if(LFO_Osc2) {
				Phase2 += (int)(PhaseAdd2 * GlideFactor * LFOOscTab[(pwavetabLFO2[((unsigned)PhaseLFO2) >> 21] * LFO2Amount >> 7) + 0x8000]);
			} else Phase2 += (int)(PhaseAdd2*GlideFactor);
			...
			if(LFO_Osc2) Phase2 += (int)(PhaseAdd2 * LFOOscTab[(pwavetabLFO2[((unsigned)PhaseLFO2) >> 21] * LFO2Amount >> 7) + 0x8000]);
	...
	if(Phase1 & 0xf8000000) { // new durchlauf ??
		...
		if(LFO_PW1) { // LFO_PW_Mod
			currentcenter1 = (float)(Center1 + (float)pwavetabLFO1[((unsigned)PhaseLFO1) >> 21] * LFO1Amount / (127.0 * 0x8000));
			if(currentcenter1 <= 0) currentcenter1 = 0.00001f;
			else if(currentcenter1 >= 1) currentcenter1 = 0.99999f;
		} else {// No LFO
			currentcenter1 = Center1;
		}
		PhScale1A = (float)(0.5 / currentcenter1);
		PhScale1B = (float)(0.5 / (1 - currentcenter1));
		currentcenter1 *= 0x8000000;
		// PW2
		if(LFO_PW2) { //LFO_PW_Mod
			currentcenter2 = (float)(Center2 + (float)pwavetabLFO2[((unsigned)PhaseLFO2) >> 21] * LFO2Amount / (127.0 * 0x8000));
			if(currentcenter2 <= 0) currentcenter2 = 0.00001f;
			else if(currentcenter2 >= 1) currentcenter2 = 0.99999f;
		} else { // No LFO
			currentcenter2 = Center2;
		}
		PhScale2A = (float)(0.5 / currentcenter2);
		PhScale2B = (float)(0.5 / (1 - currentcenter2));
	...
	if(Phase1 < currentcenter1) Ph1 = (int)(Phase1*PhScale1A);
	else Ph1 = (int)((Phase1 - currentcenter1) * PhScale1B + 0x4000000);

	if(Phase2 < currentcenter2) Ph2 = (int)(Phase2 * PhScale2A);
	else Ph2 = (int)((Phase2 - currentcenter2) * PhScale2B + 0x4000000);
