...
void CTrack::Tick( tvals const &tv)
{
	...
	{
		Center1 = (float)(tv.PulseWidth1/127.0);
	}
	if( tv.PulseWidth2 != 0xff)
	{
		Center2 = (float)(tv.PulseWidth2/127.0);
	}
	...
	if( tv.Note != NOTE_NONE) // neuer wert
	{
		...
		if(Note <= NOTE_MAX) // neue note gesetzt
		{
			...
			if( Glide)
			{
				...
				{				GlideMul = (float)std::pow(2., 1. / GlideTime); }
				else
				{				GlideMul = (float)std::pow(0.5, 1. / GlideTime); }
				GlideFactor = 1;
			...
			if( PitchMod)
			{
				...
				PEGState = EGS_ATTACK;
				PEGCount = PEGAttackTime;
				PitchMul = (float)pow( pow( 1.01, PEnvMod), 1.0/PEGAttackTime);
				PitchFactor = 1.0;
			}
			else
			{				PitchModActive = false;				}
