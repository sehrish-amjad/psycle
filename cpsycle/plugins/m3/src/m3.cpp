...
mi::mi() {
	...
	// generate LFOOscTab
	for(int p = 0; p < 0x10000; ++p) CTrack::LFOOscTab[p] = (float)(std::pow(1.00004230724139582, p - 0x8000));
}
...
void mi::InitWaveTable(){
	for(int c = 0; c < 2100; ++c) {
		...
		CTrack::WaveTable[4][c] = psy_rand16();
	}
}
...
void mi::ComputeCoefs( float *coefs, int freq, int r, int t) {
	...
	float omega = (float)(2 * psycle::plugin_interface::pi * Cutoff(freq) / currentSR);
	...
	float alpha;
	if(t<2) alpha = (float)(sn / Resonance((float)(r * (freq + 70) / (127.0 + 70))));
	else alpha = (float)(sn * std::sinh(Bandwidth(r) * omega / sn));
