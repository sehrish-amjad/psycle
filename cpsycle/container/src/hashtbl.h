...
** @brief A Hashtable.
*/
typedef struct psy_Table {
	/*! @internal */
	psy_TableHashEntry** keys_;
	uintptr_t array_size_;
	uintptr_t count_;
} psy_Table;

typedef struct psy_TableIterator {
	/*! @internal */
	uintptr_t pos_;
	uintptr_t count_;
	psy_TableHashEntry* curr_;
	psy_Table* table_;
} psy_TableIterator;
...
void psy_table_clear_all(psy_Table*,
	psy_fp_disposefunc disposefunc);
