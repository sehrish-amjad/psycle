...
typedef void (*psy_fp_command_param)(struct psy_Command*, uintptr_t param);

typedef struct psy_CommandVtable {
	psy_fp_command dispose;
	psy_fp_command_param execute;
...
INLINE void psy_command_execute(psy_Command* self, uintptr_t param)
{
	assert(self);

	self->vtable->execute(self, param);
}
