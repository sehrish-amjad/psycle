...
INLINE psy_EventDriverCmd psy_eventdrivercmd_make_cmd(intptr_t id)
{
	psy_EventDriverCmd rv;
	
	rv.type = psy_EVENTDRIVER_CMD;
	rv.id = id;
	return rv;
}
...
typedef void (*psy_eventdriver_fp_configure)(struct psy_EventDriver*, const psy_Property*);
...
INLINE void psy_audio_decode_input(uint32_t input, uint32_t* keycode,
...
INLINE void psy_eventdriver_configure(psy_EventDriver* self, const psy_Property*
	properties)
{
...
INLINE void psy_eventdriver_setcmddef(psy_EventDriver* self, const
