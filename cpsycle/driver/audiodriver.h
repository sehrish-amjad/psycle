...
#define PSY_AUDIODRIVER_JACK_GUID 0x0007
#define PSY_AUDIODRIVER_SDL2_GUID 0x0008
...
typedef void (*psy_audiodriver_fp_connect)(struct psy_AudioDriver*, void* context,
	...
	void* handle);
typedef double(*psy_audiodriver_fp_samplerate)(struct psy_AudioDriver*);
typedef const char* (*psy_audiodriver_fp_capturename)(struct psy_AudioDriver*, int index);
typedef uintptr_t (*psy_audiodriver_fp_numcaptures)(const struct psy_AudioDriver*);
...
INLINE double psy_audiodriver_samplerate(psy_AudioDriver* self)
{
	return self->vtable->samplerate(self);
}

INLINE void psy_audiodriver_refresh_ports(psy_AudioDriver* self)
{
	self->vtable->refresh_ports(self);
}
...
INLINE uintptr_t psy_audiodriver_numcaptures(const psy_AudioDriver* self)
{
...
INLINE void psy_audiodriver_read_buffers(psy_AudioDriver* self, int index,
	float** pleft, float** pright, uintptr_t numsamples)
{
...
INLINE int psy_audiodriver_restart(psy_AudioDriver* self,
	const psy_Property* configuration)
{
	assert(self);

	if (psy_audiodriver_opened(self)) {
		psy_audiodriver_close(self);
	}
	psy_audiodriver_configure(self, configuration);		
	return psy_audiodriver_open(self);
}

INLINE float* psy_audiodriver_work(psy_AudioDriver* self, int* num,
	int* hostisplaying)
{
	return self->callback(self->callbackcontext, num, hostisplaying);
}
