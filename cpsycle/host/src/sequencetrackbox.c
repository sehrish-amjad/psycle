	...
	uintptr_t trackidx, Workspace* workspace)
{
	...
	if (self->sequence) {
		psy_signal_connect(&self->sequence->signal_solo_changed, self,
			sequencetrackbox_on_solochanged);
		psy_signal_connect(&self->sequence->signal_mute_changed, self,
	...
	psy_signal_connect(&self->trackbox.desc.component.signal_mouse_down, self,
...
void sequencetrackbox_on_destroyed(SequenceTrackBox* self)
{
	...
	if (self->sequence) {
		psy_signal_disconnect(&self->sequence->signal_solo_changed, self,
			sequencetrackbox_on_solochanged);
		psy_signal_disconnect(&self->sequence->signal_mute_changed, self,
			sequencetrackbox_on_mutechanged);
	}
...
void sequencetrackbox_on_solo_track(SequenceTrackBox* self, TrackBox* sender)
{
	...
	if (self->sequence) {		
		if (psy_audio_sequence_is_track_soloed(self->sequence, self->trackidx)) {
			psy_audio_sequence_deactivate_solo_track(self->sequence);
		} else {
			psy_audio_sequence_activate_solo_track(self->sequence, self->trackidx);
		}
	}
}
...
void sequencetrackbox_on_mutetrack(SequenceTrackBox* self, TrackBox* sender)
{
	...
	if (self->sequence) {				
		if (psy_audio_sequence_is_track_muted(self->sequence, self->trackidx)) {
			psy_audio_sequence_unmute_track(self->sequence, self->trackidx);
		} else {
			psy_audio_sequence_mute_track(self->sequence, self->trackidx);
		}
	}
}
...
{	
	...
	if (self->trackidx == trackidx) {
		if (psy_audio_sequence_is_track_soloed(sender, trackidx)) {
			trackbox_solo(&self->trackbox);
		}
	}
	if (psy_audio_sequence_is_track_muted(sender, trackidx)) {
...
void sequencetrackbox_show_name(SequenceTrackBox* self)
{
	assert(self);
	...
	if (self->sequence) {
		...
		if (track) {
			trackbox_set_description(&self->trackbox,
	...
	psy_ui_MouseEvent* ev)
{
	...
	editposition = psy_audio_sequenceselection_first(&self->workspace->song->sequence_.selection);
