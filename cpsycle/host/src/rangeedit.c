...
void rangeedit_init(RangeEdit* self, psy_ui_Component* parent)
{
	...
	psy_ui_text_init(&self->edit_from, rangeedit_base(self));	
	psy_ui_text_set_char_number(&self->edit_from, 5);
	psy_ui_text_enable_input_field(&self->edit_from);
	psy_ui_label_init(&self->to, rangeedit_base(self));
	psy_ui_label_set_text(&self->to, "to");
	psy_ui_text_init(&self->edit_to, rangeedit_base(self));
	psy_ui_text_set_char_number(&self->edit_to, 5);
	psy_ui_text_enable_input_field(&self->edit_to);
}
	...
	psy_Property* to)
{
	...
	rv = rangeedit_alloc_init(parent);
	if (rv) {
		rangeedit_exchange(rv, from, to);
	}
	return rv;
}

void rangeedit_exchange(RangeEdit* self, psy_Property* from,
	psy_Property* to)
{
	assert(self);

	psy_ui_text_exchange(&self->edit_from, from);
	psy_ui_text_exchange(&self->edit_to, to);
}
