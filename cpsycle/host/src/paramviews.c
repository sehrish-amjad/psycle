...
void paramviews_show(ParamViews* self, uintptr_t mac_id)
{
	...
	if (!frame) {
		frame = machineframe_alloc_init(self->view, machine, self,
			self->workspace);
		if (frame) {
...
FrameMachine* paramviews_frame(ParamViews* self, uintptr_t mac_id)
{
	assert(self);
	
	return (FrameMachine*)psy_table_at(&self->frames, mac_id);
}
