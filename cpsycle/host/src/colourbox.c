...
{	
	...
	psy_ui_text_set_char_number(&self->edit, (int)20.0);
	...
	psy_ui_text_exchange(&self->edit, self->property);
...
void colourbox_exchange(ColourBox* self, psy_Property* property)
{
	assert(self);
	assert(property);
	...
	if (property) {
		...
		psy_ui_text_exchange(&self->edit, property);
