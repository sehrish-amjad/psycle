...
void switchui_set_param(SwitchUi* self,
...
{			
	...
	bg_size = psy_ui_size_px(&style->background.size,
		psy_ui_component_textmetric(&self->component), NULL);
	psy_ui_graphics_textout_rectangle(g,
		psy_ui_realpoint_make(bg_size.width, 0.0), 0,
		psy_ui_realrectangle_make(
			psy_ui_realpoint_make(bg_size.width, 0),
			psy_ui_realsize_make(
				size.width - bg_size.width,
				size.height / 2)), label, strlen(label));	
}

bool switchui_checked(SwitchUi* self)
{
	return (self->param && psy_audio_machineparam_norm_value(self->param) !=
		0.f);
}
