...
void oscilloscope_init_memory(Oscilloscope* self)
{
	...
	if (machine) {
		...
		if (memory) {
			...
			psy_audio_machine_set_buffermemory_size(machine, SCOPE_BUF_SIZE);
...
void oscilloscope_on_destroyed(Oscilloscope* self)
{
	...
	if (workspace_song(self->workspace)) {
		...
		machine = psy_audio_machines_at(
			&workspace_song(self->workspace)->machines_, self->wire.src);
		if (machine && psy_audio_machine_buffermemory(machine)) {			
			psy_audio_exclusivelock_enter();
			psy_audio_machine_reset_buffermemory(machine);			
			psy_audio_exclusivelock_leave();			
		}
	}
}
	...
	if (self->running) {
		...
		if (buffer && numsamples > 0) {
			...
			if (!zero) {
				...
				py = size.height * (double)psy_audio_buffer_range_factor(buffer,
...
{	
	self->invol = psy_audio_connections_wire_volume(
		&workspace_song(self->workspace)->machines_.connections, self->wire);
...
void oscilloscope_set_zoom(Oscilloscope* self, double rate)
{
	self->scope_view_samples = (uintptr_t)((double)(SCOPE_BUF_SIZE) * rate);
}

void oscilloscope_set_begin(Oscilloscope* self, double begin)
{
	if (self->scope_view_samples > 0 &&
			self->scope_view_samples < SCOPE_BUF_SIZE) {
		self->scope_begin = (uintptr_t)(begin * ((double)(SCOPE_BUF_SIZE) -
	...
	Oscilloscope* oscilloscope)
{
	...
	psy_ui_margin_set_right(&margin, psy_ui_value_make_ew(2.0));
	...
	psy_ui_margin_set_right(&margin, psy_ui_value_make_ew(0.5));
...
{
	self->oscilloscope->ampzoom = zoombox_rate(sender);
}
