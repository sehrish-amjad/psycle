...
{	
	...
	if (self->machine && psy_audio_machine_num_tweak_parameters(self->machine) > 0) {
		paramindex = 0;
	} else {
		paramindex = psy_INDEX_INVALID;
	}
	knobui_init(&self->knob, &self->component, machine, paramindex, NULL, NULL);
...
{	
	...
	if (self->machine) {
		...
		for (i = 0; i < psy_audio_machine_num_tweak_parameters(self->machine); ++i) {
			psy_audio_MachineParam* param;
			
			param = psy_audio_machine_tweak_parameter(self->machine, i);
			if (param) {
	...
	psy_ui_Component* sender, intptr_t slot)
{
	...
	if (self->machine && slot < (intptr_t)psy_audio_machine_num_tweak_parameters(self->machine)) {
		paramindex = slot;		
	} else {
		paramindex = psy_INDEX_INVALID;
	}
	...
	psy_audio_Machine* machine)
{
	...
	if (machine && psy_audio_machine_num_tweak_parameters(self->machine) > 0) {
		paramindex = 0;
	} else {
		paramindex = psy_INDEX_INVALID;
	}
