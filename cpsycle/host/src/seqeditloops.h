...
INLINE psy_dsp_beatpos_t seqeditloop_offset(const SeqEditLoop* self)
{	
	if (seqeditloop_entry_const(self)) {
		return psy_audio_patternentry_offset(seqeditloop_entry_const(self));
	}
	return psy_dsp_beatpos_zero();
}

INLINE psy_dsp_beatpos_t seqeditloop_endoffset(const SeqEditLoop* self)
{	
	if (self->loopstate->end) {
		...
		return psy_audio_patternentry_offset(loopendentry);
	}
	return psy_dsp_beatpos_zero();
}

INLINE psy_dsp_beatpos_t seqeditloop_nextoffset(const SeqEditLoop* self)
{	
	if (self->loopstate->next) {
		...
		return psy_audio_patternentry_offset(entry);
	}
	// todo ppq
	return psy_dsp_beatpos_make_real((double)UINTPTR_MAX, psy_dsp_DEFAULT_PPQ);
}

INLINE psy_dsp_beatpos_t seqeditloop_prevoffset(const SeqEditLoop* self)
{	
...
INLINE psy_dsp_beatpos_t seqeditloop_startoffset(const SeqEditLoop* self)
{	
