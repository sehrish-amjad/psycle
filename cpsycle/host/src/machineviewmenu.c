	...
	uintptr_t idx, psy_audio_Wire wire)
{
	...
	if (psy_table_exists(&self->wires, idx)) {
		newwire = (psy_audio_Wire*)psy_table_at(&self->wires, idx);	
	} else {
	...
	MachineMenuState* state)
{
	...
	psy_ui_component_set_align(&self->hide.component, psy_ui_ALIGN_LEFT);	
	...
	psy_ui_text_init(&self->title, &self->client);
	psy_ui_text_set_char_number(&self->title, 32);
	...
	psy_ui_text_enable_input_field(&self->title);
}
...
{	
	...
	if (machine) {
		psy_ui_text_set_text(&self->title,
			psy_audio_machine_edit_name(machine));
...
static void machinemenu_align(MachineMenu*);
	...
	WireFrames* wireframes)
{
	...
	machinemenustate_init(&self->state, wireframes, machinemenu_base(self));
	psy_ui_component_init(&self->pane, machinemenu_base(self), NULL);
	...
	presetsview_init(&self->presets, machinemenu_base(self), wireframes->workspace);
	...
	psy_ui_component_start_timer(machinemenu_base(self), 0, 100);
}
...
void machinemenu_select(MachineMenu* self, uintptr_t mac_id)
{
	...
	if (!psy_ui_component_visible(machinemenu_base(self))) {
		psy_ui_component_show(machinemenu_base(self));
		machinemenu_align(self);
	}
}
	...
	psy_ui_Component* sender)
{
	...
	paramviews = workspace_param_views(self->state.wireframes->workspace);
	if (paramviews) {
...
void machinemenu_on_machine_clone(MachineMenu* self, psy_ui_Component* sender)
{
	...
	if (machine) {
		...
		if (clone) {
			...
			psy_audio_machine_set_position(clone, x, y);
...
void machinemenu_hide(MachineMenu* self)
{
	...
	psy_ui_component_hide(&self->confirm.component);
	psy_ui_component_hide(&self->presets.component);
	psy_ui_component_hide_align(machinemenu_base(self));
}

void machinemenu_align(MachineMenu* self)
{
	assert(self);

	psy_ui_component_align_invalidate(psy_ui_component_parent(
		machinemenu_base(self)));
}
