...
static void skin_parse_ini_machine(psy_Property*);
static void skin_parse_style(psy_Property*, uintptr_t style_id);

int skin_load_ini_machine(const char* path)
{
	psy_PropertyReader reader;
	psy_Property machine_skin;

	psy_property_init_key(&machine_skin, NULL);
	machine_skin.item.allow_append = TRUE;
	psy_propertyreader_init(&reader, &machine_skin, path);
	reader.config.parse_types = TRUE;
	psy_propertyreader_load(&reader);		
	skin_parse_ini_machine(&machine_skin);
	psy_propertyreader_dispose(&reader);
	psy_property_dispose(&machine_skin);
	return PSY_OK;
}

void skin_parse_ini_machine(psy_Property* property)
{
	psy_List* p;
	...
	for (; p != NULL; p = p->next) {
		...
		if (strcmp(psy_property_key(curr), "master") == 0) {
			skin_parse_style(curr, STYLE_MV_MASTER);
		} else if (strcmp(psy_property_key(curr), "effect") == 0) {
			skin_parse_style(curr, STYLE_MV_GENERATOR);
		} else if (strcmp(psy_property_key(curr), "generator") == 0) {
			skin_parse_style(curr, STYLE_MV_EFFECT);
		}
	}
}

void skin_parse_style(psy_Property* property, uintptr_t style_id)
{
	psy_ui_Style* style;
	psy_Property* p;	
	
	style = psy_ui_style(style_id);
	p = psy_property_at(property, "colour", PSY_PROPERTY_TYPE_INTEGER);
	if (p) {		
		psy_ui_style_set_colour(style, psy_ui_colour_make(
			(uint32_t)psy_property_item_int(p)));
	}
	p = psy_property_at(property, "background.colour",
