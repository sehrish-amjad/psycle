		...
		if (self->loopviewmode == WAVEBOX_LOOPVIEW_CONT_DOUBLE) {
			...
			if (self->loopviewmode == WAVEBOX_LOOPVIEW_SUSTAIN_DOUBLE) {
				return self->sample->numframes +
					waveboxcontext_numsustainloopframes(self);
			} else {
				return self->sample->numframes;
			}
	}
...
uintptr_t waveboxcontext_numsustainloopframes(WaveBoxContext* self)
{
	if (self->sample && self->sample->sustainloop.type !=
		...
		return self->sample->sustainloop.end -
			self->sample->sustainloop.start + 1;
	}
	return 0;
}
...
void waveboxcontext_updateoffsetstep(WaveBoxContext* self)
{
	...
	if (self->zoomright - self->zoomleft >= 0.0) {
...
double waveboxcontext_frame_at(WaveBoxContext* self, double frame)
{
...
static psy_ui_RealRectangle wavebox_framerangetoscreen(WaveBox*,
	uintptr_t framebegin, uintptr_t frameend);
static int wavebox_hittest_range(WaveBox*, uintptr_t framemin,
...
static double wavebox_amp(WaveBox*, double frame);
static void wavebox_on_sample_loop_changed(WaveBox* self,
...
{			
	...
	psy_signal_connect(&self->component.signal_mouse_down, self,
		wavebox_on_mouse_down);
	psy_signal_connect(&self->component.signal_mouse_move, self,
		wavebox_onmousemove);
	psy_signal_connect(&self->component.signal_mouse_up, self,
		wavebox_on_mouse_up);
	psy_signal_connect(&self->component.signal_mouse_double_click, self,
		wavebox_onmousedoubleclick);
	psy_signal_connect(&self->component.signal_language_changed, self,
...
{		
	...
	if (sample) {
		psy_signal_connect(&sample->signal_loop_changed, self,
			wavebox_on_sample_loop_changed);
	}
}
...
{		
	...
	if (!self->context.sample) {		
		...
		scaley = (size.height / 2.0) / 32768.0;
		if (self->context.sample && self->context.sample->loop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
			psy_ui_graphics_draw_solid_rectangle(g, cont_loop_rc, psy_ui_colour_make(0x00303030));
			if (self->context.loopviewmode == WAVEBOX_LOOPVIEW_CONT_DOUBLE) {
				psy_ui_graphics_draw_solid_rectangle(g, cont_doubleloop_rc, psy_ui_colour_make(0x00292929));
			}
		}
		if (self->context.sample && self->context.sample->sustainloop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
			psy_ui_graphics_draw_solid_rectangle(g, sustain_loop_rc, psy_ui_colour_make(0x00444444));
			if (self->context.loopviewmode == WAVEBOX_LOOPVIEW_SUSTAIN_DOUBLE) {
				psy_ui_graphics_draw_solid_rectangle(g, sustain_doubleloop_rc, psy_ui_colour_make(0x00292929));
			}
		}
		if (self->context.selection.hasselection) {
			psy_ui_graphics_draw_solid_rectangle(g, wavebox_framerangetoscreen(self,
		...
		for (x = clip.left; x < clip.right; ++x) {
			...
			frame = (double)((self->context.offsetstep * x +
			...
			if (self->context.selection.hasselection &&
				...
				if (realframe == self->context.selection.start ||
					realframe == (self->context.selection.end)) {
					psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00333333));
				} else {
					psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00262626));
				}
				...
				if (firstselstart || self->dragstarted == FALSE) {
					...
					firstselstart = FALSE;
					psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00FF2288));
				} else {
					psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00B1C8B0));
				}
			} else
			if (self->context.sample->loop.type != psy_audio_SAMPLE_LOOP_DO_NOT &&
				...
				psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00D1C5B6));
			} else {
				psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00B1C8B0));
			}					
			if (self->drawline) {			
		...
		if (self->context.sample && self->context.sample->sustainloop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
			psy_ui_graphics_set_colour(g, psy_ui_colour_make(0x00B6C5D1));
...
{	
	...
	if (self->context.sample && waveboxcontext_numframes(&self->context) > 0) {
		if (self->context.selection.hasselection) {
			if (wavebox_hittest(self, self->context.selection.start, psy_ui_mouseevent_pt(ev).x, 5)) {
				self->dragstarted = FALSE;
				self->dragmode = WAVEBOX_DRAG_LEFT;
				psy_ui_component_set_cursor(&self->component, psy_ui_CURSORSTYLE_COL_RESIZE);
			} else if (wavebox_hittest(self, self->context.selection.end, psy_ui_mouseevent_pt(ev).x, 5)) {
				self->dragstarted = FALSE;
				self->dragmode = WAVEBOX_DRAG_RIGHT;
				psy_ui_component_set_cursor(&self->component, psy_ui_CURSORSTYLE_COL_RESIZE);
			} else if (wavebox_hittest_range(self, self->context.selection.start,
				self->context.selection.end, psy_ui_mouseevent_pt(ev).x)) {
				self->dragstarted = FALSE;
				self->dragmode = WAVEBOX_DRAG_MOVE;
				self->dragoffset = wavebox_screentoframe(self, psy_ui_mouseevent_pt(ev).x)
					- self->context.selection.start;					
				psy_ui_component_set_cursor(&self->component, psy_ui_CURSORSTYLE_MOVE);
			} else {
		...
		if (!self->context.selection.hasselection) {
			if (self->context.sample->loop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
				if (wavebox_hittest(self, self->context.sample->loop.start, psy_ui_mouseevent_pt(ev).x, 5)) {
					self->dragmode = WAVEBOX_DRAG_LOOP_CONT_LEFT;
					psy_ui_component_set_cursor(&self->component, psy_ui_CURSORSTYLE_COL_RESIZE);
					return;
				} else
					if (wavebox_hittest(self, self->context.sample->loop.end, psy_ui_mouseevent_pt(ev).x, 5)) {
			...
			if (self->context.sample->sustainloop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
				if (wavebox_hittest(self, self->context.sample->sustainloop.start, psy_ui_mouseevent_pt(ev).x, 5)) {
					self->dragmode = WAVEBOX_DRAG_LOOP_SUSTAIN_LEFT;
					psy_ui_component_set_cursor(&self->component, psy_ui_CURSORSTYLE_COL_RESIZE);
					return;
				} else
					if (wavebox_hittest(self, self->context.sample->sustainloop.end, psy_ui_mouseevent_pt(ev).x, 5)) {
			...
			self->context.selection.start = wavebox_screentoframe(self, psy_ui_mouseevent_pt(ev).x);
...
{	
	...
	if (self->context.sample && waveboxcontext_numframes(&self->context) > 0) {
		...
		self->dragstarted = FALSE;
		changed = FALSE;		
		frame = wavebox_screentoframe(self, psy_ui_mouseevent_pt(ev).x);
		...
		if (self->dragmode == WAVEBOX_DRAG_LOOP_CONT_LEFT) {
			if (frame > self->context.sample->loop.end) {
				self->dragmode = WAVEBOX_DRAG_LOOP_CONT_RIGHT;
				psy_audio_sample_set_cont_loop(self->context.sample,
		...
		if (self->dragmode == WAVEBOX_DRAG_LOOP_SUSTAIN_LEFT) {												
			if (frame > self->context.sample->sustainloop.end) {
				self->dragmode = WAVEBOX_DRAG_LOOP_SUSTAIN_RIGHT;
				psy_audio_sample_set_sustain_loop(self->context.sample,
					psy_audio_sampleloop_make(
		...
		if (self->context.selection.hasselection &&
			(wavebox_hittest(self, self->context.selection.start, psy_ui_mouseevent_pt(ev).x, 5) ||
				wavebox_hittest(self, self->context.selection.end, psy_ui_mouseevent_pt(ev).x, 5))) {
			psy_ui_component_set_cursor(&self->component,
				psy_ui_CURSORSTYLE_COL_RESIZE);
		} else
		if (self->context.sample->loop.type != psy_audio_SAMPLE_LOOP_DO_NOT &&
			(wavebox_hittest(self, self->context.sample->loop.start, psy_ui_mouseevent_pt(ev).x, 5) ||
				wavebox_hittest(self, self->context.sample->loop.end, psy_ui_mouseevent_pt(ev).x, 5))) {
			psy_ui_component_set_cursor(&self->component,
				psy_ui_CURSORSTYLE_COL_RESIZE);
		} else
		if (self->context.sample->sustainloop.type != psy_audio_SAMPLE_LOOP_DO_NOT &&
			(wavebox_hittest(self, self->context.sample->sustainloop.start, psy_ui_mouseevent_pt(ev).x, 5) ||
				wavebox_hittest(self, self->context.sample->sustainloop.end, psy_ui_mouseevent_pt(ev).x, 5))) {
			psy_ui_component_set_cursor(&self->component,
				psy_ui_CURSORSTYLE_COL_RESIZE);
		} else
		if (self->context.selection.hasselection &&
...
uintptr_t wavebox_screentoframe(WaveBox* self, double x)
{
	...
	if (self->context.sample) {
		frame = (int)(self->context.offsetstep * x) +
			(int)(waveboxcontext_numframes(&self->context) *
				self->context.zoomleft);
		if (frame < 0) {
			frame = 0;
		} else
...
static double wavebox_amp(WaveBox* self, double frame)
{
	if (self->context.sample) {
		if (self->context.loopviewmode == WAVEBOX_LOOPVIEW_CONT_DOUBLE) {
			if (self->context.sample->loop.type != psy_audio_SAMPLE_LOOP_DO_NOT) {
...
void wavebox_on_sample_loop_changed(WaveBox* self, psy_audio_Sample* sender)
{
	assert(self);
