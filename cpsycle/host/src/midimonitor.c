	...
	psy_ui_Graphics* g)
{
	...
	psy_ui_graphics_textout(g, psy_ui_realpoint_zero(), "Ch:", psy_strlen("Ch:"));
	...
	for (ch = 0, cpx = headercolw_px; ch < psy_audio_MAX_MIDI_CHANNELS; ++ch,
		...
		if (active) {
			psy_ui_graphics_textout(g, psy_ui_realpoint_make(cpx, line_height), ".", psy_strlen("."));
		}
	}
}
...
{	
	...
	colw_px = psy_ui_value_px(&colew, tm, NULL);
	line_height = (int)(tm->tmHeight * 1.2);
	psy_ui_graphics_textout(g, psy_ui_realpoint_zero(),     "MIDI Sync: START", psy_strlen("MIDI Sync: START"));
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(0.0, line_height), "MIDI Sync: CLOCK", psy_strlen("MIDI Sync: CLOCK"));
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(0, line_height * 2), "MIDI Sync: STOP", psy_strlen("MIDI Sync: STOP"));
	if ((*self->flags & FSTAT_FASTART) == FSTAT_FASTART) {
		psy_ui_graphics_textout(g, psy_ui_realpoint_make(headercolw_px, 0.0), ".", psy_strlen("."));
	}
	if ((*self->flags & FSTAT_F8CLOCK) == FSTAT_F8CLOCK) {
		psy_ui_graphics_textout(g, psy_ui_realpoint_make(headercolw_px, line_height), ".", psy_strlen("."));
	}
	if ((*self->flags & FSTAT_FCSTOP) == FSTAT_FCSTOP) {
		psy_ui_graphics_textout(g, psy_ui_realpoint_make(headercolw_px, line_height * 2), ".", psy_strlen("."));
	}
}
...
{
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(colx_px[0], y), "Channel",
		psy_strlen("Channel"));
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(colx_px[1], y), "Generator/Effect",
		psy_strlen("Generator/Effect"));
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(colx_px[2], y), "Instrument",
		psy_strlen("Instrument"));
	psy_ui_graphics_textout(g, psy_ui_realpoint_make(colx_px[3], y), "Note Off",
		psy_strlen("Note Off"));
}
