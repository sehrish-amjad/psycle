...
/* prototypes */
static void valueui_on_preferred_size(ValueUi*, const psy_ui_Size* limit,
	psy_ui_Size* rv);
...
static void valueui_on_mouse_move(ValueUi*, psy_ui_MouseEvent*);
	...
	psy_audio_MachineParam* param)
{
	...
	psy_ui_component_set_encoding(psy_ui_label_base(&self->label),
		PSY_ENCODING_NONE);
...
ValueUi* valueui_alloc_init(psy_ui_Component* parent,
	psy_audio_Machine* machine, uintptr_t paramidx,
	psy_audio_MachineParam* param)
{
	ValueUi* rv;
...
void valueui_on_preferred_size(ValueUi* self, const psy_ui_Size* limit,
	psy_ui_Size* rv)
{		
...
void valueui_on_mouse_move(ValueUi* self, psy_ui_MouseEvent* ev)
{		
	if (paramtweak_active(&self->paramtweak)) {
		paramtweak_on_mouse_move(&self->paramtweak, ev);
		psy_ui_component_set_cursor(&self->label.component,
