...
{	
	...
	psy_signal_connect(&workspace->song->sequence_.selection.signal_select, self,
		seqedittrackdesc_onsequenceselectionselect);
	psy_signal_connect(&workspace->song->sequence_.selection.signal_deselect, self,
...
void seqedittrackdesc_build(SeqEditTrackDesc* self)
{
	...
	if (sequence) {
		...
		editposition = psy_audio_sequenceselection_first(
			&self->state->cmds->workspace->song->sequence_.selection);
		for (t = 0; t < psy_audio_sequence_width(sequence); ++t) {
			...
			if (trackbox) {				
				...
				psy_ui_component_enable_drag(
					&trackbox->trackbox.track.component);
				psy_signal_connect(
					&trackbox->trackbox.track.component.signal_drag_start,
...
{		
	...
	psy_audio_sequence_set_track_height(self->state->cmds->sequence,
		...
		if (trackid != -1) {
			...
			if (index != trackid) {
				psy_audio_sequence_swaptracks(
					&self->state->workspace->song->sequence_, trackid, index);
			}
		}
	}
}
