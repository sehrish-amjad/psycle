	...
	psy_Slot reject, psy_Slot cont)
{
	...
	if (self->slot_accept.fp) {
		psy_ui_component_show(&self->yes.component);
	}
	else {
		psy_ui_component_hide(&self->yes.component);
	}
	self->slot_reject = reject;
	if (self->slot_reject.fp) {
		psy_ui_component_show(&self->no.component);
	} else {
		psy_ui_component_hide(&self->no.component);
	}
	self->slot_cont = cont;
	if (self->slot_cont.fp) {
		psy_ui_component_show(&self->cont.component);
	} else {
		psy_ui_component_hide(&self->cont.component);
	}
}
