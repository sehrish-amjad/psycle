...
{	
	...
	if (self->keyboardstate->drawpianokeys) {
		...
		psy_ui_graphics_set_text_colour(g, self->colours.keywhite);
	}
	...
	for (key = self->keyboardstate->keymin; key < self->keyboardstate->keymax; ++key) {
		...
		if (self->keyboardstate->drawpianokeys) {
			...
			psy_ui_graphics_draw_solid_rectangle(g, r, keycolour);
	...
	psy_ui_Graphics* g)
{
	...
	psy_ui_graphics_set_background_mode(g, psy_ui_TRANSPARENT);
	psy_ui_graphics_set_text_colour(g, psy_ui_colour_make(0x00333333));	
	for (key = self->keyboardstate->keymin; key < self->keyboardstate->keymax; ++key) {
		if (!psy_dsp_isblack(key)) {
			psy_ui_RealRectangle r;			
	...
	psy_ui_Graphics* g)
{
	...
	size = psy_ui_component_size_px(&self->component);
	psy_ui_graphics_set_text_colour(g, psy_ui_colour_make(0x00CACACA));
	cp = 0;
	for (key = self->keyboardstate->keymin; key < self->keyboardstate->keymax; ++key) {
		if (!psy_dsp_isblack(key)) {
			cp += self->keyboardstate->key_extent_px;
