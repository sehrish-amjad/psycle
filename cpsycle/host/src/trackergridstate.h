...
void trackconfig_settrack(TrackConfig*, uintptr_t track,
	uintptr_t numnotes);
void trackconfig_setmulticolumn(TrackConfig*, uintptr_t track, bool multi);
...
TrackerEventPair* trackereventpair_allocinit(psy_dsp_beatpos_t offset,
	psy_dsp_beatpos_t seqoffset, psy_audio_PatternEntry*);

typedef struct TrackerEventTable {
	psy_Table tracks;	
	psy_dsp_beatpos_t top;	
	uintptr_t left;
	uintptr_t right;
...
void trackereventtable_append(TrackerEventTable*, uintptr_t track,
...
double trackerstate_blit_start(TrackerState*, bool top);
...
/* quantized */
INLINE psy_dsp_beatpos_t trackerstate_px_to_beat(
...
psy_audio_SequenceCursor trackerstate_make_cursor(TrackerState*,
	psy_ui_RealPoint pt, uintptr_t index, const psy_ui_TextMetric*);
psy_ui_Style* trackerstate_column_style(TrackerState*, TrackerColumnFlags,
	uintptr_t track);
	
void trackerstate_update_textmetric(TrackerState*, const psy_ui_Font*);

INLINE bool trackerstate_playing_following_song(const TrackerState* self)
{
	assert(self);
		
	return (psy_audio_hostsequencertime_playing(
		&self->pv->player->sequencer.hostseqtime) &&
		self->pv->player->follow_song);
}

void trackerstate_scroll_to(TrackerState*, psy_ui_Component* component,
	double dx, double dy);
void trackerstate_playbar_update_range(TrackerState*,
