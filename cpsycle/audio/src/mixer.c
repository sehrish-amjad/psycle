...
{
	if (self->channel->fx) {
		psy_snprintf(text, 128, "%s", psy_audio_machine_edit_name(self->channel->fx));
	} else {
...
static void routemachineparam_tweak(psy_audio_RouteMachineParam* self, double val);
static double routemachineparam_norm_value(const psy_audio_RouteMachineParam*);
...
static void routemachineparam_vtable_init(psy_audio_RouteMachineParam* self)
{
	if (!routemachineparam_vtable_initialized) {
		...
		routemachineparam_vtable.normvalue =
			(fp_machineparam_norm_value)
			routemachineparam_norm_value;
		routemachineparam_vtable.tweak =
			(fp_machineparam_tweak)
			routemachineparam_tweak;
		routemachineparam_vtable.type =
			(fp_machineparam_type)
			routemachineparam_type;
		routemachineparam_vtable.name =
...
void routemachineparam_tweak(psy_audio_RouteMachineParam* self, double val)
{
...
double routemachineparam_norm_value(const psy_audio_RouteMachineParam* self)
{
	if (self->send < numreturncolumns(self->mixer)) {
		return psy_table_exists(&self->channel->sendsto, self->send) ? 1.0 : 0.0;
	} else 
	if (self->send == numreturncolumns(self->mixer)) {
		return self->channel->mastersend ? 1.0 : 0.0;
	}
	return 0.0;
}
...
static void mixer_describepanning(char* text, double pan);
...
// Slider/Level
static void masterchannel_level_norm_value(psy_audio_MasterChannel*,
...
static void inputchannel_sendvol_norm_value(psy_audio_InputChannel*,
	psy_audio_CustomMachineParam* sender, double* rv);
...
static void inputchannel_gain_tweak(psy_audio_InputChannel*,
	psy_audio_CustomMachineParam* sender, double value);
static void inputchannel_gain_norm_value(psy_audio_InputChannel*,
	psy_audio_CustomMachineParam* sender, double* rv);
static void inputchannel_gain_describe(psy_audio_InputChannel*,
	psy_audio_CustomMachineParam* sender, int* active, char* txt);
// Slider/Level
static void inputchannel_level_norm_value(psy_audio_InputChannel*,
	psy_audio_CustomMachineParam* sender, double* rv);
// ReturnChannel
...
// Slider/Level
static void returnchannel_level_norm_value(psy_audio_ReturnChannel*,
	psy_audio_CustomMachineParam* sender, double* rv);
...
void masterchannel_init(psy_audio_MasterChannel* self, psy_audio_Mixer* mixer, const char* name, const char* label)
{
	...
	psy_audio_doublemachineparam_init(&self->dw_param,
		"D/W", "D/W", MPF_STATE | MPF_SMALL, &self->drymix, 0, 0x1000);	
	psy_signal_connect(&self->dw_param.machineparam.signal_describe,
		self, masterchannel_dw_describe);
	psy_audio_gainmachineparam_init(&self->gain_param,
		"Gain", "Gain", MPF_STATE | MPF_SMALL, &self->gain, 0, 0x400);
	psy_audio_doublemachineparam_init(&self->pan_param,
	...
	psy_signal_connect(&self->level_param.machineparam.signal_normvalue,
		self, masterchannel_level_norm_value);	
}

void masterchannel_dispose(psy_audio_MasterChannel* self)
{
	psy_audio_infomachineparam_dispose(&self->info_param);
	psy_audio_doublemachineparam_dispose(&self->dw_param);
	psy_audio_gainmachineparam_dispose(&self->gain_param);
	psy_audio_doublemachineparam_dispose(&self->pan_param);
...
void masterchannel_dw_describe(psy_audio_MasterChannel* self,
	psy_audio_CustomMachineParam* sender, int* active, char* txt)
{
	psy_snprintf(txt, 20, "%d%%", (int)(psy_audio_machineparam_norm_value(
...
{
	mixer_describepanning(txt, psy_audio_machineparam_norm_value(sender));
	*active = 1;
}

void masterchannel_level_norm_value(psy_audio_MasterChannel* self,
	psy_audio_CustomMachineParam* sender, double* rv)
{
	...
	uintptr_t inputslot)
{
	...
	psy_audio_doublemachineparam_init(&self->mix_param,
	...
	psy_signal_connect(&self->sendvol_param.machineparam.signal_normvalue, self,
		inputchannel_sendvol_norm_value);
...
void inputchannel_dispose(psy_audio_InputChannel* self)
{
	...
	psy_audio_doublemachineparam_dispose(&self->mix_param);
...
void inputchannel_sendvol_norm_value(psy_audio_InputChannel* self,
	psy_audio_CustomMachineParam* sender, double* rv)
{
	assert(self);

	*rv = (uintptr_t)psy_table_at(&self->sendvols,
		sender->row) / (double)0xFF;
}
	...
	psy_audio_CustomMachineParam* sender, int* active, char* txt)
{
	...
	sendvol = psy_audio_machineparam_norm_value(
		psy_audio_custommachineparam_base(sender));
	if (sendvol == 0.0) {
		strcpy(txt, "Off");
	} else {
		sprintf(txt, "%.0f%%", (sendvol * 100.0));
	}
	*active = 1;
}
	...
	psy_audio_CustomMachineParam* sender, double value)
{
	...
	psy_audio_connections_set_wire_volume(&psy_audio_machine_machines(
...
void inputchannel_gain_norm_value(psy_audio_InputChannel* self,
	psy_audio_CustomMachineParam* sender, double* rv)
{
	assert(self);

	*rv = sqrt(psy_audio_connections_wire_volume(
		&psy_audio_machine_machines(psy_audio_mixer_base(self->mixer))->connections,
		psy_audio_wire_make(
			self->inputslot,
			psy_audio_machine_slot(psy_audio_mixer_base(self->mixer))))) * 0.5;
}
...
{	
	...
	psy_snprintf(txt, 10, "%.2f dB", psy_dsp_convert_amp_to_db(
		inputchannel_wirevolume(self)));
	*active = 1;	
}

void inputchannel_level_norm_value(psy_audio_InputChannel* self,
	psy_audio_CustomMachineParam* sender, double* rv)
{	
	...
	*rv = 0.f;
	...
	if (machine) {
		...
		if (memory) {
			...
			temp = ((int)(50.0 * log10((double)inputchannel_wirevolume(self) *
				self->volume))) / 97.0;
...
double inputchannel_wirevolume(const psy_audio_InputChannel* self)
{
	double rv;

	assert(self);	

	rv = psy_audio_connections_wire_volume(
		&psy_audio_machine_machines(psy_audio_mixer_base(self->mixer))->connections,
		psy_audio_wire_make(self->inputslot,
			psy_audio_machine_slot(psy_audio_mixer_base(self->mixer))));
	return rv;
}
	...
	uintptr_t fxslot)
{
	...
	psy_audio_intmachineparam_init(&self->level_param,
		"Level", "Level", MPF_LEVEL | MPF_SMALL, NULL, 0, 100);
	psy_signal_connect(&self->level_param.machineparam.signal_normvalue, self,
		returnchannel_level_norm_value);		
...
void returnchannel_level_norm_value(psy_audio_ReturnChannel* self,
	psy_audio_CustomMachineParam* sender, double* rv)
{
	psy_audio_Machines* machines;
	psy_audio_Machine* machine;
...
static void mixer_solo_norm_value(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double* rv);
static void mixer_mute_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
static void mixer_mute_norm_value(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double* rv);
static void mixer_dry_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
static void mixer_dry_norm_value(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double* rv);
static void mixer_wet_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
static void mixer_wet_norm_value(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double* rv);
// pattern
static void mixer_solo_pattern_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
static void mixer_mute_pattern_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
static void mixer_dry_pattern_tweak(psy_audio_Mixer*,
	psy_audio_IntMachineParam* sender, double value);
...
void psy_audio_mixer_init(psy_audio_Mixer* self, psy_audio_MachineCallback* callback)
{
	...
	psy_audio_machine_set_edit_name(base, "Mixer");
	...
	uintptr_t amount, psy_audio_Player* player)
{
	...
	for (input_iter = psy_table_begin(&self->inputs);
		...
		if (input && psy_audio_inputchannel_active(input)) {			
			...
			// mix input to master
			if (!input->wetonly) {
				psy_audio_buffer_add(self->master.buffer, input->buffer,
			...
			for (sendvol_iter = psy_table_begin(&input->sendvols);
				...
				if (sendvol > 0.0) {
					...
					if (returnchannel && psy_audio_returnchannel_active(returnchannel)) {
						...
						if (buffer) {
							psy_audio_buffer_add(buffer, input->buffer,
								amount, sendvol);
						}
					}					
				}
	...
	uintptr_t amount)
{
	...
	for (iter = psy_table_begin(&self->returns);
		...
		if (channel && channel->mastersend && psy_audio_returnchannel_active(channel)) {
			psy_audio_buffer_add(self->master.buffer, channel->buffer,
	...
	uintptr_t amount, psy_audio_Player* player)
{
	...
	for (iter = psy_table_begin(&self->returns);
		...
		if (channel && channel->fx && channel->buffer) {
			...
			if (psy_table_size(&channel->sendsto) >= 0) {
				for (sendsto_iter = psy_table_begin(&channel->sendsto);
					...
					if (sendto && sendto->buffer) {
							psy_audio_buffer_add(sendto->buffer, channel->buffer, amount,
								channel->volume);						
					}
				}
			}
...
{
	dsp.mul(self->master.buffer->samples[0], amount, (float)self->master.volume);
	dsp.mul(self->master.buffer->samples[1], amount, (float)self->master.volume);
}
...
void mixer_describepanning(char* text, double pan)
{
	if (pan == 0.f) {
		strcpy(text, "left");
	} else
	...
	if (pan == 0.5f) {
		...
		sprintf(text, "%.0f%%", (float)(floor(pan * 100.0)));
	}
}
...
{
	return 0xFF; // psy_audio_machine_num_parameters(psy_audio_mixer_base(self));
}
...
//     1..C -> return panning
	if (digitx0 == 0) {
		if (digit0x == 0) {
			...
			rv = psy_audio_doublemachineparam_base(&self->master.dw_param);
		} else if (digit0x == 0x0E) {
			//  E->master gain.
			rv = psy_audio_gainmachineparam_base(&self->master.gain_param);
		} else if (digit0x == 0x0F) {
			// F->master pan.
			rv = psy_audio_doublemachineparam_base(&self->master.pan_param);
		}
		...
		if (channel) {
			if (digit0x == 0x0) {
				//  0->input wet mix.
				rv = psy_audio_doublemachineparam_base(&channel->mix_param);
				...
				// F->input panning.
				rv = psy_audio_doublemachineparam_base(&channel->pan_param);
			}
		}
	} else if (digitx0 == 0xD) {
		if (digit0x == 0) {
	...
	uintptr_t slot)
{
	...
	if ((status = psyfile_read(songfile->file, &self->master.volume,
			sizeof(float)))) {
		return status;
	}
	if ((status = psyfile_read(songfile->file, &self->master.gain,
			sizeof(float)))) {
		return status;
	}
	if ((status = psyfile_read(songfile->file, &self->master.drymix,
			sizeof(float)))) {
		return status;
	}
	...
	psy_audio_machine_set_slot(&self->custommachine.machine, slot);	
	...
	for (i = 0; i < numrets; ++i) {
			...
			returnchannel_allocinit(self, i, -1));
		{
			...
			psy_table_insert(&self->legacyreturn_, i,
				psy_audio_legacywire_alloc_init_all(
			...
			psy_table_insert(&psy_audio_song_machines(songfile->song)->connections.sends,
				inputmachine, (void*)1);
		}
		{			
			int inputmachine;
	...
	uintptr_t slot)
{
	...
	size = (sizeof(solocolumn) + sizeof(volume_) + sizeof(drywetmix_) + sizeof(gain_) +
		2 * sizeof(uint32_t));
	size += (3 * sizeof(float) + 3 * sizeof(unsigned char) + (uint32_t)psy_table_size(&self->sends) * sizeof(float)) * (uint32_t)psy_table_size(&self->inputs);
	size += (2 * sizeof(float) + 2 * sizeof(unsigned char) + (uint32_t)psy_table_size(&self->sends) * sizeof(unsigned char) + 2 * sizeof(float) + sizeof(uint32_t)) * (uint32_t)psy_table_size(&self->returns);
	size += (2 * sizeof(float) + sizeof(uint32_t)) * (uint32_t)psy_table_size(&self->sends);
	...
	for (i = 0; i < (uint32_t) numins; ++i) {
		...
		for (j = 0; j < psy_table_size(&self->sends); ++j) {
			float sendvol;

			sendvol = (int)(intptr_t)psy_table_at(&channel->sendvols, j) / (float)0xFF;
	...
	for (i = 0; i < (uint32_t) numrets; ++i) {
		...
		if ((status = psyfile_write_float(songfile->file, (float)channel->volume))) {
			return status;
		}
		// Value to multiply input_convol[] to have a 0.0...1.0 range
		if ((status = psyfile_write_float(songfile->file, (float)volMultiplier))) {
			return status;
		}
	...
	uintptr_t slot)
{
	...
	psy_signal_connect(&psy_audio_song_machines(songfile->song)->connections.signal_connected,
		self, onconnected);
	psy_signal_connect(&psy_audio_song_machines(songfile->song)->connections.signal_disconnected,
		self, ondisconnected);
}
...
void postloadinputchannels(psy_audio_Mixer* self, psy_audio_SongFile* songfile, uintptr_t slot)
{
	...
	for (c = 0; c < numinputcolumns(self); ++c) {
		...
		inputmachine = psy_audio_machines_at(psy_audio_song_machines(songfile->song), wire->input_machine);
		if (wire->input_con
			...
			&& slot != wire->input_machine && inputmachine)
		{
			...
			int outWire = psy_audio_legacywires_find_legacy_output(
			...
			if (outWire != -1) {	
				...
				psy_audio_machines_connect(psy_audio_song_machines(songfile->song),
...
void postreturnchannels(psy_audio_Mixer* self, psy_audio_SongFile* songfile, uintptr_t slot)
{
	...
	for (j = 0; j <= numreturncolumns(self); ++j)
	{
		...
		inputmachine = psy_audio_machines_at(psy_audio_song_machines(songfile->song), 
			wire->input_machine);
...
void mixer_solo_norm_value(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double* rv)
{	
...
void mixer_mute_norm_value(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double* rv)
{	
	if (sender->machineparam.param0 != psy_INDEX_INVALID) {
		psy_audio_InputChannel* channel;
...
void mixer_dry_norm_value(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
	psy_audio_InputChannel* channel;
...
void mixer_wet_norm_value(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
	psy_audio_InputChannel* channel;
...
// pattern tweak
void mixer_solo_pattern_tweak(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double value)
{	
...
void mixer_mute_pattern_tweak(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double value)
{	
	if (sender->machineparam.param0 != psy_INDEX_INVALID) {
		psy_audio_InputChannel* channel;
...
void mixer_dry_pattern_tweak(psy_audio_Mixer* self,
	psy_audio_IntMachineParam* sender, double value)
{
	psy_audio_InputChannel* channel;
	uintptr_t scaled;
