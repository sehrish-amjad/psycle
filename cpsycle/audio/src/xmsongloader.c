...
static void xmsongloader_create_xmsampler(XMSongLoader*);
static int xmsongloader_read_song_name(XMSongLoader*);
static void xmsongloader_set_song_comments(XMSongLoader*);
static int xmsongloader_load_instruments(XMSongLoader*, uintptr_t instr_start);
static int xmsongloader_loadinstrument(XMSongLoader*, uintptr_t slot,
	uintptr_t instr_start, uintptr_t* rv_nextstart);
static int xmsongloader_load_sample(XMSongLoader*, psy_audio_Sample*,
...
static int xmsongloader_load_patterns(XMSongLoader*,
...
{	
	...
	xmsongloader_reset(self);
	xmsongloader_create_xmsampler(self);
	if (!self->sampler) {
		return PSY_ERRFILE;
	}
	if ((status = xmsongloader_read_song_name(self))) {
		return status;
	}
	xmsongloader_set_song_comments(self);	
	if ((status = xmsongloader_load_patterns(self, &instr_start))) {
		return status;
	}
	if ((status = xmsongloader_load_instruments(self, instr_start))) {
		return status;
	}
	return status;
}
...
void xmsongloader_create_xmsampler(XMSongLoader* self)
{	
	self->sampler = psy_audio_xmsampler_allocinit(
		self->song->machine_factory->machinecallback);		
	if (self->sampler) {		
		psy_audio_Wire wire;
		psy_audio_Machine* machine;

		if (self->song->machine_factory->createasproxy) {
			psy_audio_MachineProxy* proxy;	
			
			proxy = psy_audio_machineproxy_allocinit(
				psy_audio_xmsampler_base(self->sampler));
			if (proxy) {
				machine = psy_audio_machineproxy_base(proxy);
			} else {
				machine_base_dispose(psy_audio_xmsampler_base(self->sampler));
				free(self->sampler);
				self->sampler = NULL;
				machine = NULL;
			}
		} else {
			machine = psy_audio_xmsampler_base(self->sampler);
		}
		if (!machine) {
			return;
		}
		psy_audio_machine_set_position(psy_audio_xmsampler_base(self->sampler),
			psy_audio_MACH_AUTO_POSITION,
			psy_audio_MACH_AUTO_POSITION);
		psy_audio_machines_insert(psy_audio_song_machines(self->song), 0,
			machine);
		wire = psy_audio_wire_make(0, psy_audio_MASTER_INDEX);
		psy_audio_machines_connect(psy_audio_song_machines(self->song), wire);
		psy_audio_connections_set_wire_volume(
			&psy_audio_song_machines(self->song)->connections,
			wire, 0.355f); /* -9dB */
	}	
}

int xmsongloader_read_song_name(XMSongLoader* self)
{
	char songname[21];	
	int status;
...
void xmsongloader_set_song_comments(XMSongLoader* self)
{
	char_dyn_t* comments;
...
int xmsongloader_load_patterns(XMSongLoader* self, uintptr_t* rv_nextstart)
{	
	int xmidx;
	int i;
	int extraticks;
...
void xmsongloader_tweaksampler(XMSongLoader* self, int twk, int val)
{
	...
	param = psy_audio_machine_tweak_parameter(psy_audio_xmsampler_base(
		self->sampler), twk);
	if (param) {
		psy_audio_machine_parameter_tweak_scaled(psy_audio_xmsampler_base(
			self->sampler), param, val);
	}
}
	...
	int patidx, int tracks, uintptr_t* rv_nextstart)
{
	...
	psy_audio_pattern_set_name(pattern, "unnamed");
	psy_audio_patterns_insert(psy_audio_song_patterns(self->song), patidx, pattern);
	psy_audio_pattern_set_length(pattern,
		psy_dsp_beatpos_make_real(numrows * 1.0 / psy_audio_song_lpb(self->song),
		psy_dsp_DEFAULT_PPQ));
		...
		for (row = 0; row < numrows; ++row) {
			...
			for (col = 0; col < tracks; ++col) {
				...
				*/
				...
				if (e._mach == 0) { /*
									** fallback to the old behaviour. This will
...
int xmsongloader_load_instruments(XMSongLoader* self, uintptr_t instr_start)
{
	int32_t i;	
...
{	
	...
	psy_dsp_envelope_set_mode(&instrument->volumeenvelope, psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&instrument->panenvelope);
	psy_dsp_envelope_set_mode(&instrument->panenvelope, psy_dsp_ENVELOPETIME_TICK);	
	psy_dsp_envelope_clear(&instrument->pitchenvelope);
	psy_dsp_envelope_set_mode(&instrument->pitchenvelope, psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&instrument->filterenvelope);
	psy_dsp_envelope_set_mode(&instrument->filterenvelope, psy_dsp_ENVELOPETIME_TICK);
	/* read header */
	...
	psy_audio_instrument_set_name(instrument, instrname);
	...
	for (i = 0; i < samplecount; i++) {
		...
		wave = psy_audio_sample_alloc_init(0);		
		...
		/* Only get REAL samples.
		  if (self->smplen[*cursample] > 0) { */
			psy_audio_samples_insert(psy_audio_song_samples(self->song), wave,
	...
	for (i = 0; i < samplecount; i++) {
		...
		wave = psy_audio_samples_at(psy_audio_song_samples(self->song),
			psy_audio_sampleindex_make(slot, i));
		assert(wave);
		if (!wave) {
			return PSY_ERRFILE;
		}
		if ((status = xmsongloader_load_sample(self, wave, i, start, &start))) {
			return status;
		}
	...
	for (note = 1; note < 96; ++note) {
		if (samph.snum[note] != instentry.sampleindex.subslot) {
			...
			psy_audio_instrument_add_entry(instrument, &instentry);
...
int xmsongloader_load_sample(XMSongLoader* self, psy_audio_Sample* wave,
	int32_t sampleidx, uintptr_t start, uintptr_t* rv_nextstart)
{
	/* parse */
	...
	psy_audio_sample_alloc_wave_data(wave);
	...
	if (b16Bit)
	{
		...
		for (j = 0; j <= samplecnt - 2; j += 2)
		{
			...
			wave->channels.samples[0][out] = (float)wNew;			
			out++;
		}
	} else
	{		
		...
		for (j = 0; j < samplecnt; j++)
		{
			...
			wave->channels.samples[0][j] = (float)wNew;			
		}
	}
	/* cleanup */
	free(smpbuf);
...
void xmsongloader_setenvelopes(psy_audio_Instrument* inst, const XMSAMPLEHEADER* sampleHeader)
{
	...
	psy_dsp_envelope_set_mode(&inst->volumeenvelope, psy_dsp_ENVELOPETIME_TICK);
	if (sampleHeader->vtype & 1) { /* enable volume envelope */
		...
		** volume_fade_speed in this case, so that a check during playback time is not needed.		
		*/
		psy_audio_instrument_set_volume_fade_speed(inst,
			psy_dsp_map_32768_1(sampleHeader->volfade));				

		psy_dsp_envelope_set_enabled(&inst->volumeenvelope, TRUE);
		...
		*/
		...
		if (sampleHeader->vtype & 2) {
			psy_dsp_envelope_set_sustain_begin(&inst->volumeenvelope, sampleHeader->vsustain);
			psy_dsp_envelope_set_sustain_end(&inst->volumeenvelope, sampleHeader->vsustain);
		} else
		{
			/*
		...
		if(sampleHeader->vtype & 4){
			if(sampleHeader->vloops < sampleHeader->vloope){
				psy_dsp_envelope_set_loop_start(&inst->volumeenvelope,
					sampleHeader->vloops);
				psy_dsp_envelope_set_loop_end(&inst->volumeenvelope,
					sampleHeader->vloope);				
			}
			/*
			...
			*/
			...
			/*	inst.AmpEnvelope().LoopStart(XMInstrument::Envelope::INVALID);
				...
				psy_dsp_envelope_set_loop_start(&inst->volumeenvelope,
					psy_dsp_envelope_num_of_points(&inst->volumeenvelope) - 1);
				psy_dsp_envelope_set_loop_end(&inst->volumeenvelope,
					psy_dsp_envelope_num_of_points(&inst->volumeenvelope) - 1);				
			}
		}
		...
		{
			/* 
			** We can't ignore the loop because IT Envelopes do a fadeout
			** when the envelope end is reached and FT does not. 
			*/
			psy_dsp_envelope_set_loop_start(&inst->volumeenvelope,
				psy_dsp_envelope_num_of_points(&inst->volumeenvelope) - 1);
			psy_dsp_envelope_set_loop_end(&inst->volumeenvelope,
		...
		psy_dsp_envelope_set_enabled(&inst->volumeenvelope, FALSE);
	}
	
	/* pan envelope	*/
	psy_dsp_envelope_clear(&inst->panenvelope);
	psy_dsp_envelope_set_mode(&inst->panenvelope, psy_dsp_ENVELOPETIME_TICK);
	if(sampleHeader->ptype & 1){ /* enable volume envelope */
		int envelope_point_num;
		int i;

		psy_dsp_envelope_set_enabled(&inst->panenvelope, TRUE);
			
		if(sampleHeader->ptype & 2){
			psy_dsp_envelope_set_sustain_begin(&inst->panenvelope, sampleHeader->psustain);
			psy_dsp_envelope_set_sustain_end(&inst->panenvelope, sampleHeader->psustain);			
		}			
		if(sampleHeader->ptype & 4){
			if (sampleHeader->ploops < sampleHeader->ploope) {
				psy_dsp_envelope_set_loop_start(&inst->panenvelope,
					sampleHeader->ploops);
				psy_dsp_envelope_set_loop_end(&inst->panenvelope,
					sampleHeader->ploope);
			} else {
				psy_dsp_envelope_set_loop_start(&inst->panenvelope,
					sampleHeader->ploope);
		...
		psy_dsp_envelope_set_enabled(&inst->panenvelope, FALSE);
	}
}
...
{	
	...
	psy_audio_sample_set_name(wave, cname);	
...
int xmsongloader_loadxi(XMSongLoader* self, psy_audio_InstrumentIndex index)
{
	...
	if ((status = psyfile_read(self->fp, &fileheader,
			sizeof(XMINSTRUMENTFILEHEADER)))) {
		return status;
	}
	...
	if ((status = psyfile_read(self->fp, &insheader,
			sizeof(XMSAMPLEFILEHEADER)))) {
		return status;
	}
	...
	psy_audio_instruments_insert(psy_audio_song_instruments(self->song), instr,
		psy_audio_instrumentindex_make(index.group, instidx));
	psy_dsp_envelope_clear(&instr->volumeenvelope);
	psy_dsp_envelope_set_mode(&instr->volumeenvelope,
		psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&instr->panenvelope);
	psy_dsp_envelope_set_mode(&instr->panenvelope,
		psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&instr->pitchenvelope);
	psy_dsp_envelope_set_mode(&instr->pitchenvelope,
		psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&instr->filterenvelope);
	psy_dsp_envelope_set_mode(&instr->filterenvelope,
		psy_dsp_ENVELOPETIME_TICK);

	psy_audio_instrument_set_name(instr, fileheader.name);
	...
	} else {*/
	{
		...
		for (note = 1; note < 96; ++note) {
			if (insheader.snum[note] != instentry.sampleindex.subslot) {
				instentry.keyrange.high = note - 1;
				psy_audio_instrument_add_entry(instr, &instentry);
				instentry.keyrange.low = note;
...
static bool modsongloader_is_valid(MODSongLoader*);
static void modsongloader_create_xmsampler(MODSongLoader*);
static void modsongloader_set_sampler_panning_mode(MODSongLoader*);
static void modsongloader_set_sampler_slide_mode(MODSongLoader*);
static int modsongloader_read_song_name(MODSongLoader*);
static void modsongloader_set_song_comments(MODSongLoader*);
static void modsongloader_set_num_song_tracks(MODSongLoader*);
static void modsongloader_set_channel_panning(MODSongLoader*);
static int modsongloader_load_patterns(MODSongLoader*);
static int modsongloader_load_single_pattern(MODSongLoader*, int patidx,
	int tracks);
static unsigned char modsongloader_convert_period_to_note(
	unsigned short period);
static bool modsongloader_write_pattern_entry(MODSongLoader*,
	psy_audio_LegacyPattern, int row, int col, psy_audio_LegacyPatternEntry*);
static void modsongloader_make_sequence(MODSongLoader*, struct MODHEADER*);
static int modsongloader_load_instrument(MODSongLoader*, int idx);
static int modsongloader_load_sample_header(MODSongLoader*, psy_audio_Sample*,
	int instridx);
static int modsongloader_load_samples(MODSongLoader*, psy_audio_Sample*,
	int instridx);
...
void modsongloader_init(MODSongLoader* self, psy_audio_SongFile* songfile)
{
	...
	self->fp = songfile->file;
	self->song = songfile->song;
	modtovirtual_init(&self->modtovirtual);
...
void modsongloader_dispose(MODSongLoader* self)
{
	...
	modtovirtual_dispose(&self->modtovirtual);
}

void modsongloader_reset(MODSongLoader* self)
{	
	memset(self->smplen, 0, sizeof(self->smplen));	
	modtovirtual_clear(&self->modtovirtual);
}
...
int modsongloader_load(MODSongLoader* self)
{
	...
	status = PSY_OK;
	if (!modsongloader_is_valid(self)) {
		return PSY_ERRFILE;
	}
	modsongloader_reset(self);	
	modsongloader_create_xmsampler(self);
	if (!self->sampler) {
		return PSY_ERRFILE;
	}
	modsongloader_set_sampler_panning_mode(self);
	modsongloader_set_sampler_slide_mode(self);
	if ((status = modsongloader_read_song_name(self))) {
		return status;
	}
	modsongloader_set_song_comments(self);
	/* get samples */
	...
	for (i=0; i < 31; i++) {
		...
		wave = psy_audio_sample_alloc_init(1);	
		if ((status = modsongloader_load_sample_header(self, wave, i))) {
			psy_audio_sample_deallocate(wave);
			return status;
		}
		psy_audio_samples_insert(psy_audio_song_samples(self->song), wave,
	...
	if ((status = psyfile_read(self->fp, &self->header,
			sizeof(self->header)))) {
		return status;
	}
	modsongloader_set_num_song_tracks(self);
	...
	for (i = 0; i < 31; ++i) {
		if (self->samples[i].sampleLength > 0 ) {
			modtovirtual_set(&self->modtovirtual, i, virtidx);
			virtidx++;
		}
	}
	modsongloader_set_channel_panning(self);
	if ((status = modsongloader_load_patterns(self))) {
		return status;
	}
	for(i = 0; i < 31; ++i) {
		modsongloader_load_instrument(self, i);
		/* sampulse: 0 */
		psy_audio_song_insert_virtual_generator(self->songfile->song,
			(uintptr_t)modtovirtual_at(&self->modtovirtual, i), 0, i);
	}
	modsongloader_make_sequence(self, &self->header);
	return PSY_OK;
}

bool modsongloader_is_valid(MODSongLoader* self)
{
	bool bIsValid;
	char id[5];
	int status;
...
void modsongloader_create_xmsampler(MODSongLoader* self)
{	
	self->sampler = psy_audio_xmsampler_allocinit(
		self->song->machine_factory->machinecallback);		
	if (self->sampler) {		
...
void modsongloader_set_sampler_panning_mode(MODSongLoader* self)
{	
	psy_audio_xmsampler_setpanningmode(self->sampler, 
		psy_audio_PANNING_TWOWAY);
}

void modsongloader_set_sampler_slide_mode(MODSongLoader* self)
{
	psy_audio_xmsampler_setisamigaslides(self->sampler, TRUE);	
}

int modsongloader_read_song_name(MODSongLoader* self)
{
	char songname[21];
	int status;
...
void modsongloader_set_song_comments(MODSongLoader* self)
{
	char_dyn_t* comments;
...
void modsongloader_set_num_song_tracks(MODSongLoader* self)
{
	char pID[5];
...
void modsongloader_set_channel_panning(MODSongLoader* self)
{
	if (psy_audio_song_num_song_tracks(self->song) <= 8) {
		...
		for (i = 0; i < psy_audio_song_num_song_tracks(self->song); ++i) {
			if (((i%4) == 0) || ((i%4) == 3)) {
				psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
					&self->sampler->m_Channel[i], 0.25f, TRUE);					
			} else {
				psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
					&self->sampler->m_Channel[i], 0.75f, TRUE);				
			}
		}
	} else {
		uintptr_t i;
		...
		for (i = 0; i < psy_audio_song_num_song_tracks(self->song); ++i) {
			psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
				&self->sampler->m_Channel[i], 0.5f, TRUE);
		}
	}
}

int modsongloader_load_patterns(MODSongLoader* self)
{	
	uint8_t j;
	int i;
	int npatterns;	
	...
	for(j = 0; j < npatterns; ++j){
		...
		if ((status = modsongloader_load_single_pattern(self, j,
...
void modsongloader_make_sequence(MODSongLoader* self,
...
int modsongloader_load_single_pattern(MODSongLoader* self, int patidx, int tracks)
{
	short iNumRows = 64;
	psy_audio_Pattern* pattern;	
	...
	psy_audio_pattern_set_name(pattern, "unnamed");
	psy_audio_patterns_insert(psy_audio_song_patterns(self->song), patidx, pattern);
	psy_audio_pattern_set_length(pattern, psy_dsp_beatpos_make_real(
	...
	for(row = 0; row < iNumRows; row++)
	{
		...
		{	
			...
			type = (mentry[2] & 0x0F);
			param = mentry[3];
			_note = modsongloader_convert_period_to_note(period);
				...
				case XMCMD_SETSPEED:
					if ( param < 32)
					{
						...
						if (extraticks != 0) {
							...
							modsongloader_write_pattern_entry(self, ppattern, row,
			...
			/* if _instrument without _note, or _note without _instrument, cannot use virtual _instrument, so use sampulse directly */
				...
				if (e._inst != 255) {
					...
					if (psy_table_exists(&self->modtovirtual.table, e._inst)) {
						lastmach[col] = (uint8_t)modtovirtual_at(
							&self->modtovirtual, e._inst);
					}
				}
			}
			/* default behaviour, let's find the virtual _instrument. */
				...
				if (!psy_table_exists(&self->modtovirtual.table, e._inst)) {
					...
					e._mach = (uint8_t)modtovirtual_at(
						&self->modtovirtual, e._inst);
					e._inst = 255;					
				}
			}
			modsongloader_write_pattern_entry(self, ppattern, row, col, &e);
		}		
	}
...
unsigned char modsongloader_convert_period_to_note(unsigned short period)
{
	if (period==0) {
		return 255;
	}
...
bool modsongloader_write_pattern_entry(MODSongLoader* self,
	psy_audio_LegacyPattern pattern, int row, int col,
	psy_audio_LegacyPatternEntry* e)
{
	psy_audio_LegacyPatternEntry* pData;
...
int modsongloader_load_instrument(MODSongLoader* self, int idx)
{	
	psy_audio_Instrument* instrument;			
	...
	psy_audio_instrument_set_name(instrument, self->samples[idx].sampleName);	
	if (self->samples[idx].sampleLength > 0) {
		int status;
		psy_audio_InstrumentEntry instentry;		

		if ((status = modsongloader_load_samples(self,
			psy_audio_samples_at(psy_audio_song_samples(self->song),
...
int modsongloader_load_sample_header(MODSongLoader* self, psy_audio_Sample* wave,
	int instridx)
{	
	bool bLoop;
	char tmpfine;
	...
	psy_audio_sample_set_name(wave, self->samples[instridx].sampleName);
	return PSY_OK;
}

int modsongloader_load_samples(MODSongLoader* self, psy_audio_Sample* wave,
	int instidx)
{
	/* parse */
	uintptr_t j;
	...
	if (smpbuf) {
		...
		{			
			...
			value = (smpbuf[j] << 8);
			wave->channels.samples[0][j] = (float)value;
		}
		/* cleanup */
		free(smpbuf);
	}
