...
/// ValueType is a real value from  0 to 1.0  (or -1.0 1.0, or whatever else) which can be used as a multiplier.
typedef double XMEnvelopeValueType;
...
INLINE double xmenvelopecontroller_sratedeviation(const XMEnvelopeController* self)
{
...
typedef struct psy_audio_XMSamplerVoice {
	...
	double m_VolumeFadeSpeed;
	double m_VolumeFadeAmount;
	...
	int m_TremoloSpeed;
	int m_TremoloDepth;
	double m_TremoloAmount;
	int m_TremoloPos;

	// Panbrello
	int m_PanbrelloSpeed;
	int m_PanbrelloDepth;
	double m_PanbrelloAmount;
	int m_PanbrelloPos;
...
{
	return psy_audio_sample_auto_vibrato(self->m_WaveDataController.sample);	
}
...
INLINE double psy_audio_xmsamplervoice_realvolume(const psy_audio_XMSamplerVoice* self)
{
	return (!self->m_bTremorMute) ? (self->m_RealVolume + self->m_TremoloAmount) : 0;
}

INLINE double psy_audio_xmsamplervoice_activevolume(const psy_audio_XMSamplerVoice* self)
{
...
INLINE double psy_audio_xmsamplervoice_currrandvol(const psy_audio_XMSamplerVoice* self)
{
	return self->m_CurrRandVol;
}

INLINE void psy_audio_xmsamplervoice_setpanfactor(psy_audio_XMSamplerVoice* self, double pan)
{
	self->m_PanFactor = pan;
	self->m_PanRange = (0.5 - fabs(pan - 0.5));
}

INLINE double psy_audio_xmsamplervoice_panfactor(const psy_audio_XMSamplerVoice* self)
{
	return self->m_PanFactor;
}

INLINE double psy_audio_xmsamplervoice_panrange(const psy_audio_XMSamplerVoice* self)
{
	return self->m_PanRange;
}

INLINE double psy_audio_xmsamplervoice_activepan(const psy_audio_XMSamplerVoice* self)
{
