...
static void itmodule2_set_song_properties(ITModule2*);
static void itmodule2_create_xmsampler(ITModule2*);
...
static bool itmodule2_loaditcompresseddata(ITModule2*,
	float* pWavedata, uint32_t iLen, bool b16Bit, unsigned char convert);
...
void itmodule2_init(ITModule2* self, psy_audio_SongFile* songfile)
{
	...
	modtovirtual_init(&self->ittovirtual);	
...
void itmodule2_dispose(ITModule2* self)
{
	...
	modtovirtual_dispose(&self->ittovirtual);
	self->songfile = NULL;
...
void itmodule2_reset(ITModule2* self)
{
	...
	free(self->embeddeddata);
	self->embeddeddata = NULL;
	modtovirtual_clear(&self->ittovirtual);
}
...
{	
	...
	if ((status = psyfile_read(self->fp, &self->fileheader,
			sizeof(self->fileheader)))) {
		return status;
	}
	...
	if ((status = psyfile_read(self->fp, &self->s3mFileH,
			sizeof(self->s3mFileH)))) {
		return status;
	}
	if (psyfile_seek(self->fp, 0) == -1) {
		return PSY_ERRFILE;
...
{	
	...
	itmodule2_set_song_properties(self);
	itmodule2_create_xmsampler(self);
	if (!self->sampler) {
		return PSY_ERRFILE;
	}
	psy_audio_song_set_bpm(self->song, (double)
		self->fileheader.iTempo);
	psy_audio_song_set_tpb(self->song, 24);	
	extraticks = 0;
	psy_audio_song_set_lpb(self->song, calclpbfromspeed(self->fileheader.iSpeed,
		&extraticks));
	if (extraticks != 0) {
	...
	if (self->fileheader.insNum) {
		...
		if ((status = psyfile_read(self->fp, pointersi,
				self->fileheader.insNum * sizeof(uint32_t)))) {			
			free(pointersi);
			return status;
		}
	} else {
	...
	if (self->fileheader.sampNum) {
		...
		if ((status = psyfile_read(self->fp, pointerss,
				self->fileheader.sampNum * sizeof(uint32_t)))) {			
			free(pointersi);
			free(pointerss);
			return status;
		}
	...
	if (self->fileheader.patNum) {
		...
		if ((status = psyfile_read(self->fp, pointersp,
				self->fileheader.patNum * sizeof(uint32_t)))) {			
			free(pointersi);
			free(pointerss);
			free(pointersp);
			return status;
	...
	{			
		...
		psy_audio_instruments_insert(psy_audio_song_instruments(self->songfile->song),
	...
	for (i = 0; i < self->fileheader.patNum; i++)
	{
		if (pointersp[i] == 0)
		{
			...
			psy_audio_patterns_insert(psy_audio_song_patterns(self->songfile->song),
				i, pattern);
			psy_audio_pattern_set_length(pattern,
				psy_dsp_beatpos_make_real(64 * 1.0 / psy_audio_song_lpb(self->song),
					psy_dsp_DEFAULT_PPQ));
		} else {
	...
	psy_audio_reposition(psy_audio_song_sequence(self->songfile->song));
	free(pointersi);
	free(pointerss);
	free(pointersp);	
	return PSY_OK;
...
void itmodule2_set_song_properties(ITModule2* self)
{
	char_dyn_t* comments;
...
void itmodule2_create_xmsampler(ITModule2* self)
{		
	...
	if (self->sampler) {		
		...
		psy_audio_connections_set_wire_volume(
			&(psy_audio_song_machines(self->song)->connections), wire,
			psy_dsp_map_128_1(psy_min(self->fileheader.mVol, 128)));				
	}	
}
...
void itmodule2_setupchannels(ITModule2* self)
{
	...
	*/
	...
	for (i = 0; i < 64; ++i)
	{
		if (stereo && !(self->fileheader.chanPan[i] &
			IT2_CHAN_FLAGS_IS_DISABLED))
		{			
			if (self->fileheader.chanPan[i] == IT2_CHAN_FLAGS_IS_SURROUND)
			{		
				psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
					&self->sampler->m_Channel[i], 0.5f, TRUE);
				psy_audio_xmsamplerchannel_setdefaultissurround(
					&self->sampler->m_Channel[i], TRUE);				
			} else {
				psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
					&self->sampler->m_Channel[i], 
					(self->fileheader.chanPan[i] & 0x7F) / 64.0f,
					TRUE);		
			}
		}
		else {
		...
		psy_audio_xmsamplerchannel_setdefaultvolumefloat(
			&self->sampler->m_Channel[i], self->fileheader.chanVol[i] / 64.0f,
			TRUE);
		if ((self->fileheader.chanPan[i] & IT2_CHAN_FLAGS_IS_DISABLED))
		{
			psy_audio_xmsamplerchannel_setdefaultismute(
				&self->sampler->m_Channel[i], TRUE);
		} else {
			psy_audio_xmsamplerchannel_setdefaultismute(
				&self->sampler->m_Channel[i], FALSE);			
			self->maxextracolumn = i;
		}
		psy_audio_xmsamplerchannel_setdefaultfiltertype(
			&self->sampler->m_Channel[i], F_ITLOWPASS); 		
	}
...
int itmodule2_readmidiembedded(ITModule2* self)
{
	if (self->fileheader.special & IT2_SPECIAL_FLAGS_MIDIEMBEDDED)
	{
		...
		for (i = 0; i < 128; i++)
		{
			...
			if (strcmp(zxx2, "F0F00") == 0)
			{
				...
				psy_audio_xmsampler_setzxxmacro(self->sampler, i, mode, tmp);
			}
		}
	} 
	else /* initializing with the default midi.cfg values. */
	{
		...
		for (i = 0; i < 16; ++i)
		{
			psy_audio_xmsampler_setzxxmacro(self->sampler, i, 1, i * 8);			
		}

		for (i = 16; i < 128; ++i)
		{
			psy_audio_xmsampler_setzxxmacro(self->sampler, i, -1, 0);			
		}
	}
	return PSY_OK;
}
...
void itmodule2_tweaksampler(ITModule2* self, int twk, int val)
{
	...
	param = psy_audio_machine_tweak_parameter(
		psy_audio_xmsampler_base(self->sampler), twk);
	if (param) {
		psy_audio_machine_parameter_tweak_scaled(
			psy_audio_xmsampler_base(self->sampler), param, val);
	}
}

int itmodule2_loadolditinst(ITModule2* self, const itInsHeader1x* curh, psy_audio_Instrument* xins)
{	
	psy_audio_instrument_set_name(xins, curh->sName);

	xins->volume_fade_speed = curh->fadeout / 512.0f;
	...
	if (curh->flg & IT2_ENV_USE_ENVELOPE) {
		...
		psy_dsp_envelope_set_mode(&xins->volumeenvelope, psy_dsp_ENVELOPETIME_TICK);
		psy_dsp_envelope_set_enabled(&xins->volumeenvelope, TRUE);

		if (curh->flg & IT2_ENV_USE_SUSTAIN) {
			psy_dsp_envelope_set_sustain_begin(&xins->volumeenvelope,
				curh->sustainS);
			psy_dsp_envelope_set_sustain_end(&xins->volumeenvelope,
				curh->sustainE);
		}
		if (curh->flg & IT2_ENV_USE_LOOP) {
			psy_dsp_envelope_set_loop_start(&xins->volumeenvelope,
				curh->loopS);
			psy_dsp_envelope_set_loop_end(&xins->volumeenvelope,
				curh->loopE);
		}
		...
		for (i = 0; i < 25; i++) {
			...
			psy_dsp_envelope_append(&xins->volumeenvelope,
				psy_dsp_envelopepoint_make(tick, (double)value / 64.0));
		}
	}
...
int itmodule2_loaditinst(ITModule2* self, const itInsHeader2x* curh, psy_audio_Instrument* xins)
{
	...
	xins->volume_fade_speed = (curh->fadeout / 1024.0f);
	...
	*/
	...
	psy_dsp_envelope_clear(&xins->volumeenvelope);
	psy_dsp_envelope_set_mode(&xins->volumeenvelope, psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_set_enabled(&xins->volumeenvelope,
		curh->volEnv.flg & IT2_ENV_USE_ENVELOPE);		
	if (curh->volEnv.flg & IT2_ENV_ENABLE_CARRY) {
		psy_dsp_envelope_set_carry(&xins->volumeenvelope, TRUE);		
	}
	...
	for (i = 0; i < envelope_point_num; i++) {
		...
		psy_dsp_envelope_append(&xins->volumeenvelope,
			psy_dsp_envelopepoint_make(envtmp, (double)curh->volEnv.nodes[i].first / 64.0));
	}
	/* Pan envelope */
	psy_dsp_envelope_clear(&xins->panenvelope);
	psy_dsp_envelope_set_mode(&xins->panenvelope, psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_set_enabled(&xins->panenvelope,
		curh->panEnv.flg & IT2_ENV_USE_ENVELOPE);
	if (curh->panEnv.flg & IT2_ENV_ENABLE_CARRY) {
		psy_dsp_envelope_set_carry(&xins->panenvelope, TRUE);
	}
	if (curh->panEnv.flg & IT2_ENV_USE_SUSTAIN) {
		psy_dsp_envelope_set_sustain_begin(&xins->panenvelope,
			curh->panEnv.sustainS);
		psy_dsp_envelope_set_sustain_end(&xins->panenvelope,
			curh->panEnv.sustainE);
	}
	if (curh->panEnv.flg & IT2_ENV_USE_LOOP) {
		psy_dsp_envelope_set_loop_start(&xins->panenvelope,
			curh->panEnv.loopS);
		psy_dsp_envelope_set_loop_end(&xins->panenvelope,
			curh->panEnv.loopE);
	}
	...
	for (i = 0; i < envelope_point_num; i++) {
		...
		psy_dsp_envelope_append(&xins->panenvelope,
			psy_dsp_envelopepoint_make(envtmp, (double)curh->panEnv.nodes[i].first / 32.0));
	}

	/* Pitch/Filter envelope */
	psy_dsp_envelope_clear(&xins->pitchenvelope);
	psy_dsp_envelope_set_mode(&xins->pitchenvelope, psy_dsp_ENVELOPETIME_TICK);
	psy_dsp_envelope_clear(&xins->filterenvelope);
	psy_dsp_envelope_set_mode(&xins->filterenvelope, psy_dsp_ENVELOPETIME_TICK);
	...
	if (curh->pitchEnv.flg & IT2_ENV_ISFILTER)
	{
		...
		psy_dsp_envelope_set_enabled(&xins->filterenvelope,
			curh->pitchEnv.flg & IT2_ENV_USE_ENVELOPE);
		psy_dsp_envelope_set_enabled(&xins->pitchenvelope, FALSE);
		if (curh->pitchEnv.flg & IT2_ENV_ENABLE_CARRY) {
			psy_dsp_envelope_set_carry(&xins->filterenvelope, TRUE);
		}
		if (curh->pitchEnv.flg & IT2_ENV_USE_SUSTAIN) {
			psy_dsp_envelope_set_sustain_begin(&xins->filterenvelope,
				curh->pitchEnv.sustainS);
			psy_dsp_envelope_set_sustain_end(&xins->filterenvelope,
				curh->pitchEnv.sustainE);
		}
		if (curh->pitchEnv.flg & IT2_ENV_USE_LOOP) {
			psy_dsp_envelope_set_loop_start(&xins->filterenvelope,
				curh->pitchEnv.loopS);
			psy_dsp_envelope_set_loop_end(&xins->filterenvelope,
				curh->pitchEnv.loopE);
		}
		...
		for (i = 0; i < envelope_point_num; i++) {
			...
			psy_dsp_envelope_append(&xins->filterenvelope,
				psy_dsp_envelopepoint_make(envtmp, (double)curh->pitchEnv.nodes[i].first / 64.0));
		}
	} else {
		psy_dsp_envelope_set_enabled(&xins->pitchenvelope,
			curh->pitchEnv.flg& IT2_ENV_USE_ENVELOPE);
		psy_dsp_envelope_set_enabled(&xins->filterenvelope, FALSE);
		if (curh->pitchEnv.flg & IT2_ENV_ENABLE_CARRY) {
			psy_dsp_envelope_set_carry(&xins->pitchenvelope, TRUE);
		}
		if (curh->pitchEnv.flg & IT2_ENV_USE_SUSTAIN) {
			psy_dsp_envelope_set_sustain_begin(&xins->pitchenvelope,
				curh->pitchEnv.sustainS);
			psy_dsp_envelope_set_sustain_end(&xins->pitchenvelope,
				curh->pitchEnv.sustainE);
		}
		if (curh->pitchEnv.flg & IT2_ENV_USE_LOOP) {
			psy_dsp_envelope_set_loop_start(&xins->pitchenvelope,
				curh->pitchEnv.loopS);
			psy_dsp_envelope_set_loop_end(&xins->pitchenvelope,
				curh->pitchEnv.loopE);
		}
		...
		for (i = 0; i < envelope_point_num; i++) {
			...
			psy_dsp_envelope_append(&xins->pitchenvelope,
				psy_dsp_envelopepoint_make(envtmp, (double)curh->pitchEnv.nodes[i].first / 32.0));
		}
	}
	/* xins.ValidateEnabled(); */
	return PSY_OK;
...
int itmodule2_readsequence(ITModule2* self)
{
	...
	status = PSY_OK;
	psy_audio_sequence_append_track(psy_audio_song_sequence(
		self->songfile->song), psy_audio_sequencetrack_alloc_init());
	i = 0;
	for (j = 0; j < self->fileheader.ordNum; ++j) {
		uint8_t patidx;
        ...
        if (patidx != 254 && patidx != 255 ) {			
			psy_audio_sequence_insert(
				psy_audio_song_sequence(self->songfile->song),
				psy_audio_orderindex_make(0, i), patidx);
			++i;
		}
	}
	if (i == 0) {
		/* Add at least one pattern to the sequence. */
		psy_audio_sequence_insert(psy_audio_song_sequence(self->songfile->song),
			psy_audio_orderindex_make(0, 0), 0);
	}
	if (psyfile_skip(self->fp, self->fileheader.ordNum - j) == -1) {
		return PSY_ERRFILE;
	}	
	return status;
}
...
bool itmodule2_loaditsample(ITModule2* self, psy_audio_Sample* _wave)
{
	...
	*/
	...
	if (curh.flg & IT2_SAMPLE_FLAGS_HAS_SAMPLE)
	{
		...
		psy_audio_sample_alloc_wave_data(_wave);		
		...
		*/
		...
		psy_audio_sample_set_name(_wave, renamed);
...
bool itmodule2_loaditsampledata(ITModule2* self, psy_audio_Sample* _wave, uint32_t iLen, bool bstereo, bool b16Bit, unsigned char convert)
{
	...
	if (b16Bit) {
		for (j = 0; j < iLen; j += 2) {
			...
			_wave->channels.samples[0][out] = (float)wNew;						
			out++;
		}
		if (bstereo) {
			psyfile_read(fp, smpbuf, iLen);
...
bool itmodule2_loaditcompresseddata(ITModule2* self, float* pWavedata, uint32_t iLen, bool b16Bit, unsigned char convert)
{
	...
	while (j < iLen) /* While we haven't decompressed the whole sample */
	{
		...
		while (blockpos < blocksize) {
			...
			pWavedata[j + blockpos] = (float)wNew;			
...
bool itmodule2_loaditpattern(ITModule2* self, int patidx, int* numchans)
{
	...
	pattern = psy_audio_pattern_alloc_init();
	psy_audio_pattern_set_name(pattern, "unnamed");
	psy_audio_patterns_insert(psy_audio_song_patterns(song), patidx, pattern);
	psy_audio_pattern_set_length(pattern,
		psy_dsp_beatpos_make_real(rowCount * 1.0 / psy_audio_song_lpb(self->song),
		psy_dsp_DEFAULT_PPQ));
	...
	for (row = 0; row < rowCount; row++)
	{
		...
		while (newEntry)
		{
			...
			if (mask[channel] & 1)
			{
				...
				else if (note == 254) pent.note = psy_audio_NOTECOMMANDS_RELEASE; /*! @todo: Attention ! Psycle doesn't have a note-cut note. */
				else pent.note = note;
			...
			/* if instrument without note, or note without instrument, cannot use virtual instrument, so use sampulse directly */
				...
				if (pent.inst != 255) {
					...
					if (psy_table_exists(&self->ittovirtual.table, pent.inst)) {
						lastmach[channel] = (uint8_t)modtovirtual_at(
							&self->ittovirtual, pent.inst);
					}
				}
			}
			/* default behaviour, let's find the virtual instrument. */
				...
				if (!psy_table_exists(&self->ittovirtual.table, pent.inst)) {						
					...
					lastinst[channel] = (uint8_t)pent.inst;
					pent.mach = (uint8_t)(uint8_t)modtovirtual_at(
						&self->ittovirtual, pent.inst);
					pent.inst = 255;
					lastmach[channel] = pent.mach;
				}
		...
		if (!append) {
			insert = psy_audio_pattern_find_node(pattern, col,
				psy_dsp_beatpos_make_real(
					(double)(row * 1.0 / psy_audio_song_lpb(self->song)),
					psy_dsp_DEFAULT_PPQ),
				psy_dsp_beatpos_make_real(1.0 / psy_audio_song_lpb(self->song),
					psy_dsp_DEFAULT_PPQ),
				&prev);
		} else {
			insert = NULL;
...
static bool itmodule2_loads3msampledatax(ITModule2*, psy_audio_Sample*,
	uint32_t iLen, bool bstereo, bool b16Bit, bool packed);
static bool itmodule2_loads3msamplex(ITModule2* self, psy_audio_Sample* _wave,
	s3mSampleHeader* currHeader);
...
bool itmodule2_loads3minstx(ITModule2* self, uint16_t iSampleIdx)
{
	...
	psy_audio_instruments_insert(psy_audio_song_instruments(song), instr,
		psy_audio_instrumentindex_make(0, iSampleIdx));
		
	psy_audio_instrument_set_name(instr, curh.sName);	
	...
	if (curh.tag == SCRS_ID && curh.type == 1)
	{
		...
		wave = psy_audio_sample_alloc_init(1);
		psy_audio_samples_insert(psy_audio_song_samples(song), wave,
			psy_audio_sampleindex_make(iSampleIdx, 0));
		result = itmodule2_loads3msamplex(self, wave,
			(s3mSampleHeader*)(&curh));		
		if (result) {
			/*
...
bool itmodule2_loads3msamplex(ITModule2* self, psy_audio_Sample* _wave, s3mSampleHeader* currHeader)
{
	...
	psy_audio_sample_set_name(_wave, currHeader->filename);

	newpos = ((currHeader->hiMemSeg << 16) + currHeader->lomemSeg) << 4;
	psyfile_seek(fp, newpos);
	return itmodule2_loads3msampledatax(self, _wave, currHeader->length,
		bstereo, b16Bit, currHeader->packed);
}

bool itmodule2_loads3msampledatax(ITModule2* self, psy_audio_Sample* wave,
	uint32_t iLen, bool bstereo, bool b16Bit, bool packed)
{
	PsyFile* fp;
	psy_audio_Song* song;
		...
		*/
		...
		if (b16Bit) {
			...
			if (bstereo) {
				...
				{
					wNew = ((0xFF & smpbuf[j]) | (smpbuf[j + 1] << 8)) + offset;
					wave->channels.samples[1][out] = (float)wNew;					
				}
			}
		} else { /* 8 bit sample */			
			unsigned int j;
...
bool itmodule2_loads3mpatternx(ITModule2* self, uint16_t patidx)
{
	...
	psy_audio_pattern_set_length(pattern,
		psy_dsp_beatpos_make_real(
			64 * 1.0 / psy_audio_song_lpb(self->song),
			psy_dsp_DEFAULT_PPQ));
		...
		while (newEntry)
		{
			...
			/* default behaviour, let's find the virtual instrument. */
				...
				if (!psy_table_exists(&self->ittovirtual.table, pent.inst)) {
					...
					pent.mach = (uint8_t)modtovirtual_at(&self->ittovirtual,
						pent.inst);
					pent.inst = 255;
					lastmach[channel] = pent.mach;
				}
