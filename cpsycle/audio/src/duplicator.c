	...
	psy_audio_MachineCallback* callback)
{
	...
	self->isticking = 0;
	psy_audio_machine_set_edit_name(psy_audio_duplicator_base(self), "Note Duplicator");
			...
			for (it = psy_audio_duplicatormap_begin(&self->map);
				...
				if (output->machine != -1) {					
					...
					mapchannel = psy_audio_duplicatormap_channel(&self->map,
						psy_audio_patternentry_track(patternentry), output);
					if (newentry) {						
					...
					if (psy_audio_patternentry_front(patternentry)->note >= 
						...
						psy_audio_duplicatormap_release(&self->map,
							psy_audio_patternentry_track(patternentry), mapchannel, output);
					}
				}
			}
		}
