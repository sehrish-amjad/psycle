...
static void set_curr_program(psy_audio_LV2Plugin*, uintptr_t prgidx);
...
{	
	...
	psy_audio_machine_set_callback(psy_audio_lv2plugin_base(self), callback);
	...
	for (it = lilv_plugins_begin(plugins);
		...
		if (i == shell_index) {		
			...
			const LV2_Feature* features[]    = { &map_feature, &unmap_feature, 
				NULL }; // &schedule_feature
	...
	if (!psy_audio_machine_edit_name(psy_audio_lv2plugin_base(self))) {
		psy_audio_machine_set_edit_name(psy_audio_lv2plugin_base(self),
			lilv_node_as_string(lilv_plugin_get_name(self->plugin)));
	}
	return status;
} 
			...
			if (lilv_port_is_a(self->plugin, port, self->input_port_class)) {
				if (indexinput < 2) {
					lilv_instance_connect_port(self->instance,
						lPortIndex, psy_audio_buffer_at(bc->output_,
						indexinput));
...
void generateaudio(psy_audio_LV2Plugin* self, psy_audio_BufferContext* bc)
{
	...
	if (psy_audio_buffercontext_num_samples(bc) > 0) {
		...
		lilv_instance_run(self->instance, (uint32_t)psy_audio_buffercontext_num_samples(bc));
	}
}
...
void set_curr_program(psy_audio_LV2Plugin* self, uintptr_t prgidx)
{
	assert(self);
