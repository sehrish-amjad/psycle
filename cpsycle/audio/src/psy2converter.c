	...
	int* index, int type, const char* name)
{
	...
	machine = psy_audio_machinefactory_make_machine(songfile->song->machine_factory,
		psy_audio_PLUGIN, plugincatchername, psy_INDEX_INVALID);
	psyfile_read(songfile->file, _editName, 16);
	_editName[15] = 0;
	if (machine) {
		psy_audio_machine_set_edit_name(machine, _editName);
	}
	if (type == nativeplug) {
...
void internalmachineconverter_retweak_song(InternalMachinesConvert* self, psy_audio_Song* song, double samplerate)
{
	...
	for (it = psy_table_begin(&psy_audio_song_patterns(song)->slots);
			!psy_tableiterator_equal(&it, psy_table_end());
			psy_tableiterator_inc(&it)) {
		...
		for (p = psy_audio_pattern_begin(pattern); p != NULL; psy_list_next(&p)) {
			...
			machine = psy_audio_machines_at(psy_audio_song_machines(song), event->mach);
	...
	int* integral_value)
{
	...
	sr = samplerate;
	bps = (psy_audio_song_bpm(song) * (double) 1.f) /
		(sr * 60.0f);
	bpl = 1 / (double)psy_audio_song_lpb(song);
	spr =  bpl * 1/bps;
