...
/* prototypes */
static int psy_audio_psy3loader_read_version(psy_audio_PSY3Loader*);
static int psy_audio_psy3loader_read_chunks(psy_audio_PSY3Loader*);
static int psy_audio_psy3loader_read_chunk(psy_audio_PSY3Loader*,
	const char* header);
...
static int psy_audio_psy3loader_loadismachinebus(psy_audio_PSY3Loader*,
	psy_audio_Machine*);
static void psy_audio_psy3loader_set_instrument_names(
	psy_audio_PSY3Loader*);
static void psy_audio_psy3loader_postload(psy_audio_PSY3Loader*);
/* song machine test */
static int psy_audio_psy3loader_test_machine_chunks(psy_audio_PSY3Loader*);
static int psy_audio_psy3loader_read_machine_chunk(psy_audio_PSY3Loader*,
	const char* header);
static int psy_audio_psy3loader_test_macd(psy_audio_PSY3Loader*);
static int psy_audio_psy3loader_test_machine_load_chunk(
	psy_audio_PSY3Loader*, int32_t index);
...
{	
	...
	status = psy_audio_psy3loader_read_chunks(self);
	psy_audio_reposition(psy_audio_song_sequence(self->song));
	psy_audio_psy3loader_set_instrument_names(self);
	psy_audio_psy3loader_postload(self);
	return status;
}

int psy_audio_psy3loader_read_version(psy_audio_PSY3Loader* self)
{	
	uint32_t temp32;
	int status;
	...
	if (self->fp->fileversion >= 8) {
		...
		*/
		if (self->fp->filesize > bytesread) {
			if (psyfile_skip(self->fp, self->fp->filesize -
...
int psy_audio_psy3loader_read_chunks(psy_audio_PSY3Loader* self)
{
	char header[9];
	int status;
	...
	header[4] = 0;		
	while ((psyfile_read(self->fp, header, 4) == PSY_OK) &&
		self->fp->chunkcount) {
		if ((status = psy_audio_psy3loader_read_chunk(self, header))) {
			/*
		...
		if (self->songfile->logger) {
			psy_logger_progress(self->songfile->logger,
...
int psy_audio_psy3loader_read_chunk(psy_audio_PSY3Loader* self,
	const char* header)
{		
	static Chunk chunks[] = {
	...
	for (c = 0; chunks[c].read != NULL; ++c) {
		if (strcmp(header, chunks[c].header) == 0) {
			...
			if (psyfile_readchunkbegin(self->fp) || (strcmp(header,
					"PATD") == 0)) {
...
void psy_audio_psy3loader_set_instrument_names(psy_audio_PSY3Loader* self)
{
	...
	for (it = psy_audio_instruments_groupbegin(psy_audio_song_instruments(self->song), 0);
			!psy_tableiterator_equal(&it, psy_table_end());
			psy_tableiterator_inc(&it)) {
		...
		sample = psy_audio_samples_at(psy_audio_song_samples(self->song),
...
{		
	...
	*/
	...
	songtracks = temp;
	psy_audio_song_set_num_song_tracks(self->song, songtracks);
	/* bpm */
	{ /* \todo: this was a hack added in 1.9alpha to allow decimal bpm
		...
		bpmfine = temp16;
		psy_audio_song_set_bpm(self->song, bpmcoarse +
			(bpmfine / 100.0));
	}
	/* linesperbeat */
	if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
	...
	psy_audio_song_set_lpb(self->song, temp);	
	/* current octave */
	if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
		return status;
	}
	psy_audio_song_set_octave(self->song, temp);	
	/*
	** machinesoloed
	...
	*/
	...
	for(i = 0 ; i < songtracks; ++i)
	{
		...
		if (temp != FALSE) {
			psy_audio_patterns_mute_track(psy_audio_song_patterns(self->song), i);
		}		
		/* trackarmed */
		if ((status = psyfile_read(self->fp, &temp, sizeof(temp)))) {
			return status;
		...
		if (temp != FALSE) {
			psy_audio_patterns_arm_track(psy_audio_song_patterns(self->song), i);
		}		
	}
	if (tracksoloed >= 0) {
		psy_audio_patterns_set_solo_track(psy_audio_song_patterns(self->song),
			tracksoloed);
	}
	...
	if(self->fp->currchunk.version >= 1) {
		...
		psy_audio_song_patterns(self->song)->sharetracknames = sharetracknames != FALSE;
		if (sharetracknames) {
			int32_t t;
			...
			for(t = 0; t < songtracks; ++t) {
				...
				if ((status = psyfile_readstring(self->fp, txt, 512))) {
					return status;
				}
				psy_audio_patterns_set_shared_track_name(
					psy_audio_song_patterns(self->song), t, txt);				
			}			
		}
	}
	if (self->fp->currchunk.version >= 2) {
		...
		psy_audio_song_set_tpb(self->song, temp);				
		if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
			return status;
		}
		psy_audio_song_set_extra_ticks_per_beat(self->song, temp);		
	} else {
	...
	if (self->fp->currchunk.version >= 3) {
		...
		psy_audio_song_set_sampler_index(self->song, utemp);
	}
	if (self->fp->currchunk.version >= 4) {
		if ((status = psyfile_read(self->fp, &utemp, sizeof utemp))) {
			return status;
		}
		if (utemp != 0) {
...
/*	if (fullopen) */
	{
		/* 
		** \todo: warning! this is done here, because the plugins, when loading,
		** need an up-to-date information.
		** it should be coded in some way to get this information from the
		** loading song, since doing it here
...
{	
	...
	sequence = psy_audio_song_sequence(self->song);	
	...
	for (i = (uint32_t)psy_audio_sequence_width(sequence); i <= (uint32_t)index;
			++i) {
		psy_audio_sequence_append_track(sequence,
			psy_audio_sequencetrack_alloc_init());
	}
	track = psy_audio_sequence_track_at(sequence, index);		
	assert(track);
	...
	psy_audio_sequencetrack_set_name(track, ptemp);
	for (i = 0; i < (uint32_t)playlength; ++i) {		
		...
		if (patternslot == INT32_MAX - 2) {
			...
			*/
			psy_audio_sequence_insert_sample(sequence,
				psy_audio_orderindex_make(index, i),
				psy_audio_sampleindex_zero());
			...
			*/
			psy_audio_sequence_insert_marker(sequence,
				psy_audio_orderindex_make(index, i),
				"untitled");
		} else {
	...
	if (self->fp->currchunk.version > 0) {
		...
		*/
		...
		for (p = track->nodes, i = 0; i < (uint32_t)playlength && p != NULL;
			...
			if ((status = psyfile_read(self->fp, &repositionoffset,
					sizeof(float)))) {
				return status;
			}			
			seqentry->repositionoffset = psy_dsp_beatpos_make_real(
				repositionoffset, psy_dsp_DEFAULT_PPQ);
		}			
	}
	if (self->fp->currchunk.version > 1) {
		/*
		...
		*/
		...
		for (s = track->nodes, c = 0; s != NULL && c < nummarkers;
				psy_list_next(&s)) {
			psy_audio_SequenceEntry* seqentry;
			...
			if (seqentry->type == psy_audio_SEQUENCEENTRY_MARKER) {
				...
				marker->length = psy_dsp_beatpos_make_real(length,
					psy_dsp_DEFAULT_PPQ);
		...
		for (s = track->nodes, c = 0; s != NULL && c < numsamples;
				psy_list_next(&s)) {
			psy_audio_SequenceEntry* seqentry;
			...
			if (seqentry->type == psy_audio_SEQUENCEENTRY_SAMPLE) {
				...
				if ((status = psyfile_read(self->fp, &subslot,
						sizeof(int32_t)))) {
					return status;
				}
				sample->sampleindex.subslot = subslot;
				sample->sampleindex.slot = slot;
				if ((status = psyfile_read(self->fp, &samplerindex,
						sizeof(uint32_t)))) {
					return status;
				}
		...
		psy_audio_sequencetrack_set_height(track, tmp_float);
		if (c != numsamples) {
			return PSY_ERRFILE;
		}
	}
...
{			
	...
	pattern = NULL;
	lpb = (int32_t)psy_audio_song_lpb(self->song);
	bpl = 1 / (double) lpb;
	if ((status = psyfile_read(self->fp, &index, sizeof index))) {
		return status;
	}	
	...
	*/
	...
	if (self->fp->currchunk.version > 1) {
		...
		psy_audio_patterns_insert(psy_audio_song_patterns(self->song), index, pattern);
		psy_audio_pattern_set_name(pattern, patternname);
	} else {
		...
		psy_audio_convert_legacypattern(
			pattern, pdest,
			(uint32_t)psy_audio_song_num_song_tracks(self->song),
			patternlines,
			lpb);
		free(pdest);
		pdest = 0;
	...
	if (self->fp->currchunk.version > 0) {
		if (!self->song->patterns_.sharetracknames) {
			uint32_t t;
			for (t = 0; t < self->song->patterns_.songtracks; t++) {
				char name[512];

				if ((status = psyfile_readstring(self->fp, name, 512))) {
					return status;
				}
				psy_audio_pattern_set_track_name(pattern, t, name);				
			}
		}
	}
	if (self->fp->currchunk.version > 1) {
...
{				
	...
	pattern->time_sig_.cmd = utemp;
	if ((status = psyfile_read(self->fp, &utemp, sizeof utemp))) {
		return status;
	}
	pattern->time_sig_.parameter = utemp;
	/* ppq res */
	if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
		return status;
	}	
	/* pattern length in ppq ticks */
	if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
		return status;
	}
	beats.ticks = temp;
	beats.res = psy_dsp_DEFAULT_PPQ;
	psy_audio_pattern_set_length(pattern, beats);	
	/* num entries */
	if ((status = psyfile_read(self->fp, &temp, sizeof temp))) {
		return status;
	}	
	...
	for (c = 0; c < numentries; ++c) {
		...
		entry->events_ = 0; /* todo */
		/* read track */
		if ((status = psyfile_read(self->fp, &temp, sizeof(temp)))) {
			return status;
		}
		psy_audio_patternentry_set_track(entry, temp);
		/* read offset in ppq ticks */
		if ((status = psyfile_read(self->fp, &temp, sizeof(temp)))) {
			return status;
		}
		entry->offset_.ticks = temp;
		entry->offset_.res = psy_dsp_DEFAULT_PPQ;
		/* num multi events */
		if ((status = psyfile_read(self->fp, &temp, sizeof(temp)))) {
			return status;
		}
		...
		for (j = 0; j < numevents; ++j) {
			...
			ev.parameter = temp;
			psy_audio_patternentry_add_event(entry, ev);
		}
		psy_list_append(&pattern->events_, entry);
	}	
	return PSY_OK;
}
...
{	
	...
	{	
		...
		*/
		...
		psy_audio_instrument_set_name(instrument, instrum_name);
...
int psy_audio_psy3loader_xmloadwav(psy_audio_PSY3Loader* self, psy_audio_Sample* wave)
{
	...
	psy_audio_sample_set_name(wave, wavename);
	/* wavelength */
	if ((status = psyfile_read(self->fp, &temp, sizeof(temp)))) {
...
{		
	...
	*/
	...
	psy_audio_instrument_set_name(instrument, name);
	...
	if (instrument) {
		instrument->volume_fade_speed = m_VolumeFadeSpeed;
	}
	...
	}
	{
		...
		for(i = 0; i < note_map_size; i++) {
			...
			note = i;			
			if ((status = psyfile_read(self->fp, &targetnote,
					sizeof(targetnote)))) {
				return status;
			}
			if ((status = psyfile_read(self->fp, &sampleslot,
					sizeof(sampleslot)))) {
				return status;
			}			
			if (instrument) {
	...
	assert(instrument);
	if ((status = psy_audio_psy3loader_xminstrumentenvelopeload(self,
			&instrument->volumeenvelope, 0, 0))) {
		return status;
	}
	if (islegacy && legacyeins==0) {
		/* Workaround for a bug in that version */
		if ((status = psy_audio_psy3loader_xminstrumentenvelopeload(self,
				&instrument->filterenvelope, islegacy,
				self->fp->currchunk.version))) {
			return status;
		}
		if ((status = psy_audio_psy3loader_xminstrumentenvelopeload(self,
				&instrument->panenvelope, islegacy,
				self->fp->currchunk.version))) {
			return status;
		}
	} else {
	...
	if ((status = psy_audio_psy3loader_xminstrumentenvelopeload(self,
			&instrument->pitchenvelope, islegacy,
			self->fp->currchunk.version))) {
		return status;
	}
	/* ValidateEnabled(); */
...
/*
** The meaning of the first value (int), is time, and the unit depends on the
** context.
*/
typedef struct {
	int first;
	...
	bool legacy, uint32_t legacyversion)
{
	...
	bool m_Enabled;
	/*
	** if m_Carry and a new note enters, the envelope position is set to that of
	** the previous note *on the same channel
	**/
	bool m_Carry;
	/*
	** Array of Points of the envelope.
	** first : time at which to set the value. Unit can be different things
	** depending on the context.
	** second : 0 .. 1.0f . (or -1.0 1.0 or whatever else) Use it as a
	** multiplier.
	...
	*/
	...
	int32_t m_Mode;
	/* 
	** Indicates that this envelope is operated as an ADSR (it is an option
	...
	**/
	...
	}
	{
		...
		if ((status = psyfile_read(self->fp, &num_of_points,
				sizeof(num_of_points)))) {
			return status;
		}
		for(i = 0; i < num_of_points; i++){
			...
			** mode.
			*/
			if ((status = psyfile_read(self->fp, &value.first,
					sizeof(value.first)))) {
				return status;
			}
			/*
			...
			** 1.0
			*/
			if ((status = psyfile_read(self->fp, &value.second,
					sizeof(value.second)))) {
				return status;
			}
			/* todo range */
	...
	if (m_Mode == psy_dsp_ENVELOPETIME_SECONDS) {
		...
		for (c = 0, node = envelope->points; node != NULL; psy_list_next(&node),
				++c) {
	...
	*/
	...
	{ /* wave data */
		...
		if ((psyfile_currchunkversion(self->fp) &
			...
			if ((status = psyfile_read(self->fp, &samplesubidx,
					sizeof(uint32_t)))) {
				return status;
			}			
		} else {
	...
	int32_t loadIdx)
{
	...
	if ((strcmp(Header,"WAVE") == 0 &&
		...
		psy_audio_Sample* sample;
		/*
		** This index represented the index position of this wave for the
		** instrument. 0 to 15.
		**/
		...
		/* todo (value_mapper::map_256_1(pan)); */
		sample->panfactor = (float)pan / 256.f ;
		...
		psy_audio_sample_set_name(sample, instrum_name);
		...
		if (sample->stereo) {
			if ((status = psyfile_read(self->fp, &packedsize,
					sizeof(packedsize)))) {
				psy_audio_sample_deallocate(sample);
				return status;
			}
		...
		psy_audio_samples_insert(psy_audio_song_samples(self->song), sample,
	...
	psy_audio_PSY3Loader* self, int32_t index)
{
	...
	if ((status = psy_audio_psy3loader_machineloadchunk_createmachine(self,
			index, modulename, catchername, &replaced, &machine))) {
		return status;
	}
	if (!machine) {
		return PSY_ERRFILE;
	...
	if (machine) {
		psy_audio_machines_insert(psy_audio_song_machines(self->song), index, machine);
	}
	{
		...
		psy_audio_machine_set_position(machine, x, y);		
		if (bypass) {
		...
		psy_audio_machine_set_panning(machine, panning / 128.f);
	}
	machinewires = psy_audio_machinewires_alloc_init();
	...
	for (i = 0; i < MAX_CONNECTIONS; ++i) {
		...
		if ((status = psyfile_read(self->fp, &wiremultiplier,
				sizeof(wiremultiplier)))) {
			return status;
		}
		...
		legacywire = psy_audio_legacywire_alloc_init_all(input, incon, inconvol,
	...
	if (replaced) {
		...
		psy_audio_machine_set_edit_name(machine, text);
		...
		psy_audio_machine_set_edit_name(machine, editname);
	}	
	if (!replaced) {
		if ((status = psy_audio_machine_load_specific(machine, self->songfile,
				index))) {
			return status;
		}
	}
	if (psyfile_currchunkversion(self->fp) >= 1) {
		...
		if ((status = psy_audio_machine_load_wire_mapping(machine, self->songfile,
				index))) {
			return status;
		}
	}
...
{	
	...
	*rv_machine = NULL;
	...
	psy_audio_plugin_name(modulename, catchername, 0);	
	/* todo shellidx; */
	*rv_machine = psy_audio_machinefactory_make_machine(
		self->song->machine_factory, type, catchername, psy_INDEX_INVALID);
	if (!*rv_machine) {
		*rv_machine = psy_audio_machinefactory_make_machine(
			self->song->machine_factory, psy_audio_DUMMY,
...
{	
	...
	psy_audio_song_insert_virtual_generator(self->song, virtual_inst, mac_idx,
		inst_idx);
	return PSY_OK;
}

int psy_audio_psy3loader_loadpsins(psy_audio_PSY3Loader* self,
...
int psy_audio_psy3loader_check_machines(psy_audio_PSY3Loader* self)
{
	int status;
	...
	status = psy_audio_psy3loader_test_machine_chunks(self);	
	return status;
}

int psy_audio_psy3loader_test_machine_chunks(psy_audio_PSY3Loader* self)
{
	char header[9];
	int status;
	...
	while ((psyfile_read(self->fp, header, 4) == PSY_OK) &&
			self->fp->chunkcount) {
		if ((status = psy_audio_psy3loader_read_machine_chunk(self, header))) {			
			psyfile_readchunkbegin(self->fp);
			psyfile_seekchunkend(self->fp);			
		}		
	}
	return PSY_OK;
...
int psy_audio_psy3loader_read_machine_chunk(psy_audio_PSY3Loader* self,
	const char* header)
{		
	static Chunk chunks[] = {		
		{"MACD", psy_audio_psy3loader_test_macd},		
		{"    ", NULL}
	};
	uintptr_t c;
...
int psy_audio_psy3loader_test_macd(psy_audio_PSY3Loader* self)
{		
	int32_t index;
	int status;
	...
	if (index < MAX_MACHINES) {			
		if ((status = psy_audio_psy3loader_test_machine_load_chunk(self,
				index))) {
			return status;
		}
	}
...
int psy_audio_psy3loader_test_machine_load_chunk(
	psy_audio_PSY3Loader* self, int32_t index)
{
	char modulename[256];
