...
void lenvelope_work(LEnvelope* self, int num)
{
	...
	if (!lenvelope_is_playing(self) || (self->stage_ == self->sus_ &&
			!self->susdone_)) {
		psy_audio_array_fill(&self->out_, (float)self->lv_);		
		return;
	}
	...
	for (k = 0; k < num; ++k) {
		if (self->nexttime_ == self->sc_) {
			...
			if ((self->stage_ > peak_size - 1) || (!self->susdone_ &&
				...
				self->m_ = 0;
				for (j = k; j < num; ++j) {
					psy_audio_array_set(&self->out_, j, (float)self->lv_);
				}
				break;
			}
			lenvelope_calcstage(self,		
	...
	if (lua_istable(L, 2)) {
		...
		for (i = 1; i <= len; ++i) {
			...
			*t = luaL_checknumber(L, 5);			
			...
			peakval = malloc(sizeof(double));
			*peakval = luaL_checknumber(L, 5);				
			psy_table_insert(&peaks, i - 1, (void*)peakval);
			lua_pop(L, 1);
			if (argnum == 3) {
...
int setpeak(lua_State* L)
{
	...
	if (stage > 0) {
		...
		peakval = (lua_Number*)psy_table_at(&self->peaks_, stage - 1);
		if (!peakval) {
			peakval = malloc(sizeof(double));
		}
		*peakval = luaL_checknumber(L, 3);		
		psy_table_insert(&self->peaks_, stage - 1, peakval);		
	}
	return psyclescript_chaining(L);
}
