...
typedef struct psy_audio_XMSamplerChannel {
	...
	double m_Volume;///<  (0..1.0f) value used for Playback (channel volume)
	...
	double m_LastVoiceRandVol; // memory of note volume of last voice played.
	bool m_bMute; // value used for playback.

	double m_PanFactor;// (0..1.0f) value used for Playback
	int m_DefaultPanFactor;  //  0..200 .  &0x100 == Surround. // value used for Storage and reset
	double m_LastVoicePanFactor;
	...
	double m_GlobalVolSlideSpeed;
	double m_ChanVolSlideSpeed;
	double m_PanSlideSpeed;

	int m_TremoloSpeed;
	int m_TremoloDepth;
	double m_TremoloDelta;
	int m_TremoloPos;

	// Panbrello
	int m_PanbrelloSpeed;
	int m_PanbrelloDepth;
	double m_PanbrelloDelta;
	int m_PanbrelloPos;
...
INLINE double psy_audio_xmsamplerchannel_volume(const psy_audio_XMSamplerChannel* self)
{
	return self->m_Volume;
}

INLINE void psy_audio_xmsamplerchannel_setvolume(psy_audio_XMSamplerChannel* self, double value)
{
	self->m_Volume = value;
}
...
INLINE double psy_audio_xmsamplerchannel_defaultvolumefloat(const psy_audio_XMSamplerChannel* self)
{
...
INLINE void psy_audio_xmsamplerchannel_setdefaultvolumefloat(psy_audio_XMSamplerChannel* self,
	double value, bool updatecurrent)
{
...
INLINE void psy_audio_xmsamplerchannel_setlastvoicerandvol(psy_audio_XMSamplerChannel* self, double value)
{
...
INLINE double psy_audio_xmsamplerchannel_panfactor(const psy_audio_XMSamplerChannel* self)
{
	return self->m_PanFactor;
}

void psy_audio_xmsamplerchannel_setpanfactor(psy_audio_XMSamplerChannel* self, double value);
...
INLINE double psy_audio_xmsamplerchannel_defaultpanfactorfloat(const psy_audio_XMSamplerChannel* self)
{
	return (self->m_DefaultPanFactor & 0xFF) / 200.0f;
}

// default: bool ignoresurround = false
INLINE void psy_audio_xmsamplerchannel_setdefaultpanfactorfloat(
	psy_audio_XMSamplerChannel* self,
	double value, bool ignoresurround)
{
...
INLINE double psy_audio_xmsamplerchannel_lastvoicepanfactor(const psy_audio_XMSamplerChannel* self)
{
	return self->m_LastVoicePanFactor;
}

INLINE void psy_audio_xmsamplerchannel_setlastvoicepanfactor(psy_audio_XMSamplerChannel* self, double value)
{
