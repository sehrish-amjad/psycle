...
int xmsongexport_writesongheader(XMSongExport* self)
{
	...
	for (i = 0; i < MAX_BUSES; i++) {
		if (psy_audio_machines_at(psy_audio_song_machines(self->song), i)  != NULL) {
			self->isSampler[i] = hasSampler = (psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) == psy_audio_SAMPLER);
			self->isSampulse[i] = (psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) == psy_audio_XMSAMPLER);
			if (!(self->isSampler[i] || self->isSampulse[i])) {
				self->macInstruments++;
				if (psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) == psy_audio_PLUGIN) {
					const psy_audio_MachineInfo* info;

					info = psy_audio_machine_info(psy_audio_machines_at(psy_audio_song_machines(self->song), i));
					if (info && info->name) {
				...
				} else if (psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) == psy_audio_VST) {
					self->isBlitzorVst[i + 1] = TRUE;
				}
			}
		} else {
	...
	self->xmInstruments = (int32_t)psy_audio_instruments_size(psy_audio_song_instruments(self->song), 1); /* song.GetHighestXMInstrumentIndex() + 1; */
	...
	samInstruments = (int32_t)psy_audio_instruments_size(psy_audio_song_instruments(self->song), 0); /* (hasSampler) ? song.GetHighestInstrumentIndex() + 1 : 0; */
	...
	*/
	...
	self->m_Header.norder = (uint16_t)psy_audio_sequence_maxtracksize(psy_audio_song_sequence(self->song));
	self->m_Header.restartpos = 0;	
	self->m_Header.channels = (uint16_t)psy_min(psy_audio_song_num_song_tracks(self->song), 32);
	/*
	...
	*/
	self->m_Header.patterns = (uint16_t)psy_min(psy_audio_patterns_size(psy_audio_song_patterns(self->song)), 256);
	...
	self->m_Header.speed = (uint16_t)(floor(24.f / psy_audio_song_lpb(self->song))) +
		(uint16_t)psy_audio_song_extra_ticks_per_beat(self->song);
	self->m_Header.tempo = (uint16_t)psy_audio_song_bpm(self->song);

	/* Pattern order table */
	for (i = 0; i < psy_audio_sequence_track_size(psy_audio_song_sequence(self->song), 0);
			++i) {
		self->m_Header.order[i] = (uint8_t)psy_min(0xFF,
				psy_audio_sequence_patternindex(psy_audio_song_sequence(self->song),
...
int xmsongexport_savesinglepattern(XMSongExport* self, int patIdx)
{
	...
	pattern = psy_audio_patterns_at(psy_audio_song_patterns(self->song), patIdx);
	if (!pattern) {
		return PSY_ERRFILE;
	}	
	...
	maxtracks = (int32_t)psy_min(psy_audio_song_num_song_tracks(self->song), 32);
	/* check every pattern for validity */
	if (psy_audio_sequence_patternused(psy_audio_song_sequence(self->song), patIdx))
	{		
		...
		for (j = 0; j < ptHeader.rows; j++) {
			...
			for (i = 0; i < (int32_t)psy_audio_song_num_song_tracks(self->song); i++) {
			...
			for (i = 0; i < maxtracks; ++i) {
				...
				if (pData->_note == psy_audio_NOTECOMMANDS_TWEAK ||
						pData->_note == psy_audio_NOTECOMMANDS_TWEAK_SLIDE) {
				...
				if (pData->_mach < MAX_BUSES) {
					mac = psy_audio_machines_at(psy_audio_song_machines(self->song), pData->_mach);
				...
				if (mac != NULL) {
					...
					if (instr != 0) self->lastInstr[i, instr] = instr;
				}
...
{	
	...
	if (singleEffectCharacter == 0xF0) { /* Global commands */
		...
		case psy_audio_PATTERNCMD_NOTE_DELAY:
			*type = XMCMD_EXTENDED;
			*param = XMCMD_E_NOTE_DELAY | ((uint8_t)(pData->_parameter * psy_audio_song_tpb(self->song) / 256));
			break;
	...
	{ /* Sampler */
		...
		case 0x0E: {
			...
			case 0xC0:
				...
				*param = (uint8_t)((pData->_parameter * psy_audio_song_lpb(self->song)) >> 2);
				break;
			case 0xD0:
...
int xmsongexport_saveinstruments(XMSongExport* self)
{
	...
	for (i = 0; i < MAX_BUSES; i++) {
		if (psy_audio_machines_at(psy_audio_song_machines(self->song), i) != NULL &&
			psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) != psy_audio_SAMPLER &&
			psy_audio_machine_type(psy_audio_machines_at(psy_audio_song_machines(self->song), i)) != psy_audio_XMSAMPLER) {
			xmsongexport_saveemptyinstrument(self,
				psy_audio_machine_edit_name(psy_audio_machines_at(psy_audio_song_machines(self->song), i)));
		}
	}
	...
	for (j = 0, i = (self->correctionIndex == 0) ? 1 : 0; j < self->xmInstruments && j < remaining; j++, i++) {
		if (psy_audio_samples_at(psy_audio_song_samples(self->song), psy_audio_sampleindex_make(i, 0))) {
			xmsongexport_savesampulseinstrument(self, i);
		} else {
			psy_audio_Instrument* inst;

			inst = psy_audio_instruments_at(psy_audio_song_instruments(self->song), psy_audio_instrumentindex_make(1, i));
	...
	for (i = 0; i < remaining; i++) {
		if (psy_audio_samples_at(psy_audio_song_samples(self->song), psy_audio_sampleindex_make(i, 0))) {
			...
			inst = psy_audio_instruments_at(psy_audio_song_instruments(self->song), psy_audio_instrumentindex_make(0, i));
			xmsongexport_saveemptyinstrument(self, inst ? psy_audio_instrument_name(inst) : "");
		}
	}
	return PSY_OK;
...
int xmsongexport_saveemptyinstrument(XMSongExport* self, const char* name)
{
	...
	strncpy(insHeader.name, name, 22);
...
int xmsongexport_savesampulseinstrument(XMSongExport* self, int instIdx)
{
	...
	inst = psy_audio_instruments_at(psy_audio_song_instruments(self->song),
		index);
	if (!inst) {
		return PSY_ERRFILE;
	}		
	memset(&insHeader, 0, sizeof(insHeader));
	/* Names are not null terminated */
	strncpy(insHeader.name, psy_audio_instrument_name(inst), 22);
	/* insHeader.type = 0; Implicit by memset */
	memset(&samphead, 0, sizeof(samphead));	
	
	/* If no samples for this instrument, write it and exit. */
	if (psy_audio_samples_size(psy_audio_song_samples(self->song), instIdx)) {
		int status;
		...
		insHeader.samples = (uint16_t)psy_audio_samples_size(psy_audio_song_samples(self->song), instIdx);
		if ((status = psyfile_write(self->fp, &insHeader, sizeof(insHeader)))) {
			return PSY_ERRFILE;
		}
		xmsongexport_setsampulseenvelopes(self, instIdx, &samphead);
		samphead.volfade = (uint16_t)
			(floor(psy_audio_instrument_volume_fade_speed(inst) * 32768.0f));
		samphead.shsize = sizeof(XMSAMPLESTRUCT);
		if ((status = psyfile_write(self->fp, &samphead, sizeof(samphead)))) {
			return PSY_ERRFILE;
		}
		for (ite = psy_audio_samples_groupbegin(psy_audio_song_samples(self->song), instIdx);
...
int xmsongexport_savesamplerinstrument(XMSongExport* self, int instIdx)
{
	...
	sample = psy_audio_samples_at(psy_audio_song_samples(self->song), psy_audio_sampleindex_make(instIdx, 0));
	if (sample) {
		/* Names are not null terminated */
		strncpy(temp, psy_audio_sample_name(sample), 22);
	}
	/* Names are not null terminated */
	strncpy(insHeader.name, temp, 22);
	/* insHeader.type = 0; Implicit by memset */
...
int xmsongexport_savesampleheader(XMSongExport* self, int instIdx, int sampleIdx)
{
	...
	/* Names are not null terminated */
	strncpy(stheader.name, psy_audio_sample_name(wave), 22);
	...
	struct XMSAMPLEHEADER* sampleHeader)
{
	...
	if (psy_dsp_envelope_is_enabled(&inst->volumeenvelope)) {
		...
		sampleHeader->vnum = (uint8_t)(psy_min(12u, psy_dsp_envelope_num_of_points(env)));
		if (psy_dsp_envelope_mode(env) == psy_dsp_ENVELOPETIME_SECONDS) {
			convert = (24.f * (float)(psy_audio_song_bpm(self->song))) / 60000.f * 1000.f;
		}
		/*
		...
		*/
		idx = 0;
		for (i = 0; i < psy_dsp_envelope_num_of_points(env) && i < 12; i++) {
		...
		if (psy_dsp_envelope_sustain_begin(env) != psy_INDEX_INVALID) {
			sampleHeader->vtype |= 2;
			sampleHeader->vsustain = (uint8_t)(psy_dsp_envelope_sustain_begin(env));
		}
		if (psy_dsp_envelope_loop_start(env) != psy_INDEX_INVALID) {
			sampleHeader->vtype |= 4;
			sampleHeader->vloops = (uint8_t)(psy_dsp_envelope_loop_start(env));
			sampleHeader->vloope = (uint8_t)(psy_dsp_envelope_loop_end(env));
		}
	}
	if (psy_dsp_envelope_is_enabled(&inst->panenvelope)) {
		float convert;		
		...
		/* Max number of envelope points in Fasttracker format is 12. */
		sampleHeader->pnum = (uint8_t)(psy_min(12u, psy_dsp_envelope_num_of_points(env)));
		convert = 1.f;
		if (psy_dsp_envelope_mode(env) == psy_dsp_ENVELOPETIME_SECONDS) {
			convert = (24.f * (float)(psy_audio_song_bpm(self->song))) / 60000.f * 1000.f;
		}
		...
		*/
		...
		if (psy_dsp_envelope_sustain_begin(env) != psy_INDEX_INVALID) {
			...
			sampleHeader->psustain = (uint8_t)(psy_dsp_envelope_sustain_begin(env));
		}
		if (psy_dsp_envelope_loop_start(env) != psy_INDEX_INVALID) {
			sampleHeader->ptype |= 4;
			sampleHeader->ploops = (uint8_t)(psy_dsp_envelope_loop_start(env));
			sampleHeader->ploope = (uint8_t)(psy_dsp_envelope_loop_end(env));
		}
	}	
}
	...
	struct XMSAMPLEHEADER* sampleHeader)
{
	...
	index = psy_audio_instrumentindex_make(0, instrIdx);
	instrument = psy_audio_instruments_at(psy_audio_song_instruments(self->song),
		index);
	if (!instrument) {
		return;
	}
	...
	if (inst.ENV_AT != 1 || inst.ENV_DT != 1 || inst.ENV_SL != 100 || inst.ENV_RT != 220)
	{
		...
		sampleHeader->vnum = 4;
		convert = (float)(psy_audio_song_bpm(self->song)) * 24.f / (44100.f * 60.f);
		/*
		** Format of FastTracker points is :
		** Point : frame number. ( 1 frame= line*(24/TPB), samplepos= frame*(samplesperrow*TPB/24))
		** Value : 0..64. , divide by 64 to use it as a multiplier.
