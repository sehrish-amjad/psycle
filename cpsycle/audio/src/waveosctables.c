...
void psy_audio_waveosctables_makeall(psy_audio_WaveOscTables* self, double sr)
{
    double f_lo;
    int i;    
    ...
    double sr)
{
    ...
    index = psy_audio_sampleindex_make(shape, i);
    w = psy_audio_sample_alloc_init(1);
    ...
    psy_audio_instrument_add_entry(table, entry);
}
