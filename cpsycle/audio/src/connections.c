...
const psy_audio_WireSocket* psy_audio_connections_input_const(
	const psy_audio_Connections* self, psy_audio_Wire wire)
{
	return psy_audio_connections_input((psy_audio_Connections*)self, wire);
}
	...
	psy_audio_MachineSockets* sockets)
{
	...
	if (sockets) {
		...
		for (it = psy_audio_wiresockets_begin(&sockets->inputs);
			...
			socket = (psy_audio_WireSocket*)psy_tableiterator_value(&it);
			if (!psy_list_find_entry(ins, (void*)socket->slot)) {
		...
		for (it = psy_audio_wiresockets_begin(&sockets->outputs);
			...
			socket = (psy_audio_WireSocket*)psy_tableiterator_value(&it);
			if (!psy_list_find_entry(outs, (void*)socket->slot)) {
...
void psy_audio_connections_set_wire_volume(psy_audio_Connections* self,
	psy_audio_Wire wire, double factor)
{	
	psy_audio_WireSocket* input_entry;
...
double psy_audio_connections_wire_volume(const psy_audio_Connections* self,
	psy_audio_Wire wire)
{	
	const psy_audio_WireSocket* input_entry;

	input_entry = psy_audio_connections_input_const(self, wire);
	return (input_entry)
		? input_entry->volume
		: 1.0;	
}
