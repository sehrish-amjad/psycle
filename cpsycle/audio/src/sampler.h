...
{	
	...
	Double _pos;
	int64_t _speed;
	double _vol; /* 0..1 value of this voice volume */
	double _pan;
	double _lVolDest;
	double _rVolDest;
	double _lVolCurr;
...
{
	self->sratefactor = 1.0;
}

void samplerenvelope_tick(SamplerEnvelope*, int decaysamples);
...
void psy_audio_samplervoice_work(psy_audio_SamplerVoice*, uintptr_t numsamples,
	float* pSamplesL, float* pSamplesR);
int psy_audio_samplervoice_tick(psy_audio_SamplerVoice*,
...
/*!
** @struct psy_audio_Sampler
** @brief The classic Psycle sampler
**
** @details
** It is one of the two Psycle internal machines that are able to play sampled
** sounds.
*/
