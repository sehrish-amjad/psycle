...
static int alteRand(int x) { return (x * psy_rand16()) / 32768; }
...
bool samplerwavedatacontroller_postwork(SamplerWaveDataController* self)
{
	...
	{
		self->_pos.HighPart -= (uint32_t)((self->loop.end - self->loop.start));
	}
...
void samplerenvelope_tick(SamplerEnvelope* self, int decaysamples)
{
	...
	case SAMPLER_ENV_ATTACK:
		...
		if (self->_value > 1.0)
		{
			...
			self->_step = ((1.0 - self->_sustain) / decaysamples) * self->sratefactor;
		}
		break;
...
static void psy_audio_sampler_stopinstrument(psy_audio_Sampler*,
	int insIdx);
static void psy_audio_sampler_setsamplerate(psy_audio_Sampler*, double sr);
...
static const char* psy_audio_sampler_aux_column_name(psy_audio_Sampler*, uintptr_t index);
static uintptr_t psy_audio_sampler_num_aux_columns(psy_audio_Sampler*);
	...
	psy_audio_MachineCallback* callback)
{
	...
	psy_audio_machine_set_edit_name(psy_audio_sampler_base(self), "Sampler");
...
void psy_audio_sampler_setsamplerate(psy_audio_Sampler* self, double sr)
{
	uintptr_t i;
	...
	}
	{
		...
		*/
		...
		psy_audio_ticktimer_reset(&self->ticktimer,
			(uintptr_t)psy_audio_machine_samples_per_tick(
				psy_audio_sampler_base(self)));
	}
}
...
{		
	...
	*/
	...
	sampleindex = psy_audio_sampleindex_zero();
	if (data.note == psy_audio_NOTECOMMANDS_PLAY_SMPL) {
		psy_audio_Sample* sample;

		sampleindex.slot = data.inst;
		sampleindex.subslot = data.parameter;		
		sample = psy_audio_samples_at(samples, sampleindex);
		if (sample) {
			data.note = 
				self->defaultspeed ? psy_audio_NOTECOMMANDS_MIDDLEC : 48
				+ sample->zone.tune;
		} else {
			return;
		}
	...
	if (psy_audio_sampleindex_valid(&sampleindex)) {
		if (!psy_audio_samples_at(samples, sampleindex)) {
			return;
		}		
	} else if (!psy_audio_samples_at(samples, 
	...
	uintptr_t channelNum, int basec, psy_List* multicmdMem, psy_audio_SampleIndex sampleindex)
{
	...
	if (psy_audio_sampleindex_valid(&sampleindex)) {
		...
		finetune = psy_audio_sample_fine_tune(sample) * 0.01;
		...
		/* Init filter */				
		{			
			filter_settype(&self->filter, F_NONE);		
		}
		...
		self->controller._lVolCurr = self->controller._lVolDest;
		self->controller._rVolCurr = self->controller._rVolDest;
		self->controller._pos.HighPart = 0;
		...
		if (ev->inst == channelNum) {
			...
			case PS1_SAMPLER_CMD_RETRIG: {
				if ((ev->parameter & 0x0f) > 0)
				{
					...
					self->effval = (int)((psy_audio_machine_curr_samples_per_row(psy_audio_sampler_base(self->sampler))
			...
			case PS1_SAMPLER_CMD_EXTENDED: {
				...
				if ((ev->parameter & 0xf0) == PS1_SAMPLER_CMD_EXT_NOTEOFF) {
					...
					self->triggernoteoff = (uintptr_t)((psy_audio_machine_curr_samples_per_row(
					...
					/* This means there is always 6 ticks per row whatever number of rows. */
					self->triggernotedelay = (int)((psy_audio_machine_curr_samples_per_row(
						psy_audio_sampler_base(self->sampler)) / 6.f) * (ev->parameter & 0x0f));
				}
				/* } */
			} break;
	...
	if (pEntry->note < psy_audio_NOTECOMMANDS_RELEASE && !doporta)
	{
		...
		self->sampleindex = psy_audio_sampleindex_make(self->instrument, 0);
		if (self->inst) {
			psy_List* layer;
			...
			if (layer) {
				...
				if (entry->use_loop) {
					...
					self->controller._pos.HighPart = (uint32_t)entry->loop.start;
					self->controller._pos.LowPart = 0;
		...
		if (self->inst->loop)
		{
			double const totalsamples = (double)(
				psy_audio_machine_curr_samples_per_row(psy_audio_sampler_base(self->sampler))
				* self->inst->lines);
			speeddouble = (sample->numframes / totalsamples);
		} else
		{
			double finetune = psy_audio_sample_fine_tune(sample) * 0.01;
			speeddouble = pow(2.0f, (pEntry->note + tune - basec + finetune) / 12.0f) *
				(sample->samplerate / psy_audio_machine_samplerate(
					psy_audio_sampler_base(self->sampler)));
		}
		...
		/* Init Amplitude Envelope */		
		{
			...
			self->envelope._sustain =
				psy_dsp_envelope_sustain_value(&self->inst->volumeenvelope);
		...
		if (!dopan) {
			if (self->inst->randompanning != 0.f) {
				dopan = TRUE;
				self->controller._pan = psy_dsp_map_32768_1(psy_rand16());
			} else {
				dopan = TRUE;
				self->controller._pan = self->controller.wave->panfactor;
			}
		...
		if (self->inst) {
			...
			self->filterenv._sustain =
				psy_dsp_envelope_sustain_value(&self->inst->filterenvelope);
...
void psy_audio_sampler_generateaudio(psy_audio_Sampler* self, psy_audio_BufferContext* bc)
{
	...
	*/
	self->samplerowcounter += psy_audio_buffercontext_num_samples(bc);
	psy_audio_ticktimer_update(&self->ticktimer, psy_audio_buffercontext_num_samples(bc), bc);
}
	...
	psy_audio_BufferContext* bc)
{
	...
	for (voice = 0; voice < self->numvoices; ++voice) {
		psy_audio_samplervoice_work(&self->voices[voice],
			...
			psy_audio_buffer_at(bc->output_, 0),
			psy_audio_buffer_at(bc->output_, 1));
	}
}
				...
				*/
				...
				noteoff->delta = psy_dsp_beatpos_add(
					...
					psy_dsp_beatpos_make_real(
						(event->parameter & 0x0f) / 6.f *
						psy_audio_machine_curr_beats_per_line(
						psy_audio_sampler_base(self)),
						psy_dsp_DEFAULT_PPQ));
				...
				if ((event->parameter & 0xF0) == PS1_SAMPLER_CMD_EXT_NOTEDELAY) {
					...
					ev->cmd = 0;
					ev->parameter = 0;
					newentry->delta = psy_dsp_beatpos_add(
						newentry->delta,
						psy_dsp_beatpos_make_real(
							numticks * psy_audio_machine_beatspertick(
								psy_audio_sampler_base(self)),
						psy_dsp_DEFAULT_PPQ));
...
void psy_audio_samplervoice_work(psy_audio_SamplerVoice* self, uintptr_t numsamples, float* pSamplesL, float* pSamplesR)
{
	...
	} else if (self->envelope._stage == SAMPLER_ENV_OFF)
	{
		psy_audio_samplervoice_setup(self);
		return;
	} else if ((self->triggernoteoff > 0) && (self->samplecounter >= self->triggernoteoff))
	{
	...
	while (numsamples)
	{
		...
		if (filter_type(&self->filter) != F_NONE) {
			...
			ENV_F_DT = (int32_t)(psy_dsp_envelope_decay_time(
				&self->inst->filterenvelope) * 44100 + 0.5f);
			samplerenvelope_tick(&self->filterenv, ENV_F_DT);	
			if (ENV_F_DT <= 0) ENV_F_DT = 1;
			newcutoff = self->cutoff +
				(int)psy_dsp_round(self->filterenv._value * self->comodify * 128.0);
			if (newcutoff < 0) {
				newcutoff = 0;
		...
		if (self->inst) {
			right_output *= (float)(self->controller._rVolCurr * self->envelope._value);
			left_output *= (float)(self->controller._lVolCurr * self->envelope._value);
		}
...
void psy_audio_samplervoice_noteoff(psy_audio_SamplerVoice* self)
{
	...
	if (self->envelope._stage != SAMPLER_ENV_OFF)
	{
		...
		ENV_RT = (int32_t)((psy_dsp_envelope_release_time(
			&self->inst->volumeenvelope)) * 44100 + 0.5f);
		ENV_F_RT = (int32_t)((psy_dsp_envelope_release_time(
			&self->inst->filterenvelope)) * 44100 + 0.5f);
...
{	
	...
	factor = 1.0 / (12.0 * psy_audio_machine_ticks_per_beat(
		psy_audio_sampler_base(self->sampler)));
	switch (self->effcmd) {
		/* 0x01 : Pitch Up */
		case PS1_SAMPLER_CMD_PORTAUP:
...
void psy_audio_sampler_initparameters(psy_audio_Sampler* self)
{
	...
	for (quality = 0; quality < psy_dsp_RESAMPLERQUALITY_NUMRESAMPLERS;
			++quality) {
		psy_audio_choicemachineparam_set_description(&self->param_resamplingmethod,
	...
	psy_audio_choicemachineparam_init(&self->param_defaultspeed,
		...
		0, 1);
	psy_audio_choicemachineparam_set_description(&self->param_defaultspeed, 0,
		"C3");
	psy_audio_choicemachineparam_set_description(&self->param_defaultspeed, 1,
		"C4");
	psy_audio_choicemachineparam_init(&self->param_slidemode,
		"Slide mode", "Slide mode", MPF_STATE,
		(int32_t*)&self->linearslide,
		0, 1);
	psy_audio_choicemachineparam_set_description(&self->param_slidemode, 0,
		"old buggy slide");
	psy_audio_choicemachineparam_set_description(&self->param_slidemode, 1,
		"Linear slide mode");
...
const char* psy_audio_sampler_aux_column_name(psy_audio_Sampler* self,
	uintptr_t index)
{
	psy_audio_Instruments* insts;
	static char rv[256];
	...
	insts = psy_audio_machine_instruments(psy_audio_sampler_base(self));
	if (insts) {
		psy_audio_Instrument* inst;

		inst = psy_audio_instruments_at(insts, psy_audio_instrumentindex_make(
			self->instrumentbank, index));
		if (inst) {
			if (self->instrumentbank == 0) {
				psy_audio_InstrumentEntry* entry;
				psy_audio_Samples* samples;

				/* 
				** PS1 Sampler doesn't have a instrument name,
				** instead try to use the first sample name
				*/
				entry = psy_audio_instrument_entryat(inst, 0);
				if (entry) {
					samples = psy_audio_machine_samples(psy_audio_sampler_base(self));
					if (samples) {
						psy_audio_Sample* sample;
						...
						if (sample) {
							psy_snprintf(rv, 256, "%02X:%s", (int)index,
			...
			psy_snprintf(rv, 256, "%02X:%s", (int)index,
				psy_audio_instrument_name(inst));
			return rv;
		}
	}
	psy_snprintf(rv, 256, "%02X:%s", (int)index, "");
...
static uintptr_t psy_audio_sampler_num_aux_columns(psy_audio_Sampler* self)
{
	/* psy_audio_Instruments* insts; */
	...
	/* insts = psy_audio_machine_instruments(psy_audio_sampler_base(self));
	if (insts) {
		return psy_audio_instruments_size(insts, self->instrumentbank);		
	}
	return 0; */
}
