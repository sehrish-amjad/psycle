...
{	
	...
	self->randomvolume = 0.0;
	self->randompanning = 0.0;
	self->randomcutoff = 0.0;
	self->randomresonance = 0.0;
	...
	psy_dsp_envelope_set_time_and_value(&self->filterenvelope, 		
		1, 0.005f, 1.f);
	psy_dsp_envelope_set_time_and_value(&self->filterenvelope,
...
void psy_audio_instrument_set_name(psy_audio_Instrument* self, const char* name)
{
	assert(self);
...
void psy_audio_instrument_setindex(psy_audio_Instrument* self, uintptr_t index)
{
	...
	psy_audio_instrument_add_entry(self, &entry);
}
...
void psy_audio_instrument_add_entry(psy_audio_Instrument* self, const psy_audio_InstrumentEntry* entry)
{	
	assert(self);
