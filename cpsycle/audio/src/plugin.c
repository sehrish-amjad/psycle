...
static void pluginparam_tweak(psy_audio_PluginMachineParam*, double val);
static void pluginparam_reset(psy_audio_PluginMachineParam*);
static double pluginparam_norm_value(psy_audio_PluginMachineParam*);
...
static void pluginparam_vtable_init(psy_audio_PluginMachineParam* self)
{
	if (!pluginparam_vtable_initialized) {
		...
		pluginparam_vtable.normvalue = (fp_machineparam_norm_value)
			pluginparam_norm_value;
...
void pluginparam_tweak(psy_audio_PluginMachineParam* self, double value)
{
	int scaled;
...
double pluginparam_norm_value(psy_audio_PluginMachineParam* self)
{
	return ((self->cinfo->Parameters[self->index]->MaxValue - self->cinfo->Parameters[self->index]->MinValue) != 0)
		...
		(double)(self->cinfo->Parameters[self->index]->MaxValue - self->cinfo->Parameters[self->index]->MinValue)
		: 0.f;
}
...
static void psy_audio_plugin_tweak_preset(psy_audio_Plugin*, psy_audio_Preset*);
/* private methods */
...
static void set_curr_program(psy_audio_Plugin*, uintptr_t prgidx);
...
static void vtable_init(psy_audio_Plugin* self)
{
	if (!vtable_initialized) {
		...
		vtable.tweakpreset = (fp_machine_tweakpreset)psy_audio_plugin_tweak_preset;
	...
	callback, const char* path, const char* root)
{
	...
	psy_library_set_env(&self->library, path, root);
	psy_library_load(&self->library, path);
	psy_library_restore_env(&self->library);	
	self->mi = 0;
	self->plugininfo = 0;
		...
		if (!self->mi) {
			...
			if (cinfo) {				
				...
				psy_audio_plugin_machineinfo_set(self->plugininfo, cinfo,
					self->library.path);
				psy_audio_machine_set_edit_name(psy_audio_plugin_base(self),
					cinfo->ShortName);
	...
	if (!psy_audio_machine_edit_name(psy_audio_plugin_base(self))) {
		psy_audio_machine_set_edit_name(psy_audio_plugin_base(self), "Plugin");
	}
	return status;
}
...
psy_audio_Machine* clone(psy_audio_Plugin* self)
{
	...
	if (rv) {
		...
		if (rv->library.module) {
			...
			psy_audio_plugin_tweak_preset(rv, &preset);
			psy_audio_preset_dispose(&preset);
			psy_audio_machine_position(psy_audio_plugin_base(self), &x, &y);
			psy_audio_machine_set_position(psy_audio_plugin_base(rv), x, y);
		} else {
...
{	
	...
	if (path && strcmp(path, "") != 0) {
		...
		psy_library_set_env(&library, path, root);
		psy_library_load(&library, path);					
		GetInfo =(GETINFO)psy_library_functionpointer(&library, "GetInfo");
		if (GetInfo != NULL) {	
			CMachineInfo* nativeinfo = GetInfo();
			if (nativeinfo) {				
				psy_audio_plugin_machineinfo_set(rv_info, nativeinfo,
					library.path);
				rv = TRUE;
			}
		}
		psy_library_restore_env(&library);
...
{	
	mi_work(self->mi,
		...
		psy_audio_buffercontext_num_song_tracks(bc));
}
...
void currentpreset(psy_audio_Plugin* self, psy_audio_Preset* preset)
{
	...
	if (info) {
		...
		for (gbp = 0; gbp < (uintptr_t)info->numParameters; ++gbp) {
			psy_audio_preset_set_value(preset, gbp, mi_val(self->mi, gbp));
		}		
		size = mi_getdatasize(self->mi);
		if (size > 0) {
			pData = (unsigned char*)malloc(size);
			mi_getdata(self->mi, pData);
			psy_audio_preset_put_data(preset, size, pData);
			free(pData);
		}
	}
}
	...
	uintptr_t slot)
{
	...
	count = (uint32_t)psy_audio_machine_num_parameters(
	...
	for (i = 0; i < count; ++i) {
		...
		param = psy_audio_machine_parameter(psy_audio_plugin_base(self), i);
		if (param) {
			scaled = psy_audio_machine_parameter_scaled_value(
...
void set_curr_program(psy_audio_Plugin* self, uintptr_t prgidx)
{
	...
	if (self->presets) {
		...
		preset = (psy_audio_Preset*)psy_table_at(
			&self->presets->container_, self->currprog);
		if (preset) {
			psy_audio_plugin_tweak_preset(self, preset);
		}
	}
}

void psy_audio_plugin_tweak_preset(psy_audio_Plugin* self, psy_audio_Preset* preset)
{
	if (preset) {
