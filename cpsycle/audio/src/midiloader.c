...
{    
    ...
    if (self->currtrack.pattern) {
        if (self->currtrack.patternnode) {
            ...
            psy_audio_pattern_set_length(self->currtrack.pattern,
				psy_dsp_beatpos_add(psy_audio_patternentry_offset(lastentry),
					psy_dsp_beatpos_make_real(1.0, psy_dsp_DEFAULT_PPQ)));
        } else {
            psy_audio_pattern_set_length(self->currtrack.pattern,
				psy_dsp_beatpos_make_real(16.0, psy_dsp_DEFAULT_PPQ));
        }
    }
    return status;
}
...
void midiloader_appendtrack(MidiLoader* self, uintptr_t trackidx)
{
    ...
    patidx = psy_audio_patterns_append(psy_audio_song_patterns(song), pattern);
    /* append new sequence track */
    self->currtrack.track = psy_audio_sequencetrack_alloc_init();
    psy_audio_sequence_append_track(psy_audio_song_sequence(song), self->currtrack.track);
    psy_audio_sequence_insert(psy_audio_song_sequence(song),
...
int midiloader_readnoteon(MidiLoader* self)
{
    ...
    if (vol == 0) {
        ...
        if (vol != 64) {
            ev.cmd = 0xC;
            ev.parameter = (uint8_t)((vol / (double)0x7F) * 0x80);
        }
...
int midiloader_readmeta_sequencetrackname(MidiLoader* self)
{
    ...
    psy_audio_pattern_set_name(self->currtrack.pattern, text);
    free(text);
    return PSY_OK;
}
