...
{	
	...
	rv = 2;
	/* PATD */
	rv += (uint32_t)psy_audio_patterns_size(psy_audio_song_patterns(self->song));
	/* MACD */
	rv += (uint32_t)psy_audio_machines_size(psy_audio_song_machines(self->song));
	/* INSD */
	rv += (uint32_t)psy_audio_instruments_size(psy_audio_song_instruments(self->song), 0);
	/* SMID */
	rv += (uint32_t)psy_audio_instruments_smidcount(self);
	/* SMSB */
	rv += (uint32_t)psy_audio_samples_count(psy_audio_song_samples(self->song));
	/* SEQD */
	rv += (uint32_t)psy_audio_sequence_width(psy_audio_song_sequence(self->song));
	return rv;
}
...
uint32_t psy_audio_instruments_smidcount(psy_audio_PSY3Saver* self)
{
	...
	rv = 0;
	for (it = psy_audio_instruments_begin(psy_audio_song_instruments(self->song));
...
int psy_audio_psy3saver_write_sngi(psy_audio_PSY3Saver* self)
{
	...
	/* machinesoloed */
	if (psy_audio_machines_soloed(psy_audio_song_machines(self->song)) !=
			psy_INDEX_INVALID) {
		temp = (int32_t)psy_audio_machines_soloed(psy_audio_song_machines(self->song));
	} else {
		temp = -1;
	}
	...
	/* tracksoloed */
	if (psy_audio_patterns_track_soloed(psy_audio_song_patterns(self->song)) !=
			psy_INDEX_INVALID) {
		temp = (int32_t)psy_audio_patterns_track_soloed(psy_audio_song_patterns(self->song));
	} else {
		temp = -1;
	}
	if ((status = psyfile_write(self->fp, &temp, sizeof(int32_t)))) {
	...
	/* seqbus */
	if (psy_audio_machines_selected(psy_audio_song_machines(self->song)) !=
			psy_INDEX_INVALID) {
		temp = (int32_t)psy_audio_machines_selected(psy_audio_song_machines(self->song));
	} else {
		temp = 0;
	}
	if ((status = psyfile_write(self->fp, &temp, sizeof(int32_t)))) {
	...
	/* instselected */
	if (psy_audio_instruments_selected(psy_audio_song_instruments(self->song)).subslot !=
			psy_INDEX_INVALID) {
		temp = (int32_t)
			psy_audio_instruments_selected(psy_audio_song_instruments(self->song)).subslot;
	} else {
		temp = -1;
	}
	if ((status = psyfile_write(self->fp, &temp, sizeof(int32_t)))) {
	...
	/* sequence width */
	if (psy_audio_sequence_width(psy_audio_song_sequence(self->song)) > 0) {
		temp = (int32_t)psy_audio_sequence_width(psy_audio_song_sequence(self->song));
	} else {
	...
	for (i = 0; i < psy_audio_song_patterns(self->song)->songtracks; ++i) {
		uint8_t temp8;

		temp8 = psy_audio_patterns_is_track_muted(psy_audio_song_patterns(self->song), i);
		if ((status = psyfile_write(self->fp, &temp8, sizeof(uint8_t)))) {
			return status;
		}
		 /* remember to count them */
		temp8 = psy_audio_patterns_is_track_armed(psy_audio_song_patterns(self->song), i);
		if ((status = psyfile_write(self->fp, &temp8, sizeof(uint8_t)))) {
			return status;
		}
	}
	...
	if ((status = psyfile_write_uint8(self->fp, 
			psy_audio_song_patterns(self->song)->sharetracknames) != 0)) {
		return status;
	}
	if (psy_audio_song_patterns(self->song)->sharetracknames) {
		uint32_t t;

		for(t = 0; t < psy_audio_song_num_song_tracks(self->song); ++t) {			
			if ((status = psyfile_writestring(self->fp, 
					psy_audio_patterns_track_name(
						psy_audio_song_patterns(self->song),
						psy_INDEX_INVALID, /* no pattern index */
						t)))) {
				return status;
			} 
		}
	...
	/* auto note off */
	if ((status = psyfile_write_uint32(self->fp, (uint32_t)
		(self->song->properties_.auto_note_off_ != 0)))) {
		return status;
	}
	if ((status = psyfile_updatesize(self->fp, sizepos, NULL))) {
		return status;
	...
	for (index = 0; index < psy_audio_sequence_width(psy_audio_song_sequence(self->song));
		...
		if ((status = psyfile_write_int32(self->fp,
				(int32_t)psy_audio_sequence_track_size(
					psy_audio_song_sequence(self->song), index)))) {
			return status;
		}
		/* sequence name */
		if ((status = psyfile_writestring(self->fp,
			(psy_audio_sequencetrack_name(track))
				? psy_audio_sequencetrack_name(track)
				: "seq0"))) {
			return status;
		}		
		...
		for (s = track->nodes; s != NULL; psy_list_next(&s)) {
			...
			if ((status = psyfile_write_float(self->fp, (float)
					psy_dsp_beatpos_real(sequenceentry->repositionoffset)))) {
				return status;
			}
		}
		...
		for (s = track->nodes; s != NULL; psy_list_next(&s)) {
			...
			if (seqentry->type == psy_audio_SEQUENCEENTRY_MARKER) {
				...
				if ((status = psyfile_write_float(self->fp, 
						(float)psy_dsp_beatpos_real(marker->length)))) {
					return status;
				}
		...
		if ((status = psyfile_write_float(self->fp, (float)
				psy_audio_sequencetrack_height(track)))) {
			return status;
		}
		if ((status = psyfile_updatesize(self->fp, sizepos, NULL))) {
			return status;
...
{	
	...
	it = psy_audio_patterns_begin(psy_audio_song_patterns(self->song));
	for (;	!psy_tableiterator_equal(&it, psy_table_end()); psy_tableiterator_inc(&it)) {
		...
		if (psy_audio_sequence_patternused(psy_audio_song_sequence(self->song), i) ||
			...
			pattern = psy_audio_patterns_at(psy_audio_song_patterns(self->song), i);
			...
			if(!psy_audio_song_patterns(self->song)->sharetracknames) {
				...
				for(t = 0; t < psy_audio_song_patterns(self->song)->songtracks; ++t) {
					psyfile_writestring(self->fp, psy_audio_pattern_track_name(pattern, t));
				}
			}
...
{	
	...
	if ((status = psyfile_write_uint32(self->fp,
			(uint32_t)pattern->time_sig_.cmd))) {
		return status;
	}
	if ((status = psyfile_write_uint32(self->fp,
			(uint32_t)pattern->time_sig_.parameter))) {
		return status;
	}
	/* ppq res */
	if ((status = psyfile_write_int32(self->fp,
			(int32_t)psy_dsp_DEFAULT_PPQ))) {
		return status;
	}
	/* length in ppq ticks */		
	if ((status = psyfile_write_int32(self->fp, psy_audio_pattern_length(pattern).ticks))) {
		return status;	
	}		
	/* num pattern entries */
	if ((status = psyfile_write_int32(self->fp,
			(int32_t)psy_list_size(pattern->events_)))) {
		return status;
	}	
	/* write events */
	for (node = psy_audio_pattern_begin(pattern); node != 0; node = node->next) {
		...
		if ((status = psyfile_write_int32(self->fp,
				(int32_t)psy_audio_patternentry_track(entry)))) {
			return status;
		}
		/* ppq-ticks */
		if ((status = psyfile_write_int32(self->fp, (int32_t)
				psy_audio_patternentry_offset(entry).ticks))) {
			return status;
		}
		/* num events */
		if ((status = psyfile_write_int32(self->fp,
				(int32_t)psy_list_size(psy_audio_patternentry_begin(entry))))) {
			return status;
		}				
		entry = (psy_audio_PatternEntry*) node->entry;				
...
int psy_audio_psy3saver_write_macd(psy_audio_PSY3Saver* self)
{
	...
	for (index = 0; index < MAX_MACHINES; ++index) {
		...
		machine = psy_audio_machines_at(psy_audio_song_machines(self->song), index);
		if (machine) {			
			uint32_t sizepos;
	...
	uint32_t slot)
{
	...
	if (info) {			
		...
		psyfile_writestring(self->fp, psy_audio_machine_edit_name(machine)
			? psy_audio_machine_edit_name(machine) : "");
		psy_audio_machine_save_specific(machine, self->songfile, slot);
		if ((status = psy_audio_machine_save_wire_mapping(machine, self->songfile, slot))) {
			return status;
		}
...
int psy_audio_psy3saver_write_connections(psy_audio_PSY3Saver* self, uintptr_t slot)
{
	...
	sockets = psy_audio_connections_at(&psy_audio_song_machines(self->song)->connections, slot);
	if ((status = psyfile_write_int32(self->fp,
	...
	for (c = 0; c <= maxkey; ++c) {
		...
		if ((status = psyfile_write_float(self->fp, (float)invol))) {
			return status;
		}
	...
	psy_audio_Machine* machine)
{
	...
	for (i = 0; i < 256; ++i) {
		...
		if (param < psy_audio_machine_num_parameters(machine)) {
			++numMaps;
		}
	}
	...
	psy_audio_Machine* machine)
{
	...
	isbus = psy_audio_machine_is_bus(machine) != FALSE;
	...
	psy_audio_Instrument* instrument, int32_t version)
{
	...
	if ((status = psyfile_write_float(self->fp,
			(float)instrument->volume_fade_speed))) {
		return status;
	}		
	...
	psy_dsp_Envelope* envelope, uint32_t version)
{
	...
	/* envelope num points */
	if ((status = psyfile_write_uint32(self->fp,
			(uint32_t)psy_dsp_envelope_num_of_points(envelope)))) {
		return status;
	}
	for (i = 0; i < psy_dsp_envelope_num_of_points(envelope); i++)
	{
		/* time in ms (int) */
		if ((status = psyfile_write_int32(self->fp,
...
int psy_audio_psy3saver_write_smsb(psy_audio_PSY3Saver* self)
{
	...
	for (it = psy_audio_samples_begin(psy_audio_song_samples(self->song));
			!psy_tableiterator_equal(&it, psy_table_end());
			psy_tableiterator_inc(&it)) {
...
{			
	...
	if (psy_audio_buffer_num_channels(&sample->channels) > 0) {
	...
	if (psy_audio_buffer_num_channels(&sample->channels) > 1) {
	...
	if ((status = psyfile_write_uint8(self->fp, (uint8_t)
			psy_audio_sample_stero(sample)))) {
		free(data1);
		free(wavedata_left);
		return status;
	}
	if ((status = psyfile_write_uint8(self->fp, (uint8_t)
		psy_audio_sample_pan_enabled(sample)))) {
		free(data1);
		free(wavedata_left);
		return status;
	}
	if ((status = psyfile_write_float(self->fp, (float)
			psy_audio_sample_panning(sample)))) {
		free(data1);
		free(wavedata_left);
		return status;
	}
	if ((status = psyfile_write_uint8(self->fp, (uint8_t)
			psy_audio_sample_surround(sample)))) {
		free(data1);
		free(wavedata_left);
		return status;
	}
...
int psy_audio_psy3saver_write_virg(psy_audio_PSY3Saver* self)
{
	...
	for (i = MAX_MACHINES; i < MAX_VIRTUALINSTS; i++) {
		...
		if ((machine = psy_audio_machines_at(psy_audio_song_machines(self->song), i))) {
			psy_audio_MachineParam* param;			
			
			param = psy_audio_machine_parameter(machine, 0);
			if (param) {				
				inst_idx = (int32_t)psy_audio_machine_parameter_scaled_value(machine, param);
			}
			param = psy_audio_machine_parameter(machine, 1);
			if (param) {
				mac_idx = (int32_t)psy_audio_machine_parameter_scaled_value(machine, param);
			}						
	...
	psy_audio_Instrument* instr)
{
	...
	for (ite = instr->entries; ite != NULL; psy_list_next(&ite)) {
		...
		if (!psy_list_find_entry(sampidxs, &entry->sampleindex)) {
	...
	*/
	for (ite = sampidxs; ite != NULL; psy_list_next(&ite)) {
		...
		assert(sampidx);
		sample = psy_audio_samples_at(psy_audio_song_samples(self->song), *sampidx);
		if (sample) {
			if ((status = psyfile_writeheader(self->fp, "SMSB",
