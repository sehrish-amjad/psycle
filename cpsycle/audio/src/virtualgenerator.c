...
static intptr_t mode(psy_audio_VirtualGenerator* self) { return psy_audio_MACHMODE_GENERATOR; }
	...
	if (machines) {
		...
		if (sampler) {			
			...
			*/
			...
			psy_audio_machine_seq_tick(sampler, channel, &realevent);			
...
uintptr_t buffermemorysize(psy_audio_VirtualGenerator* self)
{
	...
	if (machines) {
		...
		sampler = psy_audio_machines_at(machines, self->machine_index);
		if (sampler) {
			return psy_audio_machine_buffermemory_size(sampler);
		}
	}
	/* fallback to own buffermemory if sampler machine is not valid */
...
void setbuffermemorysize(psy_audio_VirtualGenerator* self, uintptr_t size)
{
	...
	if (machines) {
		...
		sampler = psy_audio_machines_at(machines, self->machine_index);
		if (sampler) {
			psy_audio_machine_set_buffermemory_size(sampler, size);
		}
	}
	/* fallback to own buffermemory if sampler machine is not valid */
