...
psy_audio_ActiveChannelMacs* psy_audio_activechannelmacs_alloc_init(void)
{
...
void psy_audio_activechannels_set_machines(psy_audio_ActiveChannels* self,
	...
	uintptr_t channel , psy_audio_PatternEvent* ev)
{
	...
	macs = psy_audio_activechannels_active_macs(self, channel);
	if (!macs) {
		macs = psy_audio_activechannelmacs_alloc_init();
		assert(macs);
...
bool psy_audio_activechannels_play_on(const psy_audio_ActiveChannels* self, uintptr_t channel)
{
	psy_audio_ActiveChannelMacs* macs;

	assert(self);

	macs = psy_audio_activechannels_active_macs((psy_audio_ActiveChannels*)self, channel);
	if (macs) {
...
psy_audio_ActiveChannelMacs* psy_audio_activechannels_active_macs(
