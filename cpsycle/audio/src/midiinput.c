...
/* prototypes */
static void psy_audio_setcontrollermap(psy_audio_MidiInput*, intptr_t channel,
	intptr_t controller, intptr_t parameter);
static intptr_t psy_audio_getcontrollermap(psy_audio_MidiInput*, intptr_t channel,
	intptr_t controller);
...
{	
	...
	psy_audio_midiconfig_init(&self->midiconfig, cfg);
	...
	psy_audio_PatternEvent* rv)
{
	...
	case 0xB:
	{
		...
		case 0:
		{
			...
			if (self->midiconfig.gen_select_with == psy_audio_MIDICONFIG_MS_BANK) {
				...
				rv->cmd = (uint8_t)data1;
				rv->parameter = (uint8_t)data2;
			}
			break; }
		...
		case 0x7E:
		{
			self->channelsetting[channel] = (uint8_t)data2;
			return FALSE;
		}
		break;
		...
		default:
		{
			...
			intptr_t gParameter = psy_audio_getcontrollermap(self, channel, data1);
			...
			if (gParameter >= 0)
			{
				...
				machine = psy_audio_machines_at(psy_audio_song_machines(self->song),
					psy_audio_machines_selected(psy_audio_song_machines(self->song)));
				if (machine && inst < (int)psy_audio_machine_num_tweak_parameters(machine))
				{					
					psy_audio_MachineParam* param;

					param = psy_audio_machine_tweak_parameter(machine, inst);
					if (param) {
	...
	case 0xC:
	{
		...
		{
			/* machine active? */
			if (program < MAX_MACHINES && psy_audio_machines_at(psy_audio_song_machines(self->song),
				program))
			{
				/* ok, map */
	...
	if ((note >= psy_audio_NOTECOMMANDS_MIDI_SPP && note <= psy_audio_NOTECOMMANDS_MIDI_SYNC) ||
			psy_audio_machines_at(psy_audio_song_machines(self->song), busmachine)) {
		psy_audio_patternevent_init_all(rv, (uint8_t)note, (uint8_t)inst, (uint8_t)busmachine,
			psy_audio_NOTECOMMANDS_VOL_EMPTY, (uint8_t)cmd, (uint8_t)parameter);
		return TRUE;
	} else {
		psy_audio_patternevent_init_all(rv, (uint8_t)note, (uint8_t)inst, (uint8_t)busmachine,
			psy_audio_NOTECOMMANDS_VOL_EMPTY, (uint8_t)cmd, (uint8_t)parameter);
		...
		case psy_audio_MIDICONFIG_MS_USE_SELECTED:
			if (self->song) {
				if (psy_audio_machines_at(psy_audio_song_machines(self->song),
						psy_audio_machines_selected(psy_audio_song_machines(self->song)))) {
					return psy_audio_machines_selected(psy_audio_song_machines(self->song));
				}
			}
			break;
		case psy_audio_MIDICONFIG_MS_MIDI_CHAN:
			if (self->song && psy_audio_machines_at(psy_audio_song_machines(self->song),
					channel)) {
				return channel;
			}
			break;
...
void psy_audio_setcontrollermap(psy_audio_MidiInput* self, intptr_t channel,
	intptr_t controller, intptr_t parameter)
{
	assert(channel < psy_audio_MAX_MIDI_CHANNELS);
...
intptr_t psy_audio_getcontrollermap(psy_audio_MidiInput* self,
	intptr_t channel, intptr_t controller)
{
	assert(channel < psy_audio_MAX_MIDI_CHANNELS);
	assert(controller < psy_audio_MAX_CONTROLLERS);
