	...
	uintptr_t lpb, int* rv_lines)
{
	...
	/* alloc pattern data */
	lines = (int32_t)(psy_dsp_beatpos_real(psy_audio_pattern_length(pattern)) * lpb + 0.5);
	*rv_lines = lines;
	...
	for (node = psy_audio_pattern_begin(pattern); node != 0; node = node->next) {
		...
		row = (int32_t)(psy_dsp_beatpos_real(psy_audio_patternentry_offset(entry)) * lpb);
		track = (int32_t)psy_audio_patternentry_track(entry);
	...
	uint32_t lpb)
{
	...
	for (row = 0; row < numrows; ++row) {
		...
		for (track = 0; track < numtracks; ++track) {
			...
			if (!empty) {
				...
				node = psy_audio_pattern_insert(dst, node, track, 
					psy_dsp_beatpos_make_lines(row, lpb, psy_dsp_DEFAULT_PPQ),
					&entry);
	...
	dst->length_ = psy_dsp_beatpos_make_lines(numrows, lpb,
		psy_dsp_DEFAULT_PPQ);	
}
...
/* ModToVirtual */
void modtovirtual_init(ModToVirtual* self)
{
	psy_table_init(&self->table);	
}

void modtovirtual_dispose(ModToVirtual* self)
{
	psy_table_dispose(&self->table);
}

void modtovirtual_clear(ModToVirtual* self)
{
	psy_table_clear(&self->table);
}

void modtovirtual_set(ModToVirtual* self, intptr_t mod,
	intptr_t virtual_inst)
{
	psy_table_insert(&self->table, mod, (void*)virtual_inst);
}

intptr_t modtovirtual_at(const ModToVirtual* self, intptr_t mod)
{
	return (intptr_t)psy_table_at(&((ModToVirtual*)self)->table,
		mod);
}
...
psy_audio_LegacyInstrument psy_audio_legacyinstrument(const psy_audio_Instrument* instrument)
{
	...
	rv.ENV_DT = (int32_t)((psy_dsp_envelope_decay_time(
		&instrument->volumeenvelope)) * 44100 + 0.5f);	
	rv.ENV_SL = (int32_t)(psy_dsp_envelope_sustain_value(
		&instrument->volumeenvelope) * 100);
	rv.ENV_RT = (int32_t)((psy_dsp_envelope_release_time(
		&instrument->volumeenvelope)) * 44100 + 0.5f);
	/* filter envelope */
	rv.ENV_F_AT = (int32_t)(psy_dsp_envelope_attacktime(
		&instrument->filterenvelope) * 44100 + 0.5f);
	rv.ENV_F_DT = (int32_t)(psy_dsp_envelope_decay_time(
		&instrument->filterenvelope) * 44100 + 0.5f);
	rv.ENV_F_SL = (int32_t)(psy_dsp_envelope_sustain_value(
		&instrument->filterenvelope) * 128);
	rv.ENV_F_RT = (int32_t)(psy_dsp_envelope_release_time(
		&instrument->filterenvelope) * 44100 + 0.5f);
...
{		
	...
	*/
	...
	psy_dsp_envelope_set_adsr(&instrument->volumeenvelope,				
		ENV_AT * 1.f / 44100,
		ENV_DT * 1.f / 44100,
		ENV_SL / 100.f,
		ENV_RT * 1.f / 44100);
	...
	*/	
	...
	psy_dsp_envelope_set_adsr(&instrument->filterenvelope,
		ENV_F_AT * 1.f / 44100,
		ENV_F_DT * 1.f / 44100,
