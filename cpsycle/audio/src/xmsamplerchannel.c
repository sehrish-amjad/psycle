...
static void psy_audio_xmsamplerchannel_filtertype_norm_value(psy_audio_XMSamplerChannel*,
	psy_audio_ChoiceMachineParam* sender, double* rv);
...
static void psy_audio_xmsamplerchannel_defaultpan_norm_value(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double* rv);
static void psy_audio_xmsamplerchannel_slider_tweak(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double value);
static void psy_audio_xmsamplerchannel_slider_norm_value(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double* rv);
static void psy_audio_xmsamplerchannel_defaultismute_tweak(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double value);
static void psy_audio_xmsamplerchannel_defaultismute_norm_value(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double* rv);
static void psy_audio_xmsamplerchannel_defaultissurround_tweak(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double value);
static void psy_audio_xmsamplerchannel_defaultissurround_norm_value(psy_audio_XMSamplerChannel*,
	psy_audio_IntMachineParam* sender, double* rv);
...
void psy_audio_xmsamplerchannel_initparamview(psy_audio_XMSamplerChannel* self)
{
	...
	for (ft = 0; ft < filter_numfilters(); ++ft) {
		psy_audio_choicemachineparam_set_description(&self->param_filtertype,
			ft, filter_name(ft));
	}
	...
	psy_signal_connect(&self->param_filtertype.machineparam.signal_normvalue, self,
		psy_audio_xmsamplerchannel_filtertype_norm_value);
	...
	psy_signal_connect(&self->pan.machineparam.signal_normvalue, self,
		psy_audio_xmsamplerchannel_defaultpan_norm_value);
	...
	psy_signal_connect(&self->surround.machineparam.signal_normvalue, self,
		psy_audio_xmsamplerchannel_defaultissurround_norm_value);
	...
	psy_signal_connect(&self->mute.machineparam.signal_normvalue, self,
		psy_audio_xmsamplerchannel_defaultismute_norm_value);
	...
	psy_signal_connect(&self->slider_param.machineparam.signal_normvalue, self,
		psy_audio_xmsamplerchannel_slider_norm_value);	
}
	...
	struct psy_audio_XMSamplerVoice* pVoice)
{
	if (self->m_pForegroundVoice)
	{
		...
		if (psy_dsp_envelope_is_enabled(env) && psy_dsp_envelope_is_carry(env))
		...
		case XM_SAMPLER_CMD_SET_ENV_POSITION: {
			...
			if (psy_dsp_envelope_is_enabled(env)) {
				xmenvelopecontroller_setpositioninsamples(
					...
					(int)(parameter * psy_audio_machine_samples_per_tick(
						psy_audio_xmsampler_base(self->m_pSampler))));
			}			
			break; }
		case XM_SAMPLER_CMD_EXTENDED:
...
void psy_audio_xmsamplerchannel_performfx(psy_audio_XMSamplerChannel* self)
{
	...
	if (psy_audio_xmsamplerchannel_foregroundvoice(self)) // Effects that need a voice to be active.
	{
		...
		if (psy_audio_xmsamplerchannel_effectflags(self) & XM_SAMPLER_EFFECT_RETRIG)
		{
			...
			if (psy_dsp_envelope_is_enabled(
			...
			if (note != psy_audio_NOTECOMMANDS_RELEASE) {
 				if (psy_audio_xmsamplerchannel_foregroundvoice(self)) {
					psy_audio_xmsamplerchannel_foregroundvoice(self)->m_Slide2NoteDestPeriod =
						(int)psy_audio_xmsamplervoice_notetoperiod(
							psy_audio_xmsamplerchannel_foregroundvoice(self), note, TRUE);
				}
...
void psy_audio_xmsamplerchannel_filtertype_norm_value(psy_audio_XMSamplerChannel* self,
	psy_audio_ChoiceMachineParam* sender, double* rv)
{
	*rv = (double)psy_audio_xmsamplerchannel_defaultfiltertype(self) /
		((double)filter_numfilters() - 1);
}
...
void psy_audio_xmsamplerchannel_defaultpan_norm_value(psy_audio_XMSamplerChannel* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
	if (self->m_pSampler->channeldisplay == 0) {
...
void psy_audio_xmsamplerchannel_slider_norm_value(psy_audio_XMSamplerChannel* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
	if (self->m_pSampler->channeldisplay == 0) {
...
void psy_audio_xmsamplerchannel_defaultismute_norm_value(
	psy_audio_XMSamplerChannel* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
...
void psy_audio_xmsamplerchannel_defaultissurround_norm_value(
	psy_audio_XMSamplerChannel* self,
	psy_audio_IntMachineParam* sender, double* rv)
{
