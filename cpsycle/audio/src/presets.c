...
/* prototypes */
static intptr_t psy_audio_presets_comp_name(psy_audio_Preset* p,
	psy_audio_Preset* q);
...
{	
	...
	psy_table_init(&self->container_);
}
...
void psy_audio_presets_dispose(psy_audio_Presets* self)
{
	...
	psy_table_dispose_all(&self->container_, (psy_fp_disposefunc)
...
psy_audio_Presets* psy_audio_presets_alloc_init(void)
{
...
{		
	...
	psy_table_insert(&self->container_,
		(psy_table_empty(&self->container_))
		? 0
		: psy_table_maxkey(&self->container_) + 1, preset);	
}
	...
	psy_audio_Preset* preset)
{
	...
	oldpreset = psy_table_at(&self->container_, index);
	if (oldpreset) {
		psy_audio_preset_dispose(oldpreset);
	}
	psy_table_insert(&self->container_, index, preset);
}
...
void psy_audio_presets_remove(psy_audio_Presets* self, uintptr_t index)
{
	...
	oldpreset = psy_table_at(&self->container_, index);
	psy_table_remove(&self->container_, index);
	if (oldpreset) {
		psy_audio_preset_dispose(oldpreset);
	}	
}
...
void psy_audio_presets_remove_empty(psy_audio_Presets* self)
{
	...
	psy_table_clear(&self->container_);
	for (p = list, i = 0; p != NULL; p = p->next, ++i) {		
		psy_table_insert(&self->container_, i, (psy_audio_Preset*)p->entry);
	}
	psy_list_free(list);
	list = NULL;
}
...
void psy_audio_presets_merge(psy_audio_Presets* self, psy_audio_Presets* src)
{
	...
	psy_table_clear(&self->container_);
	psy_table_clear(&src->container_);
	for (p = list, i = 0; p != NULL; p = p->next, ++i) {		
		psy_table_insert(&self->container_, i, (psy_audio_Preset*)p->entry);
	}
	psy_list_free(list);
...
void psy_audio_presets_sort(psy_audio_Presets* self)
{
	...
	if (psy_table_size(&self->container_) <= 1) {
		return;
	}
	i = psy_table_maxkey(&self->container_);	
	psy_qsort(&self->container_,
...
psy_TableIterator psy_audio_presets_begin(psy_audio_Presets* self)
{
	...
	return psy_table_begin(&self->container_);
}

intptr_t psy_audio_presets_comp_name(psy_audio_Preset* p,
	psy_audio_Preset* q)
{
	const char* left;
	const char* right;
