...
int psy2loader_read_patterns(PSY2Loader* self)
{
	...
	for (i = 0; i < num; ++i) {
		...
		if (numlines > 0)
		{
			...
			psy_audio_patterns_insert(psy_audio_song_patterns(self->song), i, pattern);
			...
			pattern = psy_audio_pattern_alloc_init();
			psy_audio_patterns_insert(psy_audio_song_patterns(self->song), i, pattern);
			pattern->length_ = psy_dsp_beatpos_make_real(
		...
		for (w = 0; w < OLD_MAX_WAVES; ++w) {
			...
			if (numframes > 0) {
				if (w == 0) {
					...
					psy_audio_instrument_set_name(instrument, names[i]);
...
{	
		...
		if (self->_machineActive[i]) {
			...
			if (internalmachinesconvert_pluginname_exists(&self->converter, type, "")) {
				...
				case psy_audio_MASTER:
					self->pMac[i] = psy_audio_machinefactory_make_machine(
				...
				case psy_audio_PLUGIN: {
					...
					if (internalmachinesconvert_pluginname_exists(&self->converter, type, sDllName)) {
						...
						/* todo shellidx */
						psy_audio_plugin_name(sDllName, plugincatchername, 0);
						self->pMac[i] = psy_audio_machinefactory_make_machine(
						...
						if (self->pMac[i]) {
							...
							psy_audio_machine_set_slot(self->pMac[i], i);
							...
							psy_audio_songfile_warn(self->songfile, psy_audio_machine_edit_name(self->pMac[i]));
				...
				case psy_audio_VSTFX: {				
					...
					*/
					...
					if ((!self->vstL[instance].valid)) {
						...
						psy_audio_plugin_name(
							self->vstL[instance].dllName, plugincatchername, shellIdx);
						pVstPlugin = psy_audio_machinefactory_make_machine(
							factory, psy_audio_PLUGIN, plugincatchername, psy_INDEX_INVALID);
						if (pVstPlugin) {
							int c;
							int numpars;

							psy_audio_machine_set_curr_program(pVstPlugin, program);
							psy_audio_machine_set_slot(pVstPlugin, i);
							numpars = self->vstL[instance].numpars;
							for (c = 0; c < numpars; ++c) {
								psy_audio_MachineParam* param;

								param = psy_audio_machine_tweak_parameter(pVstPlugin, c);
								if (param) {
					...
					if (berror) {
						self->pMac[i] = psy_audio_machinefactory_make_machine(factory, psy_audio_DUMMY, plugincatchername, psy_INDEX_INVALID);
						psy_audio_machine_set_slot(self->pMac[i], i);
						psy_audio_machine_set_edit_name(self->pMac[i], plugincatchername);
						/*
						** todo set mode
				...
				case psy_audio_DUMMY:
					self->pMac[i] = psy_audio_machinefactory_make_machine(factory,
			...
			assert(self->pMac[i]);
			if (self->pMac[i]) {
				psy_audio_machine_set_position(self->pMac[i], x, y);
			}
		}
	}	
	...
	for (i = 0; i < OLD_MAX_INSTRUMENTS; ++i) {
		...
		instr = psy_audio_instruments_at(psy_audio_song_instruments(self->song),
		...
		*/
		...
		for (i = 0; i < 128; i++)
		{
			...
			{				
				...
				psy_audio_machines_insert(psy_audio_song_machines(self->song), invmach[i], self->pMac[i]);
			}
		}
		/*
		...
		*/
		for (i = 0; i < 128; i++)
		{
			if (self->_machineActive[i])
			{
				...
				{
					while (psy_audio_machines_at(psy_audio_song_machines(self->song), j) && j < 64) j++;

					invmach[i] = j;
					psy_audio_machines_insert(psy_audio_song_machines(self->song), j,
						self->pMac[i]);
					/* _pMachine[j] = pMac[i]; */
				} else
				{
					while (psy_audio_machines_at(psy_audio_song_machines(self->song), k) && k < 128) k++;
					invmach[i] = k;
					/* _pMachine[k] = pMac[i]; */
					psy_audio_machines_insert(psy_audio_song_machines(self->song), k,
						self->pMac[i]);
				}
			}
		}
		
		{
			...
			for (it = psy_audio_legacywires_begin(&self->legacywires);
				...
				for (j = psy_table_begin(wiresremapped);
					...
					if (wireremapped->input_con && wireremapped->input_machine > -1 && wireremapped->input_machine < 128
						...
						psy_audio_connections_set_wire_volume(&psy_audio_song_machines(self->song)->connections,
							psy_audio_wire_make(wireremapped->input_machine, macremapped_id),
							val * wireremapped->wire_multiplier);
						psy_audio_connections_setpinmapping(&psy_audio_song_machines(self->song)->connections,
							psy_audio_wire_make(wireremapped->input_machine, macremapped_id),
		...
		uintptr_t slot)
{
	...
	psy_audio_machine_set_panning(master, 0.5f);
	...
	}
	{
		...
		param = psy_audio_machine_tweak_parameter(master, 0);
		if (param) {
	...
	uintptr_t slot)
{
	...
	/* Num Voices */
	param = psy_audio_machine_tweak_parameter(machine, 0);
	if (param) {
		psy_audio_machine_parameter_tweak_scaled(machine, param, numvoices);
	}
	/* Quality */
	param = psy_audio_machine_tweak_parameter(machine, 1);
	if (param) {
		psy_audio_machine_parameter_tweak_scaled(machine, param, interpol);
	}
	/* Default Speed (C3) */
	param = psy_audio_machine_tweak_parameter(machine, 2);
	if (param) {
	...
	
	{
		...
		*/
		...
		if (plugincatchername && strcmp(plugincatchername,
			...
			size = (uint32_t)psy_audio_machine_data_size(machine);
		}
#else
	size = (uint32_t)psy_audio_machine_data_size(machine);
#endif
		...
		if (size)
		{
			...
			psy_audio_machine_put_data(machine, pData); /* Internal load */
			free(pData);
			pData = 0;
		}
	}	
	...
	uintptr_t macid)
{
	...
	for (c = 0; c < PSY2_MAX_CONNECTIONS; ++c) {
		psy_audio_machinewires_insert(rv, c,
			psy_audio_legacywire_alloc_init());
	}
	/*
	** Read connections
