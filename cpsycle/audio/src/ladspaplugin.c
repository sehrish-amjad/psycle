	...
	uintptr_t shellidx)
{
	...
	vtable_init(self);	
	psy_audio_machine_set_callback(&self->custommachine.machine,
		callback);
	...
	if (!pfDescriptorFunction) {
		...
		if (psDescriptor) {
			...
			if (handle) {
				...
				psy_audio_machine_set_edit_name(psy_audio_ladspaplugin_base(self),
	...
	psy_audio_BufferContext* bc)
{
	...
	psy_audio_ladspainterface_run(&self->mi, (uint32_t)psy_audio_buffercontext_num_samples(bc));
}
			...
			if (LADSPA_IS_PORT_INPUT(iPortDescriptor)) {
				...
				if (indexoutput < 2) {
					self->psDescriptor->connect_port(self->handle, lPortIndex,
						psy_audio_buffer_at(bc->output_, indexoutput));
				}
				indexoutput++;
			}
		}
	...
	psy_audio_SongFile* songfile, uintptr_t slot)
{
	...
	count = (uint32_t)psy_audio_machine_num_parameters(psy_audio_ladspaplugin_base(self));
