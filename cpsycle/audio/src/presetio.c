...
int psy_audio_presetsio_load(const char* path, psy_audio_Presets* presets,
...
{	
	...
	if (!fp) {
		...
		psy_path_init(&underscorepath, path);
...
int psy_audio_presetsio_saveversion1(FILE* fp, psy_audio_Presets* presets)
{
	...
	numpresets = (int32_t)((psy_table_empty(&presets->container_))
		? 0
		: psy_table_maxkey(&presets->container_) + 1);
...
{		
	...
	if (preset->datasize > 0) {
		...
		temp32 = (int32_t)psy_audio_preset_num_parameters(preset);
	}
	fwrite(&temp32, sizeof(temp32), 1, fp);
	...
	if (preset->datasize > 0) {
		...
		for (param = 0; param < psy_audio_preset_num_parameters(preset); ++param) {
			float value;
