...
static void luapluginparam_tweak(psy_audio_LuaPluginMachineParam*, double val);
static int luapluginparam_describe(psy_audio_LuaPluginMachineParam*, char* text);
static double luapluginparam_norm_value(psy_audio_LuaPluginMachineParam*);
...
static void luapluginparam_vtable_init(psy_audio_LuaPluginMachineParam* self)
{
	if (!luapluginparam_vtable_initialized) {
		...
		luapluginparam_vtable.normvalue = (fp_machineparam_norm_value)luapluginparam_norm_value;
...
void luapluginparam_tweak(psy_audio_LuaPluginMachineParam* self, double value)
{
...
double luapluginparam_norm_value(psy_audio_LuaPluginMachineParam* self)
{
	psy_LuaImport in;
	lua_Number v;
...
{  
	...
	*/
	machineinfo_set(rv, vendor, "", 0, mode, name, name, (short) apiversion, 
		(short) version, psy_audio_LUA, "", 0, "", "Lua", "unknown",
		psy_INDEX_INVALID, "");
	free(name);
	free(vendor);
	return err;
...
int psy_audio_luaplugin_init(psy_audio_LuaPlugin* self,
        ...
        psy_Logger* logger)
{
	...
	psyclescript_init(&self->script, logger);
	...
	psy_audio_plugin_luascript_exportcmodules(&self->script);
        if (callback) {
            callback->vtable->custom_setup(callback, 1, &self->script);
        }
	...
	self->usenoteon = self->plugininfo->flags != 0;
	luaplugin_initmachine(self);	
	psy_audio_machine_set_edit_name(psy_audio_luaplugin_base(self),
...
{	
	if (!psyclescript_empty(&self->script)) {
		...
		dispose(self);
		psy_audio_luaplugin_init(self, mcb, path, self->script.logger);
	}	
}

int psy_audio_luaplugin_test(const char* path, psy_audio_MachineInfo*
	machineinfo, psy_Logger* logger)
{		
	psy_PsycleScript script;
	int err = 0;	
	
	psyclescript_init(&script, logger);
	...
	if (err != 0) {
		...
		psy_audio_luaplugin_init(&plugin, NULL, path, logger);
	...
	machineinfo_set_path(machineinfo, path);
	if (err) {		
		return 0;
	}	
...
void generateaudio(psy_audio_LuaPlugin* self, psy_audio_BufferContext* bc)
{
	if (psy_audio_buffercontext_num_samples(bc) > 0) {
		...
		if (psy_luaimport_open(&in, "work")) {
			...
			self->client->bc = bc;
			lua_pushinteger(self->script.L, psy_audio_buffercontext_num_samples(bc));
			psy_luaimport_pcall(&in, 0);
			if (in.lasterr == 0) {
...
static int psy_audio_luamachine_createparams(lua_State* L,
	psy_audio_LuaMachine* machine);
static int psy_audio_luamachine_createparam(lua_State* L, int idx,
	psy_audio_LuaMachine* machine);
static void luamachine_load(psy_audio_LuaMachine* machine, const char* name,
...
{	
	...
	if (n == 2) {
		...
		if (lua_isnumber(L, 2)) {
			...
			ud = malloc(sizeof(psy_audio_LuaMachine));
			psy_audio_luamachine_init_machine(ud, NULL);
			luamachine_load(ud, plug_name, psy_audio_machine_machinefactory(
				psy_audio_luaplugin_base(host)));
			luamachine_newshareduserdata(L, luamachine_meta, ud, 1);
			if (!ud->machine) {				
			...
			// lua_newtable(L);
			// lua_setfield(L, -2, "params");
			psy_audio_luamachine_createparams(L, ud);
		}
	}
	return 1;
}

void luamachine_load(psy_audio_LuaMachine* self, const char* name,
	psy_audio_MachineFactory* factory)
{    
	if (!factory) {		
		return;
	}
	self->machine = psy_audio_machinefactory_make_machine(factory,
		psy_audio_PLUGIN, name, psy_INDEX_INVALID);	
}

int psy_audio_luamachine_createparams(lua_State* L,
	psy_audio_LuaMachine* machine)
{
	int idx;
	...
	lua_createtable(L, (int)luamachine_numparameters(machine), 0);
	for (idx = 0; idx < luamachine_numparameters(machine); ++idx) {
		psy_audio_luamachine_createparam(L, idx, machine);
...
int psy_audio_luamachine_createparam(lua_State* L, int idx,
	psy_audio_LuaMachine* machine)
{
	char id[1024];
...
/*int luamachine_setnorm(lua_state* l)
{
...
int luamachine_work(lua_State* L)
{
	...
	if (self->bc) {
		psy_audio_buffercontext_set_num_samples(self->bc, numsamples);
	...
	if (n == 2) {		
		...
		if (self) {
			...
			*udata = malloc(sizeof(psy_audio_Array));
			psy_audio_array_init_shared(*udata, self->bc->output_->samples[idx],
				psy_audio_buffercontext_num_samples(self->bc));
	...
	*/
	...
	for (i = 0; i < num; ++i) {		
		...
		if (psy_table_at_strhash(&ids, id)) {
			...
			param = psy_audio_machine_tweak_parameter(&self->custommachine.machine, i);
			if (param) {
				intptr_t minval;
				intptr_t maxval;
				double value;
				...
				value = ((maxval - minval) != 0.0)
					? (temp) / (double)(maxval - minval)
					: 0.0;
		...
		*/
		...
		for (i = 0; i < count; i++)
		{
			...
			if (param) {
				...
				temp = (int32_t) psy_audio_machineparam_scaled_value(param);
