...
** @brief Container of Machines, their buffer and connections.
*/
...
typedef struct psy_audio_Machines {	
	...
	psy_Signal signal_aux_changed;
	struct psy_audio_Machine* master;
...
void psy_audio_machines_exchange(psy_audio_Machines*, uintptr_t srcslot,
	uintptr_t dstslot);
uintptr_t psy_audio_machines_append(psy_audio_Machines*,
	struct psy_audio_Machine*);
struct psy_audio_Machine* psy_audio_machines_at(psy_audio_Machines*,
	uintptr_t slot);
const struct psy_audio_Machine* psy_audio_machines_at_const(
	const psy_audio_Machines*, uintptr_t index);
...
struct psy_audio_Buffer* psy_audio_machines_inputs(psy_audio_Machines*,
	uintptr_t slot);
struct psy_audio_Buffer* psy_audio_machines_outputs(psy_audio_Machines*,
	uintptr_t slot);
...
struct psy_audio_Machine* psy_audio_machines_selected_machine(
	psy_audio_Machines*);
const struct psy_audio_Machine* psy_audio_machines_selectedmachine_const(const
	psy_audio_Machines*);
...
void psy_audio_machines_insertmaster(psy_audio_Machines*,
	struct psy_audio_Machine*);
struct psy_audio_Machine* psy_audio_machines_master(psy_audio_Machines*);
...
bool psy_audio_machines_isvirtualgenerator(const psy_audio_Machines*,
	uintptr_t slot);
bool psy_audio_machines_is_mixer_send(const psy_audio_Machines* self, uintptr_t slot);
...
uintptr_t psy_audio_machines_instance_count(const psy_audio_Machines*,
	psy_audio_MachineType type);
uintptr_t psy_audio_machines_first_instance(const psy_audio_Machines*,
	psy_audio_MachineType type);

void psy_audio_machines_notify_aux_change(psy_audio_Machines*);
