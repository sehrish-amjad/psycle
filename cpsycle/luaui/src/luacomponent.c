...
static int psy_luaui_viewport_create(lua_State* L);
static int psy_luaui_viewport_gc(lua_State* L);

const char* psy_luaui_viewport_meta = "psygroupmeta";

int psy_luaui_viewport_open(lua_State* L)
{
	static const luaL_Reg pm_lib[] = {
		{ NULL, NULL }
	};
	static const luaL_Reg pm_meta[] = {
	  {"new", psy_luaui_viewport_create},
	  { "__gc", psy_luaui_viewport_gc },
	  {NULL, NULL}
	};
	luaL_newmetatable(L, psy_luaui_viewport_meta);
	luaL_setfuncs(L, pm_meta, 0);
	lua_pop(L, 1);
	luaL_newlib(L, pm_lib);
	return 1;
...
int psy_luaui_viewport_create(lua_State* L)
{
	psy_ui_Component** udata;
	int n = lua_gettop(L);
	...
	if (n == 1) {
		udata = (psy_ui_Component**)lua_newuserdata(L, sizeof(psy_ui_Component*));
		luaL_setmetatable(L, psy_luaui_viewport_meta);
	}
	else {
		luaL_error(L, "Wrong number of arguments");
	}
...
int psy_luaui_viewport_gc(lua_State* L)
{
	psy_ui_Component* ptr;

	ptr = *(psy_ui_Component**)luaL_checkudata(L, 1, psy_luaui_viewport_meta);
	return 0;
}
