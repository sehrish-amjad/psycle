...
void psy_path_set_path(psy_Path* self, const char* path)
{
	free(self->path);
	free(self->name);
	free(self->prefix);
...
void psy_path_set_ext(psy_Path* self, const char* ext)
{
	free(self->ext);
...
void psy_path_update(psy_Path* self)
{
	...
	self->path[0] = '\0';
	if (psy_path_has_prefix(self)) {
		strcat(self->path, self->prefix);
	}
	if (psy_path_has_name(self)) {
		if (psy_path_has_prefix(self)) {
			strcat(self->path, path_delim);
		}
		strcat(self->path, self->name);
	}
	if (psy_path_has_ext(self)) {
		strcat(self->path, ".");
		strcat(self->path, self->ext);		
	}
}

bool psy_path_has_prefix(const psy_Path* self)
{
	return (psy_strlen(self->prefix) != 0);
}

bool psy_path_has_ext(const psy_Path* self)
{
	return (psy_strlen(self->ext) != 0);	
}

bool psy_path_has_name(const psy_Path* self)
{
...
#else
	if (_fullpath(rv, path, _MAX_PATH) == NULL) {
		return FALSE;
	}			
#endif
	return TRUE;
